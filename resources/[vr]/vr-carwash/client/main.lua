                        ...
                        if Driver == PlayerPed then
                            if not washingVehicle then
                                ...
                                if IsControlJustPressed(0, 38) then
                                    TriggerServerEvent('vr-carwash:server:washCar')
                                end
                            else
...
RegisterNetEvent('vr-carwash:client:washCar')
AddEventHandler('vr-carwash:client:washCar', function()
    local PlayerPed = PlayerPedId()
    local PedVehicle = GetVehiclePedIsIn(PlayerPed)
    local Driver = GetPedInVehicleSeat(PedVehicle, -1)
    ...
    VRCore.Functions.Progressbar("search_cabin", "Vehicle is being washed ..", math.random(4000, 8000), false, true, {
        ...
        VRCore.Functions.Notify("Washing canceled ..", "error")
        washingVehicle = false
    end)
end)
