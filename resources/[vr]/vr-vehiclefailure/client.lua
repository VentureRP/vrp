...
RegisterNetEvent('vr-vehiclefailure:client:RepairVehicle')
AddEventHandler('vr-vehiclefailure:client:RepairVehicle', function()
...
function CleanVehicle(vehicle)
	...
	VRCore.Functions.Progressbar("cleaning_vehicle", "Cleaning the car...", math.random(10000, 20000), false, true, {
		disableMovement = true,
		disableCarMovement = true,
		disableMouse = false,
		disableCombat = true,
	}, {}, {}, {}, function() -- Done
		VRCore.Functions.Notify("Vehicle cleaned!")
		SetVehicleDirtLevel(vehicle, 0.1)
        SetVehicleUndriveable(vehicle, false)
		WashDecalsFromVehicle(vehicle, 1.0)
		TriggerServerEvent('vr-vehiclefailure:server:removewashingkit', vehicle)
		TriggerEvent('inventory:client:ItemBox', VRCore.Shared.Items["cleaningkit"], "remove")
...
RegisterNetEvent('vr-vehiclefailure:client:SyncWash')
AddEventHandler('vr-vehiclefailure:client:SyncWash', function(veh)
	SetVehicleDirtLevel(veh, 0.1)
	SetVehicleUndriveable(veh, false)
	WashDecalsFromVehicle(veh, 1.0)
end)

RegisterNetEvent('vr-vehiclefailure:client:CleanVehicle')
AddEventHandler('vr-vehiclefailure:client:CleanVehicle', function()
	local vehicle = VRCore.Functions.GetClosestVehicle()
...
RegisterNetEvent('vr-vehiclefailure:client:RepairVehicleFull')
AddEventHandler('vr-vehiclefailure:client:RepairVehicleFull', function()
	local vehicle = VRCore.Functions.GetClosestVehicle()
	if vehicle ~= nil and vehicle ~= 0 then
		local ped = PlayerPedId()
		local pos = GetEntityCoords(ped)
...
function RepairVehicleFull(vehicle)
	...
	VRCore.Functions.Progressbar("repair_vehicle", "Repairing vehicle..", math.random(20000, 30000), false, true, {
		disableMovement = true,
		disableCarMovement = true,
		disableMouse = false,
		disableCombat = true,
		...
		VRCore.Functions.Notify("Vehicle repaired!")
		...
		TriggerServerEvent('vr-vehiclefailure:removeItem', "advancedrepairkit")
	end, function() -- Cancel
		StopAnimTask(PlayerPedId(), "mini@repair", "fixing_a_player", 1.0)
		VRCore.Functions.Notify("Failed!", "error")
		if (IsBackEngine(GetEntityModel(vehicle))) then
...
function RepairVehicle(vehicle)
	...
	VRCore.Functions.Progressbar("repair_vehicle", "Repairing vehicle..", math.random(10000, 20000), false, true, {
		disableMovement = true,
		disableCarMovement = true,
		disableMouse = false,
		disableCombat = true,
		...
		TriggerServerEvent('vr-vehiclefailure:removeItem', "repairkit")
	end, function() -- Cancel
		StopAnimTask(PlayerPedId(), "mini@repair", "fixing_a_player", 1.0)
		VRCore.Functions.Notify("Failed!", "error")
		if (IsBackEngine(GetEntityModel(vehicle))) then
...
Citizen.CreateThread(function()
    while true do
        ...
        if showPro then
            VRCore.Functions.DrawText3D(pos.x, pos.y, pos.z, TimeLeft .. '~g~%')
        end
    end
end)
					...
					if cfg.sundayDriver and GetVehicleClass(vehicle) ~= 14 then -- Not for boats
						...
						if speed >= 1.0 then
							...
							if brake > 127 then
								...
								--exports['vr-vehicletuning']:SetVehicleStatus(GetVehicleNumberPlateText(vehicle), "brakes", exports['vr-vehicletuning']:GetVehicleStatus(GetVehicleNumberPlateText(vehicle), "brakes") - 0.01)
							end
...
function DamageRandomComponent()
	...
	--exports['vr-vehicletuning']:SetVehicleStatus(GetVehicleNumberPlateText(vehicle), randomComponent, exports['vr-vehicletuning']:GetVehicleStatus(GetVehicleNumberPlateText(vehicle), randomComponent) - randomDamage)
end
