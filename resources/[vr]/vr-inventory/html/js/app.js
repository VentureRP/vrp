...
$(document).on("click", ".weapon-attachments-back", function(e) {
    e.preventDefault();
    $("#vrcore-inventory").css({ display: "block" });
    $("#vrcore-inventory").animate({
            left: 0 + "vw",
        },
        200
    );
...
function FormatAttachmentInfo(data) {
    $.post(
        "https://vr-inventory/GetWeaponData",
        JSON.stringify({
            weapon: data.name,
...
function handleAttachmentDrag() {
    ...
    $(".weapon-attachments-remove").droppable({
        ...
        drop: function(event, ui) {
            $.post(
                "https://vr-inventory/RemoveAttachment",
                JSON.stringify({
...
$(document).on("click", "#weapon-attachments", function(e) {
    ...
    if (!Inventory.IsWeaponBlocked(ClickedItemData.name)) {
        ...
        $("#vrcore-inventory").animate({
            ...
            200,
            function() {
                $("#vrcore-inventory").css({ display: "none" });
            }
        );
        $(".weapon-attachments-container").animate({
                left: 0 + "vw",
...
$(document).on("click", ".CombineItem", function(e) {
    ...
    if (combineslotData.toData.combinable.anim != null) {
        $.post(
            "https://vr-inventory/combineWithAnim",
            JSON.stringify({
        ...
        $.post(
            "https://vr-inventory/combineItem",
            JSON.stringify({
...
function optionSwitch(
    ...
    $.post(
        "https://vr-inventory/SetInventoryData",
        JSON.stringify({
...
function swap($fromSlot, $toSlot, $fromInv, $toInv, $toAmount) {
    ...
    if (fromData !== undefined && fromData.amount >= $toAmount) {
        ...
        if (
            ...
            $.post("https://vr-inventory/PlayDropSound", JSON.stringify({}));
            $.post(
                "https://vr-inventory/SetInventoryData",
                JSON.stringify({
                    fromInventory: $fromInv.attr("data-inventory"),
            ...
            if (fromData.amount == $toAmount) {
                if (
                    ...
                    $.post(
                        "https://vr-inventory/getCombineItem",
...
function InventoryError($elinv, $elslot) {
    ...
    $.post("https://vr-inventory/PlayDropFail", JSON.stringify({}));
}
...
(() => {
    ...
    Inventory.Close = function() {
        ...
        $("#vrcore-inventory").fadeOut(300);
        $(".combine-option-container").hide();
        $(".item-slot").remove();
        if ($("#rob-money").length) {
            $("#rob-money").remove();
        }
        $.post("https://vr-inventory/CloseInventory", JSON.stringify({}));

        if (AttachmentScreenActive) {
            $("#vrcore-inventory").css({ left: "0vw" });
