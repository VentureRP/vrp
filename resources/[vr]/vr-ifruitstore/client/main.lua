                ...
                if dist < 1.0 then
                    ...
                    if dist < 0.6 then
                        if not requiredItemsShowed2 then
                            requiredItems = {
                                [1] = {name = VRCore.Shared.Items["advancedlockpick"]["name"], image = VRCore.Shared.Items["advancedlockpick"]["image"]},
                            }
                                    ...
                                    if Config.Locations["thermite"].isDone then 
                                        VRCore.Functions.TriggerCallback('VRCore:HasItem', function(hasItem)
                                            if hasItem then
                                                ...
                                                VRCore.Functions.Notify("You are missing an advanced lockpick", "error")
                                            end
                                        end, "advancedlockpick")
                                    else
                                        VRCore.Functions.Notify("Security is still active..", "error")
                                    end
                                else
                                    VRCore.Functions.Notify("Not enough Police", "error")
                                end
                            end
                        end
                    else
...
function lockpickDone(success)
    ...
    if success then
        ...
        if math.random(1, 100) <= 40 and IsWearingHandshoes() then
            TriggerServerEvent("evidence:server:CreateFingerDrop", pos)
            VRCore.Functions.Notify("You ripped your glove..")
        end
        if math.random(1, 100) <= 10 then
            TriggerServerEvent("VRCore:Server:RemoveItem", "advancedlockpick", 1)
            TriggerEvent('inventory:client:ItemBox', VRCore.Shared.Items["advancedlockpick"], "remove")
        end
    end
end
...
function GrabItem(spot)
    ...
    VRCore.Functions.Progressbar("grab_ifruititem", "Disconnect Item", 10000, false, true, {
        ...
        if not copsCalled then
            ...
            -- if Config.SmallBanks[closestBank]["alarm"] then
                TriggerServerEvent("vr-ifruitstore:server:callCops", streetLabel, pos)
                copsCalled = true
            -- end
        end
        ...
        TriggerServerEvent('vr-ifruitstore:server:setSpotState', "isDone", true, spot)
        TriggerServerEvent('vr-ifruitstore:server:setSpotState', "isBusy", false, spot)
        TriggerServerEvent('vr-ifruitstore:server:itemReward', spot)
        TriggerServerEvent('vr-ifruitstore:server:PoliceAlertMessage', 'People try to steal items at the iFruit Store', pos, true)
    end, function() -- Cancel
        StopAnimTask(PlayerPedId(), "anim@gangops@facility@servers@", "hotwire", 1.0)
        TriggerServerEvent('vr-ifruitstore:server:setSpotState', "isBusy", false, spot)
    ...
    if usingSafe then
        if won then
            if not Config.Locations["safe"].isDone then
                ...
                TriggerServerEvent("vr-ifruitstore:server:SafeReward")
                TriggerServerEvent("vr-ifruitstore:server:SetSafeStatus", "isBusy", false)
                TriggerServerEvent("vr-ifruitstore:server:SetSafeStatus", "isDone", false)
                takeAnim()
            end
        end
    end
...
AddEventHandler('VRCore:Client:OnPlayerLoaded', function()
    isLoggedIn = true
    TriggerServerEvent("vr-ifruitstore:server:LoadLocationList")
end)
...
RegisterNetEvent('vr-ifruitstore:client:LoadList')
AddEventHandler('vr-ifruitstore:client:LoadList', function(list)
    ...
    if #(pos - vector3(Config.Locations["thermite"].x, Config.Locations["thermite"].y,Config.Locations["thermite"].z)) < 1.0 then
        if CurrentCops >= 0 then
            ...
            if requiredItemsShowed then
                ...
                requiredItemsShowed = false
                TriggerEvent('inventory:client:requiredItems', requiredItems, false)
                TriggerServerEvent("vr-ifruitstore:server:SetThermiteStatus", "isBusy", true)
...
RegisterNetEvent('vr-ifruitstore:client:setSpotState')
AddEventHandler('vr-ifruitstore:client:setSpotState', function(stateType, state, spot)
...
RegisterNetEvent('vr-ifruitstore:client:SetSafeStatus')
AddEventHandler('vr-ifruitstore:client:SetSafeStatus', function(stateType, state)
    if stateType == "isBusy" then
...
RegisterNetEvent('vr-ifruitstore:client:SetThermiteStatus')
AddEventHandler('vr-ifruitstore:client:SetThermiteStatus', function(stateType, state)
    if stateType == "isBusy" then
...
RegisterNetEvent('vr-ifruitstore:client:PoliceAlertMessage')
AddEventHandler('vr-ifruitstore:client:PoliceAlertMessage', function(msg, coords, blip)
    if blip then
...
RegisterNUICallback('thermitefailed', function()
    ...
    TriggerServerEvent("vr-ifruitstore:server:SetThermiteStatus", "isBusy", false)
    TriggerServerEvent("VRCore:Server:RemoveItem", "thermite", 1)
    TriggerEvent('inventory:client:ItemBox', VRCore.Shared.Items["thermite"], "remove")
    local coords = GetEntityCoords(PlayerPedId())
    local randTime = math.random(10000, 15000)
    CreateFire(coords, randTime)

    TriggerServerEvent('vr-ifruitstore:server:PoliceAlertMessage', 'People try to steal items at the iFruit Store', coords, true)
end)

RegisterNUICallback('thermitesuccess', function()
    VRCore.Functions.Notify("The fuses are broken", "success")
    TriggerServerEvent("VRCore:Server:RemoveItem", "thermite", 1)
    local pos = GetEntityCoords(PlayerPedId())
    if #(pos - vector3(Config.Locations["thermite"].x, Config.Locations["thermite"].y,Config.Locations["thermite"].z)) < 1.0 then
        TriggerServerEvent("vr-ifruitstore:server:SetThermiteStatus", "isDone", true)
        TriggerServerEvent("vr-ifruitstore:server:SetThermiteStatus", "isBusy", false)
    end
end)
...
RegisterNetEvent('vr-ifruitstore:client:robberyCall')
AddEventHandler('vr-ifruitstore:client:robberyCall', function(streetLabel, coords)
