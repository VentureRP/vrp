...
RegisterNetEvent('vr-fitbit:use')
AddEventHandler('vr-fitbit:use', function()
  openWatch(true)
end)
...
RegisterNUICallback('setFoodWarning', function(data)
    ...
    TriggerServerEvent('vr-fitbit:server:setValue', 'food', foodValue)

    VRCore.Functions.Notify('Fitbit: Hunger warning set to '..foodValue..'%')
end)

RegisterNUICallback('setThirstWarning', function(data)
    local thirstValue = tonumber(data.value)

    TriggerServerEvent('vr-fitbit:server:setValue', 'thirst', thirstValue)

    VRCore.Functions.Notify('Fitbit: Thirst warning set to '..thirstValue..'%')
end)
...
Citizen.CreateThread(function()
    while true do
        ...
        if isLoggedIn then
            VRCore.Functions.TriggerCallback('vr-fitbit:server:HasFitbit', function(hasItem)
                if hasItem then
