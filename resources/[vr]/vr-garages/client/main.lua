...
AddEventHandler('VRCore:Client:OnPlayerLoaded', function()
    PlayerGang = VRCore.Functions.GetPlayerData().gang
end)

RegisterNetEvent('VRCore:Client:OnGangUpdate')
AddEventHandler('VRCore:Client:OnGangUpdate', function(gang)
    PlayerGang = gang
end)

RegisterNetEvent('vr-garages:client:setHouseGarage')
AddEventHandler('vr-garages:client:setHouseGarage', function(house, hasKey)
    currentHouseGarage = house
    hasGarageKey = hasKey
end)

RegisterNetEvent('vr-garages:client:houseGarageConfig')
AddEventHandler('vr-garages:client:houseGarageConfig', function(garageConfig)
    HouseGarages = garageConfig
end)

RegisterNetEvent('vr-garages:client:addHouseGarage')
AddEventHandler('vr-garages:client:addHouseGarage', function(house, garageInfo)
    HouseGarages[house] = garageInfo
end)

RegisterNetEvent('vr-garages:client:takeOutDepot')
AddEventHandler('vr-garages:client:takeOutDepot', function(vehicle)
    if OutsideVehicles ~= nil and next(OutsideVehicles) ~= nil then
        if OutsideVehicles[vehicle.plate] ~= nil then
            ...
            VRCore.Functions.SpawnVehicle(vehicle.vehicle, function(veh)
                VRCore.Functions.TriggerCallback('vr-garage:server:GetVehicleProperties', function(properties)
                    VRCore.Functions.SetVehicleProperties(veh, properties)
                    ...
                    if vehicle.plate ~= nil then
                        ...
                        TriggerServerEvent('vr-garages:server:UpdateOutsideVehicles', OutsideVehicles)
                    end
                    ...
                    TriggerServerEvent('vr-garage:server:updateVehicleState', 0, vehicle.plate, vehicle.garage)
                    VRCore.Functions.Notify("Vehicle Off:Engine " .. enginePercent .. "% Body: " .. bodyPercent.. "% Fuel: "..currentFuel.. "%", "primary", 4500)
...
function HouseGarage(house)
    VRCore.Functions.TriggerCallback("vr-garage:server:GetHouseVehicles", function(result)
        ped = PlayerPedId();
        MenuTitle = "Depot Vehicles :"
        ClearMenu()

        if result == nil then
            VRCore.Functions.Notify("You have no vehicles in your garage", "error", 5000)
            closeMenuFull()
        else
            ...
            for k, v in pairs(result) do
                ...
                Menu.addButton(VRCore.Shared.Vehicles[v.vehicle]["name"], "TakeOutGarageVehicle", v, v.state, " Motor: " .. enginePercent.."%", " Body: " .. bodyPercent.."%", " Fuel: "..currentFuel.."%")
            end
        end
...
function DepotList()
    VRCore.Functions.TriggerCallback("vr-garage:server:GetDepotVehicles", function(result)
        ped = PlayerPedId();
        MenuTitle = "Impounded Vehicles :"
        ClearMenu()

        if result == nil then
            VRCore.Functions.Notify("There are no vehicles in the Impound", "error", 5000)
            closeMenuFull()
        else
            ...
            for k, v in pairs(result) do
                ...
                Menu.addButton(VRCore.Shared.Vehicles[v.vehicle]["name"], "TakeOutDepotVehicle", v, v.state .. " ($"..v.depotprice..",-)", " Motor: " .. enginePercent.."%", " Body: " .. bodyPercent.."%", " Fuel: "..currentFuel.."%")
            end
        end
...
function VehicleList()
    VRCore.Functions.TriggerCallback("vr-garage:server:GetUserVehicles", function(result)
        ped = PlayerPedId();
        MenuTitle = "My Vehicles :"
        ClearMenu()

        if result == nil then
            VRCore.Functions.Notify("You have no vehicles in this garage", "error", 5000)
            closeMenuFull()
        else
            ...
            for k, v in pairs(result) do
                ...
                Menu.addButton(VRCore.Shared.Vehicles[v.vehicle]["name"], "TakeOutVehicle", v, v.state, " Motor: " .. enginePercent .. "%", " Body: " .. bodyPercent.. "%", " Fuel: "..currentFuel.. "%")
            end
        end
    ...
    VRCore.Functions.TriggerCallback("vr-garage:server:GetUserVehicles", function(result)
        ...
        if result == nil then
            ...
            for k, v in pairs(result) do
                ...
                Menu.addButton(VRCore.Shared.Vehicles[v.vehicle]["name"], "TakeOutGangVehicle", v, v.state, " Motor: " .. enginePercent .. "%", " Body: " .. bodyPercent.. "%", " Fuel: "..currentFuel.. "%")
            end
        end
...
function TakeOutVehicle(vehicle)
    ...
    elseif vehicle.state == "Out" then
        VRCore.Functions.Notify("Is your vehicle in the Depot", "error", 2500)
    elseif vehicle.state == "Impound" then
        VRCore.Functions.Notify("This vehicle was impounded by the Police", "error", 4000)
    end
end
...
function TakeOutDepotVehicle(vehicle)
    if vehicle.state == "Impound" then
        TriggerServerEvent("vr-garage:server:PayDepotPrice", vehicle)
        Citizen.Wait(1000)
    end
end
            ...
            if putDist <= 25 and IsPedInAnyVehicle(ped) then
                ...
                if putDist <= 1.5 then
                    ...
                    if IsControlJustPressed(0, 38) then
                        ...
                        VRCore.Functions.TriggerCallback('vr-garage:server:checkVehicleOwner', function(owned)
                            if owned then
                                ...
                                CheckPlayers(curVeh)
                                TriggerServerEvent('vr-garage:server:updateVehicleStatus', totalFuel, engineDamage, bodyDamage, plate, k)
                                TriggerServerEvent('vr-garage:server:updateVehicleState', 1, plate, k)
                              
                                if plate ~= nil then
                                    OutsideVehicles[plate] = veh
                                    TriggerServerEvent('vr-garages:server:UpdateOutsideVehicles', OutsideVehicles)
                                end
                                VRCore.Functions.Notify("Vehicle Parked In, "..Garages[k].label, "primary", 4500)
                            else
                                VRCore.Functions.Notify("Nobody owns this vehicle", "error", 3500)
                            end
                        end, plate)
                    end
                end
                            ...
                            VRCore.Functions.TriggerCallback('vr-garage:server:checkVehicleOwner', function(owned)
                                if owned then
                                    ...
                                    if DoesEntityExist(curVeh) then
                                        VRCore.Functions.Notify("The wasn't deleted, please check if is someone inside the car.", "error", 4500)
                                    else
                                    TriggerServerEvent('vr-garage:server:updateVehicleStatus', totalFuel, engineDamage, bodyDamage, plate, Name)
                                    TriggerServerEvent('vr-garage:server:updateVehicleState', 1, plate, Name)                                    
                                    if plate ~= nil then
                                        OutsideVehicles[plate] = veh
                                        TriggerServerEvent('vr-garages:server:UpdateOutsideVehicles', OutsideVehicles)
                                    end
                                    VRCore.Functions.Notify("Vehicle Parked In, "..GangGarages[Name].label, "primary", 4500)
                                end
                                else
                                    VRCore.Functions.Notify("Nobody owns this vehicle", "error", 3500)
                                end
                    ...
                    if takeDist < 2.0 then
                        ...
                        elseif IsPedInAnyVehicle(ped) then
                            ...
                            if IsControlJustPressed(0, 38) then
                                local curVeh = GetVehiclePedIsIn(ped)
                                local plate = GetVehicleNumberPlateText(curVeh)
                                VRCore.Functions.TriggerCallback('vr-garage:server:checkVehicleHouseOwner', function(owned)
                                    if owned then
                                        ...
                                        local totalFuel = exports['LegacyFuel']:GetFuel(curVeh)
                                            CheckPlayers(curVeh)
                                        if DoesEntityExist(curVeh) then
                                                VRCore.Functions.Notify("The Vehicle wasn't deleted, please check if is someone inside the car.", "error", 4500)
                                        else
                                        TriggerServerEvent('vr-garage:server:updateVehicleStatus', totalFuel, engineDamage, bodyDamage, plate, currentHouseGarage)
                                        TriggerServerEvent('vr-garage:server:updateVehicleState', 1, plate, currentHouseGarage)
                                        VRCore.Functions.DeleteVehicle(curVeh)
                                        if plate ~= nil then
                                            OutsideVehicles[plate] = veh
                                            TriggerServerEvent('vr-garages:server:UpdateOutsideVehicles', OutsideVehicles)
                                        end
                                        VRCore.Functions.Notify("Vehicle Parked In, "..HouseGarages[currentHouseGarage], "primary", 4500)
                                    end
                                    else
                                        VRCore.Functions.Notify("Nobody owns this vehicle", "error", 3500)
                                    end
