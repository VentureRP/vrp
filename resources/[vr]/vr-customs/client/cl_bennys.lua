...
local function saveVehicle()
	...
	local myCar = VRCore.Functions.GetVehicleProperties(veh)
...
function AttemptPurchase(type, upgradeLevel)
    ...
    TriggerServerEvent("vr-customs:attemptPurchase", type, upgradeLevel)
                    ...
                    if nearDefault then
                        if not isPlyInBennys then
                            ...
                            if IsControlJustReleased(1, 38) then
                                if (v.useJob and isAuthorized((VRCore.Functions.GetPlayerData().job.name), k)) or not v.useJob then
                                    TriggerEvent('event:control:bennys', k)
                                else
                                    VRCore.Functions.Notify("You are not authorized", "error")
                                end
                            end
                        else
                            disableControls()
...
--#[Event Handlers]#--
RegisterNetEvent("vr-customs:purchaseSuccessful")
AddEventHandler("vr-customs:purchaseSuccessful", function()
    isPurchaseSuccessful = true
    attemptingPurchase = false
    VRCore.Functions.Notify("Purchase Successful")
end)

RegisterNetEvent("vr-customs:purchaseFailed")
AddEventHandler("vr-customs:purchaseFailed", function()
    isPurchaseSuccessful = false
    attemptingPurchase = false
    VRCore.Functions.Notify("Not enough money", "error")
end)
