...
function DoDropOff()
	...
	if DoesEntityExist(deliveryPed) and not IsEntityDead(deliveryPed) then
		...
		if success then
			...
			Citizen.Wait(2000)
			VRCore.Functions.Notify('The delivery was on point, your GPS will be updated with the next drop off', 'success')
		else
			VRCore.Functions.Notify('The Drop Off Failed', 'error')
		end
...
AddEventHandler("oxydelivery:client", function()
	...
	while tasking do
		...
		if dstcheck < 40.0 and not pedCreated and (oxyVehicle == veh or dstcheck2 < 15.0) then
			...
			DeleteCreatedPed()
			CreateOxyPed()
			VRCore.Functions.Notify('You Are Close To The Drop Off Point')
		end
		toolong = toolong - 1
		if toolong < 0 then
			...
			tasking = false
			OxyRun = false
			VRCore.Functions.Notify('You Took Too Long', 'error')
		end
	...
	while true do
		...
		if OxyRun then
			if not DoesEntityExist(oxyVehicle) or GetVehicleEngineHealth(oxyVehicle) < 200.0 or GetVehicleBodyHealth(oxyVehicle) < 200.0 then
				OxyRun = false
				tasking = false
				VRCore.Functions.Notify("The dealer isn't giving you anymore locations due to the state of his car", "error")
			else
				if tasking then
					Citizen.Wait(30000)
				else
...
AddEventHandler("oxydelivery:startDealing", function()
	...
	firstdeal = true
	CreateOxyVehicle()
	VRCore.Functions.Notify('A Car Has Been Provided. Your GPS Will Be Updated With Locations Soon', 'success')
end)
