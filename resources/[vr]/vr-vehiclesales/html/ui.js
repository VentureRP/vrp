...
$(document).ready(function(){
    ...
    window.addEventListener('message', function(event){
        ...
        if (eventData.action == "sellVehicle") {
            ...
            VROccasions.setupSellContract(eventData)
        }

        if (eventData.action == "buyVehicle") {
            $('.buy-container').fadeIn(150);
            VROccasions.setupBuyContract(eventData, eventData.vehicleData)
        }
    });
});
...
$(document).on('keydown', function() {
    switch(event.keyCode) {
        case 27:
            ...
            $.post('https://vr-vehiclesales/close');
            break;
    }
});
...
$(document).on('click', '#sell-vehicle', function(){
    if ($('.vehicle-sell-price').val() != "") {
        if (!isNaN($('.vehicle-sell-price').val())) {
            $.post('https://vr-vehiclesales/sellVehicle', JSON.stringify({
            ...
            $.post('https://vr-vehiclesales/error', JSON.stringify({
...
$(document).on('click', '#buy-vehicle', function(){
    $.post('https://vr-vehiclesales/buyVehicle');
    $('.buy-container').fadeOut(100);
    $.post('https://vr-vehiclesales/close');
});

VROccasions.setupSellContract = function(data) {
...
VROccasions.setupBuyContract = function(data, vData) {
