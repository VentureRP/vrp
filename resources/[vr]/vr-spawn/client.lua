...
RegisterNetEvent('vr-spawn:client:openUI')
AddEventHandler('vr-spawn:client:openUI', function(value)
...
RegisterNUICallback('setCam', function(data)
    ...
    elseif type == "normal" then
        local campos = VR.Spawns[location].coords
...
RegisterNUICallback('chooseAppa', function(data)
    ...
    TriggerServerEvent('VRCore:Server:OnPlayerLoaded')
    TriggerEvent('VRCore:Client:OnPlayerLoaded')
...
RegisterNUICallback('spawnplayer', function(data)
    ...
    if type == "current" then
        ...
        if insideMeta.house ~= nil then
            ...
            TriggerEvent('vr-houses:client:LastLocationHouse', houseId)
        elseif insideMeta.apartment.apartmentType ~= nil or insideMeta.apartment.apartmentId ~= nil then
            local apartmentType = insideMeta.apartment.apartmentType
            local apartmentId = insideMeta.apartment.apartmentId
            TriggerEvent('vr-apartments:client:LastLocationHouse', apartmentType, apartmentId)
        end
        TriggerServerEvent('VRCore:Server:OnPlayerLoaded')
        TriggerEvent('VRCore:Client:OnPlayerLoaded')
        FreezeEntityPosition(ped, false)
    ...
    elseif type == "house" then
        ...
        DoScreenFadeOut(500)
        Citizen.Wait(2000)
        TriggerEvent('vr-houses:client:enterOwnedHouse', location)
        TriggerServerEvent('VRCore:Server:OnPlayerLoaded')
        TriggerEvent('VRCore:Client:OnPlayerLoaded')
        TriggerServerEvent('vr-houses:server:SetInsideMeta', 0, false)
        TriggerServerEvent('vr-apartments:server:SetInsideMeta', 0, 0, false)
        FreezeEntityPosition(ped, false)
        RenderScriptCams(false, true, 500, true, true)
        SetCamActive(cam, false)
        DestroyCam(cam, true)
        ...
        DoScreenFadeIn(250)
    elseif type == "normal" then
        local pos = VR.Spawns[location].coords
        SetDisplay(false)
        DoScreenFadeOut(500)
        Citizen.Wait(2000)
...
RegisterNetEvent('vr-spawn:client:setupSpawns')
AddEventHandler('vr-spawn:client:setupSpawns', function(cData, new, apps)
    if not new then
        VRCore.Functions.TriggerCallback('vr-spawn:server:getOwnedHouses', function(houses)
            local myHouses = {}
            if houses ~= nil then
                for i = 1, (#houses), 1 do
