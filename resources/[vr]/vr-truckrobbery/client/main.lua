...
RegisterNetEvent('vr-armoredtruckheist:client:911alert')
AddEventHandler('vr-armoredtruckheist:client:911alert', function()
	if PoliceAlert == 0 then
		...
		if street2 ~= nil then 
			...
			TriggerServerEvent("vr-armoredtruckheist:server:callCops", streetLabel, transCoords)
...
RegisterNetEvent('vr-armoredtruckheist:client:robberyCall')
AddEventHandler('vr-armoredtruckheist:client:robberyCall', function(streetLabel, coords)
    ...
    while true do
		...
		if MissionStart == 1 then
			...
			if dist <= 55.0  then
				...
				if warning == 0 then
				warning = 1
				VRCore.Functions.Notify("Get rid of the guards before you place the bomb.", "error")
				end
			...
			if dist <= 7 and BlownUp == 0 and PlayerJob.name ~= 'police' then
				...
				if IsControlPressed(0, 47) and GuardsDead == 1 then 
					...
					TriggerEvent("vr-armoredtruckheist:client:911alert")
					Citizen.Wait(500)
				end
			end
...
if IsVehicleStopped(transport) then
	if IsVehicleSeatFree(transport, -1) and IsVehicleSeatFree(transport, 0) and IsVehicleSeatFree(transport, 1) and GuardsDead == 1 then
		if not IsEntityInWater(PlayerPedId()) then
			...
			VRCore.Functions.Notify('The load will be detonated in '..TimeToBlow / 1000 ..' seconds.', "error")
			...
			BlownUp = 1
			lootable = 1
			VRCore.Functions.Notify('You can start collecting cash.', "success")
			RemoveBlip(TruckBlip)
		else
			VRCore.Functions.Notify('Get out of the water', "error")
		end
	else
		VRCore.Functions.Notify('The vehicle must be empty to place the load', "error")
	end
else
	VRCore.Functions.Notify('You cant rob a vehicle that is moving.', "error")
end
end
...
function TakingMoney()
	...
	FreezeEntityPosition(PlayerPedId(), true)
	VRCore.Functions.Notify('You are packing cash into a bag', "success")
