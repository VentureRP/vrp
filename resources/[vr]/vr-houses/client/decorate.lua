	...
	while true do
		...
		if DecoMode then
			...
			if dist > 50.0 then
				DisableEditMode()
				closeDecorateUI()
				VRCore.Functions.Notify("You have gone out of range!")
			end
		end
	end
end)
...
RegisterNetEvent("vr-houses:client:decorate")
AddEventHandler("vr-houses:client:decorate", function()
	Citizen.Wait(500)
	if inside then 
		if hasKey then 
			...
			VRCore.Functions.Notify("You must have the keys to the house!", "error")
		end
	else
...
RegisterNetEvent("vr-houses:server:sethousedecorations")
AddEventHandler("vr-houses:server:sethousedecorations", function(house, decorations)
...
RegisterNUICallback("buySelectedObject", function(data, cb)
    VRCore.Functions.TriggerCallback('vr-houses:server:buyFurniture', function(isSuccess)
        if isSuccess then
            SetNuiFocus(false, false)
            cursorEnabled = not cursorEnabled
            SaveDecorations()
...
function EnableEditMode()
	...
	CreateEditCamera()
	DecoMode = true
	TriggerEvent('vr-anticheat:client:ToggleDecorate', true)
end
...
function DisableEditMode()
	...
	peanut = false
	DecoMode = false
	TriggerEvent('vr-anticheat:client:ToggleDecorate', false)
end
...
function LoadDecorations(house)
	if Config.Houses[house].decorations == nil or next(Config.Houses[house].decorations) == nil then
		VRCore.Functions.TriggerCallback('vr-houses:server:getHouseDecorations', function(result)
...
function SaveDecorations()
	if closesthouse ~= nil then
		...
		TriggerServerEvent("vr-houses:server:savedecorations", closesthouse, ObjectList)
	end
end
...
function CheckMovementInput()
	...
	if IsControlJustReleased(0, 21) then -- Left Shift
		...
		VRCore.Functions.Notify("Speed is ".. tostring(speeds[curSpeed]))
	end
