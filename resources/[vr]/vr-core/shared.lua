...
VRShared.RandomStr = function(length) -- VRShared.RandomStr falls under GPL License here: [esxlicense]/LICENSE
	if length > 0 then
		return VRShared.RandomStr(length-1) .. StringCharset[math.random(1, #StringCharset)]
	else
		return ''
	end
end

VRShared.RandomInt = function(length)
	if length > 0 then
		return VRShared.RandomInt(length-1) .. NumberCharset[math.random(1, #NumberCharset)]
	else
		return ''
	end
end

VRShared.SplitStr = function(str, delimiter)
	local result = { }
	local from  = 1
