...
VRClothing.ResetItemTexture = function(obj, category) {
    ...
    $.post('https://vr-clothing/updateSkin', JSON.stringify({
...
$(document).on('click', '.clothing-menu-option-item-right', function(e){
    ...
    if (canChange) {
        if (hasTracker && clothingCategory == "accessory") {
            $.post('https://vr-clothing/TrackerError');
            return
        } else {
            if (clothingCategory == "model") {
                $(inputElem).val(newValue);
                $.post('https://vr-clothing/setCurrentPed', JSON.stringify({ped: newValue}), function(model){
                    $("#current-model").html("<p>"+model+"</p>")
                });
                canChange = true;
                VRClothing.ResetValues()
                ...
                if (buttonType == "item") {
                    VRClothing.ResetItemTexture(this, clothingCategory);
                }
            } else {
                if (buttonType == "item") {
...
$(document).on('change', '.item-number', function(){
    ...
    if (hasTracker && clothingCategory == "accessory") {
        ...
        if (clothingCategory == "accessory" && inputVal == 13) {
            ...
            $.post('https://vr-clothing/updateSkinOnInput', JSON.stringify({
                clothingType: clothingCategory,
...
$(document).on('click', '.clothing-menu-header-camera-btn', function(e){
    ...
    if (selectedCam == null) {
        ...
        $.post('https://vr-clothing/setupCam', JSON.stringify({
            value: camValue
        }));
        selectedCam = this;
    } else {
...
$(document).on('keydown', function() {
    switch(event.keyCode) {
        case 68: // D
            $.post('https://vr-clothing/rotateRight');
            break;
        case 65: // A
            $.post('https://vr-clothing/rotateLeft');
            break;
        case 38: // UP
            ChangeUp();
            break;
...
VRClothing.ToggleChange = function(bool) {
    canChange = bool;
}
...
$(document).ready(function(){
    window.addEventListener('message', function(event) {
        switch(event.data.action) {
            case "open":
                VRClothing.Open(event.data);
                break;
            case "close":
                VRClothing.Close();
                break;
            case "updateMax":
                VRClothing.SetMaxValues(event.data.maxValues);
                break;
            case "reloadMyOutfits":
                VRClothing.ReloadOutfits(event.data.outfits);
                break;
            case "toggleChange":
                VRClothing.ToggleChange(event.data.allow);
                break;
            case "ResetValues":
                VRClothing.ResetValues();
                break;
...
VRClothing.ReloadOutfits = function(outfits) {
...
$(document).on('click', "#save-menu", function(e){
    e.preventDefault();
    VRClothing.Close();
    $.post('https://vr-clothing/saveClothing');
});

$(document).on('click', "#cancel-menu", function(e){
    e.preventDefault();
    VRClothing.Close();
    $.post('https://vr-clothing/resetOutfit');
});

VRClothing.SetCurrentValues = function(clothingValues) {
...
VRClothing.Open = function(data) {
    ...
    VRClothing.SetMaxValues(data.maxValues);
    $(".clothing-menu-header").html("");
    VRClothing.SetCurrentValues(data.currentClothing);
...
$(document).on('click', '.clothing-menu-outfit-option-button', function(e){
    ...
    $.post('https://vr-clothing/selectOutfit', JSON.stringify({
...
$(document).on('click', '.clothing-menu-myOutfit-option-button-remove', function(e){
    ...
    $.post('https://vr-clothing/removeOutfit', JSON.stringify({
        outfitData: outfitData.skin,
        outfitName: outfitData.outfitname,
        outfitId: outfitData.outfitId,
    }));
...
VRClothing.Close = function() {
    $.post('https://vr-clothing/close');
...
VRClothing.SetMaxValues = function(maxValues) {
...
VRClothing.ResetValues = function() {
...
$(document).on('click', '#save-outfit-save', function(e){
    ...
    $.post('https://vr-clothing/saveOutfit', JSON.stringify({
...
$(document).on('click', '.change-camera-button', function(e){
    ...
    $.post('https://vr-clothing/rotateCam', JSON.stringify({
        type: rotationType
    }))
});
