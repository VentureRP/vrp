...
AddEventHandler('VRCore:Client:OnPlayerLoaded', function()
    TriggerServerEvent("vr-clothes:loadPlayerSkin")
                        ...
                        if dist < 2 then
                            if PlayerData.job.name == Config.ClothingRooms[k].requiredJob then
                                ...
                                if IsControlJustPressed(0, 38) then -- E
                                    ...
                                    gender = "male"
                                    if VRCore.Functions.GetPlayerData().charinfo.gender == 1 then gender = "female" end
                                    VRCore.Functions.TriggerCallback('vr-clothing:server:getOutfits', function(result)
                                        openMenu({
...
RegisterNetEvent('vr-clothing:client:openOutfitMenu')
AddEventHandler('vr-clothing:client:openOutfitMenu', function()
    VRCore.Functions.TriggerCallback('vr-clothing:server:getOutfits', function(result)
        openMenu({
...
RegisterNUICallback('selectOutfit', function(data)
    ...
    TriggerEvent('vr-clothing:client:loadOutfit', data)
end)
...
RegisterNetEvent('vr-clothing:client:openMenu')
AddEventHandler('vr-clothing:client:openMenu', function()
    customCamLocation = nil
    openMenu({
...
RegisterNUICallback('TrackerError', function()
    VRCore.Functions.Notify("You can't remove your ankle bracelet ..", "error")
end)
...
RegisterNUICallback('saveOutfit', function(data, cb)
    ...
    TriggerServerEvent('vr-clothes:saveOutfit', data.outfitName, model, skinData)
end)

RegisterNetEvent('vr-clothing:client:reloadOutfits')
AddEventHandler('vr-clothing:client:reloadOutfits', function(myOutfits)
    SendNUIMessage({
...
RegisterNUICallback('removeOutfit', function(data, cb)
    TriggerServerEvent('vr-clothing:server:removeOutfit', data.outfitName, data.outfitId)
    VRCore.Functions.Notify("You have deleted your"..data.outfitName.." outfit!")
end)
...
function SaveSkin()
	...
	TriggerServerEvent("vr-clothing:saveSkin", model, clothing)
end

RegisterNetEvent('vr-clothes:client:CreateFirstCharacter')
AddEventHandler('vr-clothes:client:CreateFirstCharacter', function()
...
RegisterNetEvent("vr-clothes:loadSkin")
AddEventHandler("vr-clothes:loadSkin", function(new, model, data)
    ...
    Citizen.CreateThread(function()
        ...
        TriggerEvent('vr-clothing:client:loadPlayerClothing', data, PlayerPedId())
    end)
end)

RegisterNetEvent('vr-clothing:client:loadPlayerClothing')
AddEventHandler('vr-clothing:client:loadPlayerClothing', function(data, ped)
...
RegisterNetEvent('vr-clothing:client:loadOutfit')
AddEventHandler('vr-clothing:client:loadOutfit', function(oData)
    local ped = PlayerPedId()
    ...
    if data["accessory"] ~= nil then
        if VRCore.Functions.GetPlayerData().metadata["tracker"] then
    ...
    if oData.outfitName ~= nil then
        VRCore.Functions.Notify("You have chosen "..oData.outfitName.."! Press Confirm to confirm outfit.")
    end
end)
...
RegisterNetEvent("vr-clothing:client:adjustfacewear")
AddEventHandler("vr-clothing:client:adjustfacewear",function(type)
    if VRCore.Functions.GetPlayerData().metadata["ishandcuffed"] then return end
