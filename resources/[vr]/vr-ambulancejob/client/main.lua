            ...
            if #(pos - vector3(Config.Locations["checking"].x, Config.Locations["checking"].y, Config.Locations["checking"].z)) < 1.5 then
                ...
                if IsControlJustReleased(0, 38) then
                    if doctorCount >= Config.MinimalDoctors then
                        ...
                        VRCore.Functions.Progressbar("hospital_checkin", "Checking in..", 2000, false, true, {
                            ...
                            if bedId ~= nil then 
                                ...
                                VRCore.Functions.Notify("Beds are occupied..", "error")
                            end
                        end, function() -- Cancel
                            TriggerEvent('animations:client:EmoteCommandStart', {"c"})
                            VRCore.Functions.Notify("Checking in failed!", "error")
                        end)
                    end
                end
...
AddEventHandler('hospital:client:Revive', function()
    ...
    VRCore.Functions.Notify("You are completely healthy again!")
end)
...
AddEventHandler('hospital:client:HealInjuries', function(type)
    ...
    TriggerServerEvent("hospital:server:RestoreWeaponDamage")
    VRCore.Functions.Notify("Your wounds have been healed!")
end)
...
AddEventHandler('hospital:client:SendToBed', function(id, data, isRevive)
    ...
    Citizen.CreateThread(function ()
        Citizen.Wait(5)
        if isRevive then
            VRCore.Functions.Notify("You are being helped..")
...
AddEventHandler('hospital:client:SendBillEmail', function(amount)
    SetTimeout(math.random(2500, 4000), function()
        ...
        local charinfo = VRCore.Functions.GetPlayerData().charinfo
...
function DoLimbAlert()
    if not isDead then
        if #injured > 0 then
            ...
            VRCore.Functions.Notify(limbDamageMsg, "primary", 5000)
        end
    end
end
...
function DoBleedAlert()
    if not isDead and tonumber(isBleeding) > 0 then
        VRCore.Functions.Notify("You are "..Config.BleedingStates[tonumber(isBleeding)].label, "error", 5000)
    end
end
...
function ResetAll()
    ...
    TriggerServerEvent("VRCore:Server:SetMetaData", "hunger", 100)
    TriggerServerEvent("VRCore:Server:SetMetaData", "thirst", 100)
end
