...
$(document).on('click', '.mail', function(e){
    ...
    VR.Phone.Functions.SetupMail(MailData);
...
$(document).on('click', '#accept-mail', function(e){
    ...
    $.post('https://vr-phone/AcceptMailButton', JSON.stringify({
...
$(document).on('click', '#remove-mail', function(e){
    e.preventDefault();
    var MailData = $("#"+OpenedMail).data('MailData');
    $.post('https://vr-phone/RemoveMail', JSON.stringify({
        mailId: MailData.mailid
    }));
    $(".mail-home").animate({
        left: 0+"vh"
...
VR.Phone.Functions.SetupMails = function(Mails) {
    ...
    $("#mail-header-mail").html(VR.Phone.Data.PlayerData.charinfo.firstname+"."+VR.Phone.Data.PlayerData.charinfo.lastname+"@vrcore.com");
...
VR.Phone.Functions.SetupMail = function(MailData) {
...
$(document).on('click', '#new-advert-submit', function(e){
    ...
    if (Advert !== "") {
        ...
        $.post('https://vr-phone/PostAdvert', JSON.stringify({
            message: Advert,
        }));
    } else {
        VR.Phone.Notifications.Add("fas fa-ad", "Advertisement", "You can\'t post an empty ad!", "#ff8f1a", 2000);
    }
});

VR.Phone.Functions.RefreshAdverts = function(Adverts) {
    $("#advert-header-name").html("@"+VR.Phone.Data.PlayerData.charinfo.firstname+""+VR.Phone.Data.PlayerData.charinfo.lastname+" | "+VR.Phone.Data.PlayerData.charinfo.phone);
