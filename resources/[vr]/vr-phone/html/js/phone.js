...
$(document).on('click', '.phone-app-footer-button', function(e){
    ...
    if (PressedFooterTab !== CurrentFooterTab) {
        ...
        if (PressedFooterTab == "recent") {
            $.post('https://vr-phone/ClearRecentAlerts');
...
VR.Phone.Functions.SetupRecentCalls = function(recentcalls) {
...
$(document).on('click', '.phone-recent-call', function(e){
    ...
    console.log(VR.Phone.Data.AnonymousCall)

    $.post('https://vr-phone/CallContact', JSON.stringify({
        ContactData: cData,
        Anonymous: VR.Phone.Data.AnonymousCall,
    }), function(status){
        if (cData.number !== VR.Phone.Data.PlayerData.charinfo.phone) {
            if (status.IsOnline) {
                if (status.CanCall) {
                    if (!status.InCall) {
                        if (VR.Phone.Data.AnonymousCall) {
                            VR.Phone.Notifications.Add("fas fa-phone", "Phone", "You started a anonymous call!");
                        }
                        $(".phone-call-outgoing").css({"display":"block"});
                        $(".phone-call-incoming").css({"display":"none"});
                        $(".phone-call-ongoing").css({"display":"none"});
                        $(".phone-call-outgoing-caller").html(cData.name);
                        VR.Phone.Functions.HeaderTextColor("white", 400);
                        VR.Phone.Animations.TopSlideUp('.phone-application-container', 400, -160);
                        setTimeout(function(){
                            $(".phone-app").css({"display":"none"});
                            VR.Phone.Animations.TopSlideDown('.phone-application-container', 400, 0);
                            VR.Phone.Functions.ToggleApp("phone-call", "block");
                        }, 450);
    
                        CallData.name = cData.name;
                        CallData.number = cData.number;
                    
                        VR.Phone.Data.currentApplication = "phone-call";
                    } else {
                        VR.Phone.Notifications.Add("fas fa-phone", "Phone", "You're already in a call!");
                    }
                } else {
                    VR.Phone.Notifications.Add("fas fa-phone", "Phone", "This person is busy!");
                }
            } else {
                VR.Phone.Notifications.Add("fas fa-phone", "Phone", "This person is not available!");
            }
        } else {
            VR.Phone.Notifications.Add("fas fa-phone", "Phone", "You can't call yourself!");
        }
    });
});
...
VR.Phone.Functions.LoadContacts = function(myContacts) {
...
$(document).on('click', '#new-chat-phone', function(e){
    ...
    if (ContactData.number !== VR.Phone.Data.PlayerData.charinfo.phone) {
        ...
        setTimeout(function(){
            ...
            VR.Phone.Functions.ToggleApp("phone", "none");
            VR.Phone.Functions.ToggleApp("whatsapp", "block");
            VR.Phone.Data.currentApplication = "whatsapp";
        
            $.post('https://vr-phone/GetWhatsappChat', JSON.stringify({phone: ContactData.number}), function(chat){
                VR.Phone.Functions.SetupChatMessages(chat, {
        ...
        VR.Phone.Notifications.Add("fa fa-phone-alt", "Phone", "You can't whatsapp yourself..", "default", 3500);
    }
});
...
$(document).on('click', '#edit-contact', function(e){
    ...
    VR.Phone.Animations.TopSlideDown(".phone-edit-contact", 200, 0);
});
...
$(document).on('click', '#edit-contact-save', function(e){
    ...
    if (ContactName != "" && ContactNumber != "") {
        $.post('https://vr-phone/EditContact', JSON.stringify({
            ...
            VR.Phone.Functions.LoadContacts(PhoneContacts);
        });
        VR.Phone.Animations.TopSlideUp(".phone-edit-contact", 250, -100);
        setTimeout(function(){
            $(".phone-edit-contact-number").val("");
            $(".phone-edit-contact-name").val("");
        }, 250)
    } else {
        VR.Phone.Notifications.Add("fas fa-exclamation-circle", "Edit Contact", "Fill out all fields!");
    }
});
...
$(document).on('click', '#edit-contact-delete', function(e){
    ...
    $.post('https://vr-phone/DeleteContact', JSON.stringify({
        CurrentContactName: ContactName,
        CurrentContactNumber: ContactNumber,
        CurrentContactIban: ContactIban,
    }), function(PhoneContacts){
...
$(document).on('click', '#phone-plus-icon', function(e){
    ...
    VR.Phone.Animations.TopSlideDown(".phone-add-contact", 200, 0);
});
...
$(document).on('click', '#add-contact-save', function(e){
    ...
    if (ContactName != "" && ContactNumber != "") {
        $.post('https://vr-phone/AddNewContact', JSON.stringify({
        ...
        VR.Phone.Animations.TopSlideUp(".phone-add-contact", 250, -100);
        setTimeout(function(){
        ...
        if (SelectedSuggestion !== null) {
            $.post('https://vr-phone/RemoveSuggestion', JSON.stringify({
        ...
        VR.Phone.Notifications.Add("fas fa-exclamation-circle", "Add Contact", "Fill out all fields!");
    }
});
        ...
        if (cData.number !== VR.Phone.Data.PlayerData.charinfo.phone) {
            if (status.IsOnline) {
                if (status.CanCall) {
                    ...
                    VR.Phone.Notifications.Add("fas fa-phone", "Phone", "This person is in a call!");
                }
            } else {
                VR.Phone.Notifications.Add("fas fa-phone", "Phone", "This person is not available!");
            }
        } else {
            VR.Phone.Notifications.Add("fas fa-phone", "Phone", "You can't call your own number!");
        }
    });
}

CancelOutgoingCall = function() {
    if (VR.Phone.Data.currentApplication == "phone-call") {
        VR.Phone.Animations.TopSlideUp('.phone-application-container', 400, -160);
    ...
    if (!Canceled) {
        if (!VR.Phone.Data.CallActive) {
            ...
            setTimeout(function(){
                ...
                $(".phone-app").css({"display":"none"});
                VR.Phone.Functions.HeaderTextColor("white", 400);
                $("."+VR.Phone.Data.currentApplication+"-app").css({"display":"none"});
...
VR.Phone.Functions.SetupCurrentCall = function(cData) {
    if (cData.InCall) {
        CallData = cData;
...
$(document).on('click', '.phone-currentcall-container', function(e){
    ...
    VR.Phone.Functions.HeaderTextColor("white", 500);
    VR.Phone.Animations.TopSlideDown('.phone-application-container', 500, 0);
    VR.Phone.Animations.TopSlideDown('.phone-call-app', 500, 0);
    VR.Phone.Functions.ToggleApp("phone-call", "block");
...
$(document).on('click', '#incoming-answer', function(e){
    ...
    $.post('https://vr-phone/AnswerCall');
});

VR.Phone.Functions.AnswerCall = function(CallData) {
    $(".phone-call-incoming").css({"display":"none"});
    $(".phone-call-outgoing").css({"display":"none"});
    $(".phone-call-ongoing").css({"display":"block"});
...
VR.Phone.Functions.SetupSuggestedContacts = function(Suggested) {
