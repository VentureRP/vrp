...
$(document).on('click', '.whatsapp-chat', function(e){
    ...
    VR.Phone.Functions.SetupChatMessages(ChatData);

    $.post('https://vr-phone/ClearAlerts', JSON.stringify({
...
VR.Phone.Functions.GetLastMessage = function(messages) {
...
VR.Phone.Functions.LoadWhatsappChats = function(chats) {
    $(".whatsapp-chats").html("");
    $.each(chats, function(i, chat){
        ...
        var LastMessage = VR.Phone.Functions.GetLastMessage(chat.messages);
...
VR.Phone.Functions.ReloadWhatsappAlerts = function(chats) {
    $.each(chats, function(i, chat){
        if (chat.Unread > 0 && chat.Unread !== undefined && chat.Unread !== null) {
            $(".unread-chat-id-"+i).html(chat.Unread);
            $(".unread-chat-id-"+i).css({"display":"block"});
...
$(document).on('click', '#whatsapp-openedchat-send', function(e){
    ...
    if (Message !== null && Message !== undefined && Message !== "") {
        $.post('https://vr-phone/SendMessage', JSON.stringify({
        ...
        VR.Phone.Notifications.Add("fab fa-whatsapp", "Whatsapp", "You can't send a empty message!", "#25D366", 1750);
    }
});
...
// VR.Phone.Functions.SetupChatMessages = function(cData, NewChatData) {
//     if (cData) {
//         OpenedChatData.number = cData.number;

//         if (OpenedChatPicture == null) {
//             $.post('https://vr-phone/GetProfilePicture', JSON.stringify({
...
//                 var Sender = "me";
//                 if (message.sender !== VR.Phone.Data.PlayerData.citizenid) { Sender = "other"; }
//                 var MessageElement
...
$(document).on('click', '.whatsapp-shared-location', function(e){
    ...
    $.post('https://vr-phone/SharedLocation', JSON.stringify({
