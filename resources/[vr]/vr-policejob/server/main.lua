...
AddEventHandler('police:server:TakeOutImpound', function(plate)
    ...
    TriggerClientEvent('VRCore:Notify', src, "Vehicle is taken out of Impound!")  
end)
...
AddEventHandler('police:server:CuffPlayer', function(playerId, isSoftcuff)
    ...
    local CuffedPlayer = VRCore.Functions.GetPlayer(playerId)
...
AddEventHandler('police:server:EscortPlayer', function(playerId)
    local src = source
    local Player = VRCore.Functions.GetPlayer(source)
    local EscortPlayer = VRCore.Functions.GetPlayer(playerId)
...
AddEventHandler('police:server:BillPlayer', function(playerId, price)
    ...
    if Player.PlayerData.job.name == "police" then
        if OtherPlayer ~= nil then
            ...
            TriggerClientEvent('VRCore:Notify', OtherPlayer.PlayerData.source, "You received a fine of $"..price)
        end
    end
end)
...
AddEventHandler('police:server:JailPlayer', function(playerId, time)
    ...
    if Player.PlayerData.job.name == "police" then
        if OtherPlayer ~= nil then
            ...
            TriggerClientEvent('VRCore:Notify', src, "You sent the person to prison for "..time.." months")
        end
    end
end)
...
AddEventHandler('police:server:SendEmergencyMessage', function(coords, message)
    local src = source
    local MainPlayer = VRCore.Functions.GetPlayer(src)
    local alertData = {
...
AddEventHandler('police:server:SearchPlayer', function(playerId)
    local src = source
    local SearchedPlayer = VRCore.Functions.GetPlayer(playerId)
    if SearchedPlayer ~= nil then 
        TriggerClientEvent('chatMessage', source, "SYSTEM", "warning", "Person has $"..SearchedPlayer.PlayerData.money["cash"]..",- on him..")
        TriggerClientEvent('VRCore:Notify', SearchedPlayer.PlayerData.source, "You are being searched..")
    end
end)
...
AddEventHandler('police:server:SeizeCash', function(playerId)
    ...
    if SearchedPlayer ~= nil then 
        ...
        TriggerClientEvent('inventory:client:ItemBox', src, VRCore.Shared.Items["moneybag"], "add")
        TriggerClientEvent('VRCore:Notify', SearchedPlayer.PlayerData.source, "Your cash is confiscated..")
    end
end)
...
AddEventHandler('police:server:SeizeDriverLicense', function(playerId)
    ...
    if SearchedPlayer ~= nil then
        ...
        if driverLicense then
            ...
            TriggerClientEvent('VRCore:Notify', SearchedPlayer.PlayerData.source, "Your driving license has been confiscated..")
        else
            TriggerClientEvent('VRCore:Notify', src, "Can't confiscate driving license..", "error")
        end
    end
end)
...
AddEventHandler('police:server:RobPlayer', function(playerId)
    ...
    if SearchedPlayer ~= nil then 
        ...
        TriggerClientEvent('VRCore:Notify', SearchedPlayer.PlayerData.source, "You have been robbed of $"..money)
	TriggerClientEvent('VRCore:Notify', Player.PlayerData.source, "You have stolen $"..money)
    end
end)
...
AddEventHandler('police:server:Impound', function(plate, fullImpound, price, body, engine, fuel)
    ...
    if IsVehicleOwned(plate) then
        if not fullImpound then
            ...
            TriggerClientEvent('VRCore:Notify', src, "Vehicle taken into depot for $"..price.."!")
        else
            ...
            TriggerClientEvent('VRCore:Notify', src, "Vehicle completely seized!")
        end
    end
end)
...
AddEventHandler('evidence:server:AddBlooddropToInventory', function(bloodId, bloodInfo)
    ...
    if Player.Functions.RemoveItem("empty_evidence_bag", 1) then
        if Player.Functions.AddItem("filled_evidence_bag", 1, false, bloodInfo) then
            TriggerClientEvent("inventory:client:ItemBox", src, VRCore.Shared.Items["filled_evidence_bag"], "add")
            TriggerClientEvent("evidence:client:RemoveBlooddrop", -1, bloodId)
            BloodDrops[bloodId] = nil
        end
    else
        TriggerClientEvent('VRCore:Notify', src, "You must have an empty evidence bag with you", "error")
    end
end)
...
AddEventHandler('evidence:server:CreateCasing', function(weapon, coords)
    ...
    local weaponInfo = VRCore.Shared.Weapons[weapon]
...
AddEventHandler('police:server:showFingerprint', function(playerId)
    local src = source
    local Player = VRCore.Functions.GetPlayer(playerId)
...
AddEventHandler('police:server:SetTracker', function(targetId)
    local Target = VRCore.Functions.GetPlayer(targetId)
    local TrackerMeta = Target.PlayerData.metadata["tracker"]

    if TrackerMeta then
        Target.Functions.SetMetaData("tracker", false)
        TriggerClientEvent('VRCore:Notify', targetId, 'Your anklet is taken off.', 'error', 5000)
        TriggerClientEvent('VRCore:Notify', source, 'You took off an ankle bracelet from '..Target.PlayerData.charinfo.firstname.." "..Target.PlayerData.charinfo.lastname, 'error', 5000)
        TriggerClientEvent('police:client:SetTracker', targetId, false)
    else
        Target.Functions.SetMetaData("tracker", true)
        TriggerClientEvent('VRCore:Notify', targetId, 'You put on an ankle strap.', 'error', 5000)
        TriggerClientEvent('VRCore:Notify', source, 'You put on an ankle strap to '..Target.PlayerData.charinfo.firstname.." "..Target.PlayerData.charinfo.lastname, 'error', 5000)
...
AddEventHandler('police:server:SendTrackerLocation', function(coords, requestId)
    local Target = VRCore.Functions.GetPlayer(source)
    local TrackerMeta = Target.PlayerData.metadata["tracker"]
    ...
    TriggerClientEvent("vr-phone:client:addPoliceAlert", requestId, alertData)
end)
...
VRCore.Functions.CreateCallback('police:server:isPlayerDead', function(source, cb, playerId)
    local Player = VRCore.Functions.GetPlayer(playerId)
    cb(Player.PlayerData.metadata["isdead"])
end)

VRCore.Functions.CreateCallback('police:GetPlayerStatus', function(source, cb, playerId)
    local Player = VRCore.Functions.GetPlayer(playerId)
    local statList = {}
	if Player ~= nil then
...
VRCore.Functions.CreateCallback('police:IsSilencedWeapon', function(source, cb, weapon)
    local Player = VRCore.Functions.GetPlayer(source)
    local itemInfo = Player.Functions.GetItemByName(VRCore.Shared.Weapons[weapon]["name"])
    local retval = false
...
VRCore.Functions.CreateCallback('police:GetDutyPlayers', function(source, cb)
    local dutyPlayers = {}
    for k, v in pairs(VRCore.Functions.GetPlayers()) do
        local Player = VRCore.Functions.GetPlayer(v)
        if Player ~= nil then 
...
VRCore.Functions.CreateCallback('police:GetImpoundedVehicles', function(source, cb)
    local vehicles = {}
...
VRCore.Functions.CreateCallback('police:IsPlateFlagged', function(source, cb, plate)
    local retval = false
...
VRCore.Functions.CreateCallback('police:GetCops', function(source, cb)
    local amount = 0
...
--[[ VRCore.Commands.Add("setpolice", "Hire An Officer (Police Only)", {{name="id", help="Player ID"}}, true, function(source, args)
...
VRCore.Commands.Add("spikestrip", "Place Spike Strip (Police Only)", {}, false, function(source, args)
    local Player = VRCore.Functions.GetPlayer(source)
    if Player ~= nil then 
        if (Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty) then
...
VRCore.Commands.Add("grantlicense", "Grant a license to someone", {{name="id", help="ID of a person"},{name="license", help="License Type"}}, true, function(source, args)
    local source = source
    local Player = VRCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.grade.level >= 2 then
        if args[2] == "driver" or args[2] == "weapon" then
            ...
            if SearchedPlayer ~= nil then
                ...
                TriggerClientEvent('VRCore:Notify', SearchedPlayer.PlayerData.source, "You have been granted a license", "success", 5000)
                TriggerClientEvent('VRCore:Notify', source, "You granted a license", "success", 5000)
            end
        else
            TriggerClientEvent('VRCore:Notify', source, "Invalid license type", "error")
        end
    else
        TriggerClientEvent('VRCore:Notify', source, "You must be a Sergeant to grant licenses!", "error")
    end
end)

VRCore.Commands.Add("revokelicense", "Revoke a license from someone", {{name="id", help="ID of a person"},{name="license", help="License Type"}}, true, function(source, args)
    local source = source
    local Player = VRCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.grade.level >= 2 then
        if args[2] == "driver" or args[2] == "weapon" then
            ...
            if SearchedPlayer ~= nil then
                ...
                SearchedPlayer.Functions.SetMetaData("licences", licenseTable)
                TriggerClientEvent('VRCore:Notify', SearchedPlayer.PlayerData.source, "You've had a license revoked", "error", 5000)
                TriggerClientEvent('VRCore:Notify', source, "You revoked a license", "success", 5000)
            end
        else
            TriggerClientEvent('VRCore:Notify', source, "Invalid license type", "error")
        end
    else
        TriggerClientEvent('VRCore:Notify', source, "You must be a Sergeant to revoke licenses!", "error")
    end
end)


--[[ VRCore.Commands.Add("firepolice", "Fire An Officer (Police Only)", {{name="id", help="Player ID"}}, true, function(source, args)
    local Player = VRCore.Functions.GetPlayer(tonumber(args[1]))
    local Myself = VRCore.Functions.GetPlayer(source)
    if Player ~= nil then 
        if (Myself.PlayerData.job.name == "police" and Myself.PlayerData.job.onduty) and IsHighCommand(Myself.PlayerData.citizenid) then
...
VRCore.Commands.Add("pobject", "Place/Delete An Object (Police Only)", {{name="type", help="Type object you want or 'delete' to delete"}}, true, function(source, args)
    local Player = VRCore.Functions.GetPlayer(source)
    ...
    if Player.PlayerData.job.name == "police" then
        ...
        TriggerClientEvent('VRCore:Notify', source, 'For Emergency Services Only', 'error')
    end
end)

VRCore.Commands.Add("cuff", "Cuff Player (Police Only)", {}, false, function(source, args)
	local Player = VRCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" then
...
VRCore.Commands.Add("palert", "Make a police alert", {{name="alert", help="The Police alert"}}, false, function(source, args)
    local Player = VRCore.Functions.GetPlayer(source)
    ...
    if (Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty) then
        if args[1] ~= nil then
            ...
            TriggerEvent("vr-log:server:CreateLog", "palert", "Police alert", "blue", "**"..GetPlayerName(source).."** (CitizenID: "..Player.PlayerData.citizenid.." | ID: "..source..") **Alert:** " ..msg, false)
...
VRCore.Commands.Add("escort", "Escort Player", {}, false, function(source, args)
	local Player = VRCore.Functions.GetPlayer(source)
    TriggerClientEvent("police:client:EscortPlayer", source)
end)

--VRCore.Commands.Add("mdt", "Open MDT (Police Only)", {}, false, function(source, args)
--	local Player = VRCore.Functions.GetPlayer(source)
--    if Player.PlayerData.job.name == "police" then
...
VRCore.Commands.Add("callsign", "Give Yourself A Callsign", {{name="name", help="Name of your callsign"}}, false, function(source, args)
	local Player = VRCore.Functions.GetPlayer(source)
    Player.Functions.SetMetaData("callsign", table.concat(args, " "))
end)

VRCore.Commands.Add("clearcasings", "Clear Area of Casings (Police Only)", {}, false, function(source, args)
	local Player = VRCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" then
...
VRCore.Commands.Add("jail", "Jail Player (Police Only)", {{name="id", help="Player ID"},{name="time", help="Time they have to be in jail"}}, true, function(source, args)
	local Player = VRCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" then
...
VRCore.Commands.Add("unjail", "Unjail Player (Police Only)", {{name="id", help="Player ID"}}, true, function(source, args)
	local Player = VRCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" then
        local playerId = tonumber(args[1])
...
VRCore.Commands.Add("clearblood", "Clear The Area of Blood (Police Only)", {}, false, function(source, args)
	local Player = VRCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" then
...
VRCore.Commands.Add("seizecash", "Seize Cash (Police Only)", {}, false, function(source, args)
	local Player = VRCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
...
VRCore.Commands.Add("sc", "Soft Cuff (Police Only)", {}, false, function(source, args)
	local Player = VRCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" then
...
VRCore.Commands.Add("cam", "View Security Camera (Police Only)", {{name="camid", help="Camera ID"}}, false, function(source, args)
	local Player = VRCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" then
...
VRCore.Commands.Add("flagplate", "Flag A Plate (Police Only)", {{name="plate", help="License"}, {name="reason", help="Reason of flagging the vehicle"}}, true, function(source, args)
    local Player = VRCore.Functions.GetPlayer(source)
    ...
    if Player.PlayerData.job.name == "police" then
        ...
        TriggerClientEvent('VRCore:Notify', source, "Vehicle ("..args[1]:upper()..") is flagged for: "..table.concat(reason, " "))
    else
        TriggerClientEvent('VRCore:Notify', source, 'For Emergency Services Only', 'error')
    end
end)

VRCore.Commands.Add("unflagplate", "Unflag A Plate (Police Only)", {{name="plate", help="License plate"}}, true, function(source, args)
	local Player = VRCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" then
        if Plates ~= nil and Plates[args[1]:upper()] ~= nil then
            if Plates[args[1]:upper()].isflagged then
                Plates[args[1]:upper()].isflagged = false
                TriggerClientEvent('VRCore:Notify', source, "Vehicle ("..args[1]:upper()..") is unflagged")
            else
...
VRCore.Commands.Add("plateinfo", "Run A Plate (Police Only)", {{name="plate", help="License plate"}}, true, function(source, args)
	local Player = VRCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" then
        if Plates ~= nil and Plates[args[1]:upper()] ~= nil then
            if Plates[args[1]:upper()].isflagged then
...
VRCore.Commands.Add("depot", "Impound With Price (Police Only)", {{name="price", help="Price for how much the person has to pay (may be empty)"}}, false, function(source, args)
	local Player = VRCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" then
...
VRCore.Commands.Add("impound", "Impound A Vehicle (Police Only)", {}, false, function(source, args)
	local Player = VRCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" then
...
VRCore.Commands.Add("paytow", "Pay Tow Driver (Police, EMS Only)", {{name="id", help="ID of the player"}}, true, function(source, args)
	local Player = VRCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" or Player.PlayerData.job.name == "ambulance" then
        ...
        if OtherPlayer ~= nil then
            if OtherPlayer.PlayerData.job.name == "tow" then
                ...
                TriggerClientEvent('VRCore:Notify', source, 'You paid a bergnet worker')
            else
                TriggerClientEvent('VRCore:Notify', source, 'Person is not a bergnet worker', "error")
            end
        end
    else
        TriggerClientEvent('VRCore:Notify', source, 'For Emergency Services Only', 'error')
...
VRCore.Commands.Add("paylawyer", "Pay Lawyer (Police, Judge Only)", {{name="id", help="ID of the player"}}, true, function(source, args)
	local Player = VRCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" or Player.PlayerData.job.name == "judge" then
        ...
        if OtherPlayer ~= nil then
            if OtherPlayer.PlayerData.job.name == "lawyer" then
                ...
                TriggerClientEvent('VRCore:Notify', source, 'You paid a lawyer')
            else
                TriggerClientEvent('VRCore:Notify', source, 'Person is not a lawyer', "error")
            end
        end
    else
        TriggerClientEvent('VRCore:Notify', source, 'For Emergency Services Only', 'error')
...
VRCore.Commands.Add("radar", "Enable Police Radar (Police Only)", {}, false, function(source, args)
	local Player = VRCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" then
...
VRCore.Functions.CreateUseableItem("handcuffs", function(source, item)
    local Player = VRCore.Functions.GetPlayer(source)
...
VRCore.Commands.Add("911", "Send a report to emergency services", {{name="message", help="Message you want to send"}}, true, function(source, args)
    ...
    if Player.Functions.GetItemByName("phone") ~= nil then
        ...
        TriggerEvent("vr-log:server:CreateLog", "911", "911 alert", "blue", "**"..GetPlayerName(source).."** (CitizenID: "..Player.PlayerData.citizenid.." | ID: "..source..") **Alert:** " ..message, false)
    else
        TriggerClientEvent('VRCore:Notify', source, 'You dont have a phone', 'error')
    end
end)

VRCore.Commands.Add("911a", "Send an anonymous report to emergency services (gives no location)", {{name="message", help="Message you want to send"}}, true, function(source, args)
    local message = table.concat(args, " ")
    local Player = VRCore.Functions.GetPlayer(source)
...
VRCore.Commands.Add("911r", "Send a message back to a alert", {{name="id", help="ID of the alert"}, {name="message", help="Message you want to send"}}, true, function(source, args)
    local Player = VRCore.Functions.GetPlayer(source)
    local OtherPlayer = VRCore.Functions.GetPlayer(tonumber(args[1]))
    table.remove(args, 1)
    local message = table.concat(args, " ")
    local Prefix = "POLICE"
...
VRCore.Commands.Add("anklet", "Attach Tracking Anklet (Police Only)", {}, false, function(source, args)
    local Player = VRCore.Functions.GetPlayer(source)
...
VRCore.Commands.Add("ankletlocation", "Get the location of a persons anklet", {{"csn", "CSN of the person"}}, true, function(source, args)
    local Player = VRCore.Functions.GetPlayer(source)
    
    if Player.PlayerData.job.name == "police" then
        if args[1] ~= nil then
            local citizenid = args[1]
            local Target = VRCore.Functions.GetPlayerByCitizenId(citizenid)
            ...
            if Target ~= nil then
                if Target.PlayerData.metadata["tracker"] then
                    ...
                    TriggerClientEvent('VRCore:Notify', source, 'This person doesn\'t have an anklet on.', 'error')
                end
            end
        end
    else
...
VRCore.Commands.Add("removeanklet", "Remove Tracking Anklet (Police Only)", {{"bsn", "BSN of person"}}, true, function(source, args)
    local Player = VRCore.Functions.GetPlayer(source)
        ...
        if args[1] ~= nil then
            ...
            if Target ~= nil then
                if Target.PlayerData.metadata["tracker"] then
                    ...
                    TriggerClientEvent('VRCore:Notify', source, 'This person does not have an anklet', 'error')
                end
            end
        end
    else
...
VRCore.Commands.Add("ebutton", "Respond To A Call (Police, EMS, Mechanic Only)", {}, false, function(source, args)
    local Player = VRCore.Functions.GetPlayer(source)
    if ((Player.PlayerData.job.name == "police" or Player.PlayerData.job.name == "ambulance" or Player.PlayerData.job.name == "doctor") and Player.PlayerData.job.onduty) then
...
VRCore.Commands.Add("takedrivinglicense", "Seize Drivers License (Police Only)", {}, false, function(source, args)
    local Player = VRCore.Functions.GetPlayer(source)
    if ((Player.PlayerData.job.name == "police") and Player.PlayerData.job.onduty) then
...
VRCore.Commands.Add("takedna", "Take a DNA sanple from a person (empty evidence bag needed) (Police Only)", {{"id", "ID of the person"}}, true, function(source, args)
    local Player = VRCore.Functions.GetPlayer(source)
    local OtherPlayer = VRCore.Functions.GetPlayer(tonumber(args[1]))
    if ((Player.PlayerData.job.name == "police") and Player.PlayerData.job.onduty) and OtherPlayer ~= nil then
        if Player.Functions.RemoveItem("empty_evidence_bag", 1) then
            ...
            if Player.Functions.AddItem("filled_evidence_bag", 1, false, info) then
                TriggerClientEvent("inventory:client:ItemBox", source, VRCore.Shared.Items["filled_evidence_bag"], "add")
            end
        else
            TriggerClientEvent('VRCore:Notify', source, "You must have an empty evidence bag with you", "error")
        end
    end
end)

VRCore.Functions.CreateUseableItem("moneybag", function(source, item)
    local Player = VRCore.Functions.GetPlayer(source)
    if Player.Functions.GetItemBySlot(item.slot) ~= nil then
...
VRCore.Functions.CreateCallback('police:server:IsPoliceForcePresent', function(source, cb)
    local retval = false
    for k, v in pairs(VRCore.Functions.GetPlayers()) do
        local Player = VRCore.Functions.GetPlayer(v)
        if Player ~= nil then 
