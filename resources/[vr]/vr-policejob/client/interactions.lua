Citizen.CreateThread(function()
    while true do 
        ...
        if isHandcuffed then
            ...
            if (not IsEntityPlayingAnim(PlayerPedId(), "mp_arresting", "idle", 3) and not IsEntityPlayingAnim(PlayerPedId(), "mp_arrest_paired", "crook_p2_back_right", 3)) and not VRCore.Functions.GetPlayerData().metadata["isdead"] then
...
AddEventHandler('police:client:RobPlayer', function()
    ...
    if player ~= -1 and distance < 2.5 then
        ...
        if IsEntityPlayingAnim(playerPed, "missminuteman_1ig_2", "handsup_base", 3) or IsEntityPlayingAnim(playerPed, "mp_arresting", "idle", 3) or IsTargetDead(playerId) then
            VRCore.Functions.Progressbar("robbing_player", "Robbing person..", math.random(5000, 7000), false, true, {
...
AddEventHandler('police:client:JailPlayer', function()
    ...
    if player ~= -1 and distance < 2.5 then
        ...
        if tonumber(time) > 0 then
            ...
            VRCore.Functions.Notify("Time must be higher than 0..", "error")
        end
    else
        VRCore.Functions.Notify("No one nearby!", "error")
    end
    ...
    if not IsPedRagdoll(PlayerPedId()) then
        ...
        if player ~= -1 and distance < 1.5 then
            ...
            if not IsPedInAnyVehicle(GetPlayerPed(player)) and not IsPedInAnyVehicle(GetPlayerPed(PlayerPedId())) then
                ...
                VRCore.Functions.Notify("You cant cuff someone in a vehicle", "error")
            end
        else
            VRCore.Functions.Notify("No one nearby!", "error")
        end
            ...
            VRCore.Functions.TriggerCallback('VRCore:HasItem', function(result)
                if result then 
                    ...
                    if not IsPedInAnyVehicle(GetPlayerPed(player)) and not IsPedInAnyVehicle(GetPlayerPed(PlayerPedId())) then
                        ...
                        VRCore.Functions.Notify("You can\'t cuff someone in a vehicle", "error")
                    end
                else
                    VRCore.Functions.Notify("You don\'t have handcuffs on you", "error")
                end
            end, "handcuffs")
        else
            VRCore.Functions.Notify("No one nearby!", "error")
...
AddEventHandler('police:client:GetKidnappedDragger', function(playerId)
    VRCore.Functions.GetPlayerData(function(PlayerData)
        ...
        TriggerEvent('vr-kidnapping:client:SetKidnapping', isEscorting)
    end)
end)
...
AddEventHandler('police:client:GetCuffed', function(playerId, isSoftcuff)
    if not isHandcuffed then
        ...
        if not isSoftcuff then
            ...
            VRCore.Functions.Notify("You are cuffed!")
        else
            cuffType = 49
            GetCuffedAnimation(playerId)
            VRCore.Functions.Notify("You are cuffed, but you can walk")
        end
    else
        isHandcuffed = false
        isEscorted = false
        ...
        ClearPedTasksImmediately(PlayerPedId())
        VRCore.Functions.Notify("You are uncuffed!")
    end
end)

function IsTargetDead(playerId)
    local retval = false
    VRCore.Functions.TriggerCallback('police:server:isPlayerDead', function(result)
        retval = result
    end, playerId)
    Citizen.Wait(100)
    return retval
