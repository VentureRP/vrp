        ...
        if isLoggedIn then
            ...
            if entering ~= 0 and IsVehicleSeatFree(entering, -1) then
                if not lockpicked then
                    VRCore.Functions.TriggerCallback('vehiclekeys:CheckHasKey', function(result)
                ...
                if vehicle ~= nil and vehicle ~= 0 then
                    ...
                    if driver ~= 0 and not IsPedAPlayer(driver) then
                        if IsEntityDead(driver) then
                            IsRobbing = true
                            VRCore.Functions.Progressbar("rob_keys", "Grabbing keys..", 2500, false, true, {}, {}, {}, {}, function() -- Done
...
function LockVehicle()
    ...
    local veh = VRCore.Functions.GetClosestVehicle(pos)
            ...
            if result then
                ...
                if vehLockStatus == 1 then
                    ...
                    if(GetVehicleDoorLockStatus(veh) == 2)then
                        ...
                        VRCore.Functions.Notify("Vehicle locked!")
                    else
                        VRCore.Functions.Notify("Something went wrong with the locking system!")
                    end
                else
                    Citizen.Wait(750)
                    ClearPedTasks(ped)
                    ...
                    if(GetVehicleDoorLockStatus(veh) == 1)then
                        ...
                        Wait(200)
                        SetVehicleLights(veh,0)
                        VRCore.Functions.Notify("Vehicle unlocked!")
                    else
                        VRCore.Functions.Notify("Something went wrong with the locking system!")
                    end
                end
            else
                VRCore.Functions.Notify('You don\'t have the keys of the vehicle..', 'error')
            end
        end, plate)
    end
end
...
function LockpickDoor(isAdvanced)
    local ped = PlayerPedId()
    local pos = GetEntityCoords(ped)
    local vehicle = VRCore.Functions.GetClosestVehicle(pos)
    if vehicle ~= nil and vehicle ~= 0 then
...
function lockpickFinish(success)
    ...
    if success then
        ...
        VRCore.Functions.Notify('Opened Door!', 'success')
    else
        PoliceCall()
        TriggerServerEvent('hud:server:GainStress', math.random(1, 4))
        VRCore.Functions.Notify('Someone Called The Police!', 'error')
    end
...
function Hotwire()
    if not HasKey then
        ...
        PoliceCall()
        VRCore.Functions.Progressbar("hotwire_vehicle", "Engaging the ignition switch", hotwireTime, false, true, {
            ...
            if (math.random(0, 100) < 10) then
                ...
                TriggerServerEvent('hud:server:GainStress', math.random(1, 4))
                TriggerEvent('vehiclekeys:client:SetOwner', GetVehicleNumberPlateText(vehicle))
                VRCore.Functions.Notify("Hotwire succeeded!")
            else
                SetVehicleEngineOn(veh, false, false, true)
                TriggerServerEvent('hud:server:GainStress', math.random(1, 4))
                VRCore.Functions.Notify("Hotwire failed!", "error")
            end
            IsHotwiring = false
        end, function() -- Cancel
            StopAnimTask(ped, "anim@amb@clubhouse@tutorial@bkr_tut_ig3@", "machinic_loop_mechandplayer", 1.0)
        ...
        if math.random(1, 100) <= chance then
            ...
            if closestPed ~= nil then
                ...
                if IsPedInAnyVehicle(ped) then
                    ...
                    if VRCore.Shared.Vehicles[modelName] ~= nil then
                        Name = VRCore.Shared.Vehicles[modelName]["brand"] .. ' ' .. VRCore.Shared.Vehicles[modelName]["name"]
                    else
                        Name = "Unknown"
                    end
...
function RobVehicle(target)
    ...
    VRCore.Functions.Progressbar("rob_keys", "Attemping Robbery..", 6000, false, true, {}, {}, {}, {}, function()
        local chance = math.random(1, 100)
        if chance > 85 then
            ...
            VRCore.Functions.Notify('You Got The Keys!', 'success')
        else
            PoliceCall()
            TriggerServerEvent('hud:server:GainStress', math.random(1, 4))
            TaskSmartFleePed(target, PlayerPedId(), 40.0, 20000)
            VRCore.Functions.Notify('They Called The Cops!', 'error')
            Wait(2000)
            IsRobbing = false
        end
    end)
