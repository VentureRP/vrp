...
VRCore.Functions.CreateCallback('vehiclekeys:CheckHasKey', function(source, cb, plate)
...
AddEventHandler('vehiclekeys:server:GiveVehicleKeys', function(plate, target)
    ...
    if CheckOwner(plate, Player.PlayerData.citizenid) then
        if VRCore.Functions.GetPlayer(target) ~= nil then
            TriggerClientEvent('vehiclekeys:client:SetOwner', target, plate)
            TriggerClientEvent('VRCore:Notify', src, "You gave the keys!")
            TriggerClientEvent('VRCore:Notify', target, "You got the keys!")
        else
            TriggerClientEvent('VRCore:Notify', source,  "Player Not Online", "error")
        end
    else
        TriggerClientEvent('VRCore:Notify', source,  "You Dont Own This Vehicle", "error")
    end
end)

VRCore.Commands.Add("engine", "Toggle Engine", {}, false, function(source, args)
	TriggerClientEvent('vehiclekeys:client:ToggleEngine', source)
end)

VRCore.Commands.Add("givecarkeys", "Give Car Keys", {{name = "id", help = "Player id"}}, true, function(source, args)
	local src = source
...
VRCore.Functions.CreateUseableItem("lockpick", function(source, item)
    local Player = VRCore.Functions.GetPlayer(source)
    TriggerClientEvent("lockpicks:UseLockpick", source, false)
end)

VRCore.Functions.CreateUseableItem("advancedlockpick", function(source, item)
    local Player = VRCore.Functions.GetPlayer(source)
