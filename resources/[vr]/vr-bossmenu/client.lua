...
RegisterNetEvent('vr-bossmenu:client:openMenu')
AddEventHandler('vr-bossmenu:client:openMenu', function()
...
menu_button6:On("select", function()
    ...
    if result ~= nil then
        TriggerServerEvent("vr-bossmenu:server:withdrawMoney", tonumber(result))
        UpdateSociety()
    end
end)
...
menu_button7:On("select", function()
    ...
    if result ~= nil then
        TriggerServerEvent("vr-bossmenu:server:depositMoney", tonumber(result))
        UpdateSociety()
    end
end)
...
menu_button:On("select", function()
    menu3:ClearItems()
    VRCore.Functions.TriggerCallback('vr-bossmenu:server:GetEmployees', function(cb)
    ...
    for k,v in pairs(VRCore.Functions.GetPlayersFromCoords(GetEntityCoords(playerPed), 10.0)) do
        if v and v ~= PlayerId() then
            local menu_button10 = menu4:AddButton({
                ...
                select = function(btn)
                    local select = btn.Value
                    TriggerServerEvent('vr-bossmenu:server:giveJob', GetPlayerServerId(v))
                end
            })
        end
    end
...
function UpdateSociety()
    VRCore.Functions.TriggerCallback('vr-bossmenu:server:GetAccount', function(cb)
        ...
        local menu_button9 = manageroptions:AddButton({
            ...
            select = function(btn)
                ...
                if values == 'promote' then
                    ...
                    if result ~= nil then
                        TriggerServerEvent('vr-bossmenu:server:updateGrade', employee.source, tonumber(result))
                    end
                else
                    TriggerServerEvent('vr-bossmenu:server:fireEmployee', employee.source)
                end
            end
        })
    end
