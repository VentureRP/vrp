...
RegisterNetEvent('vr-admin:client:openMenu')
AddEventHandler('vr-admin:client:openMenu', function()
    MenuV:OpenMenu(menu)
end)
...
menu_button11:On("select",function()
    ...
    for k,v in ipairs(elements) do
        local menu_button14 = menu6:AddButton({icon = v.icon,label = v.label,value = v,description = v.description,select = function(btn)
            ...
            TriggerServerEvent('vr-weathersync:server:setWeather', selection.value)
            VRCore.Functions.Notify('Weather Changed To: '..selection.label)
        end})
    end
end)
...
function OpenCarModelsMenu(category)
    ...
    for k, v in pairs(category) do
        local menu_button10 = menu14:AddButton({
             ...
             select = function(btn)
                 TriggerServerEvent('VRCore:CallCommand', "car", { k })
             end
        })
    end
end
...
menu12_button2:On('Select', function(item)
    TriggerServerEvent('VRCore:CallCommand', "fix", {})
end)

menu12_button3:On('Select', function(item)
    TriggerServerEvent('VRCore:CallCommand', "admincar", {})
end)

menu12_button4:On('Select', function(item)
    TriggerServerEvent('VRCore:CallCommand', "dv", {})
end)

names_button:On('change', function()           
    TriggerEvent('vr-admin:client:toggleNames')
end)                                           
blips_button:On('change', function()           
    TriggerEvent('vr-admin:client:toggleBlips')
end)

-- Dealer List
menu_button4:On('Select', function(item)
    menu7:ClearItems()
    VRCore.Functions.TriggerCallback('test:getdealers', function(dealers)
...
menu_button2:On('select', function(item)
    menu4:ClearItems()
    VRCore.Functions.TriggerCallback('test:getplayers', function(players)
...
menu_button13:On("select", function(item, value)
    TriggerServerEvent("vr-weathersync:server:setTime", value, value)
    VRCore.Functions.Notify("Time changed to " .. value .. " hs 00 min")
    ...
    for k, v in ipairs(elements) do
        local menu_button10 = EditDealer:AddButton({
            ...
            select = function(btn)
                ...
                if values == "goto" then
                    TriggerServerEvent('VRCore:CallCommand', "dealergoto", { dealer["name"] })
                elseif values == "remove" then
                    TriggerServerEvent('VRCore:CallCommand', "deletedealer", { dealer["name"] })
                    EditDealer:Close()
                    menu7:Close()
                end
            end
        ...
        local menu_button10 = Players:AddButton({
            ...
            select = function(btn)
                ...
                if values ~= "ban" and values ~= "kick" and values ~= "perms" then
                    TriggerServerEvent('vr-admin:server:'..values, player)
    ...
    local menu_button17 = menu8:AddButton({
        ...
        select = function(btn)
            if banreason ~= 'Unknown' and banlength ~= nil then
                TriggerServerEvent('vr-admin:server:ban', banplayer, banlength, banreason)
                banreason = 'Unknown'
                banlength = nil
            else
                VRCore.Functions.Notify('You must give a Reason and set a Length for the ban!', 'error')
            end
        end
    })
end
    ...
    local menu_button18 = menu9:AddButton({
        ...
        select = function(btn)
            if kickreason ~= 'Unknown' then
                TriggerServerEvent('vr-admin:server:kick', kickplayer, kickreason)
                kickreason = 'Unknown'
            else
                VRCore.Functions.Notify('You must give a reason!', 'error')
            end
        end
    })
end
...
function OpenPermsMenu(permsply)
    VRCore.Functions.TriggerCallback('vr-admin:server:getrank', function(rank)
        if rank then
            ...
            local menu_button21 = menu10:AddButton({
                ...
                select = function(btn)
                    if selectedgroup ~= 'Unknown' then
                        TriggerServerEvent('vr-admin:server:setPermissions', permsply.id, selectedgroup)
			VRCore.Functions.Notify('Authority group changed!', 'success')
                        selectedgroup = 'Unknown'
                    else
                        VRCore.Functions.Notify('Choose a group!', 'error')
                    end
                end
            })
        else
