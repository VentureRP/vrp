...
RegisterNetEvent('vr-admin:client:toggleBlips')
AddEventHandler('vr-admin:client:toggleBlips', function()
    if not ShowBlips then
        ShowBlips = true
        VRCore.Functions.Notify("Blips activated", "success")
    else
        ShowBlips = false
        VRCore.Functions.Notify("Blips deactivated", "error")
    end
end)

RegisterNetEvent('vr-admin:client:toggleNames')
AddEventHandler('vr-admin:client:toggleNames', function()
    if not ShowNames then
        ShowNames = true
        VRCore.Functions.Notify("Names activated", "success")
    else
        ShowNames = false
        VRCore.Functions.Notify("Names deactivated", "error")
    end
end)

RegisterNetEvent('vr-admin:client:Show')
AddEventHandler('vr-admin:client:Show', function(players)
...
Citizen.CreateThread(function()
    while true do
        Wait(1000)
        if ShowBlips then
            TriggerServerEvent('vr-admin:server:GetPlayersForBlips')
        end
    end
end)