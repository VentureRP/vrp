                            ...
                            if dist < 0.6 then
                                if not Config.Locations[case]["isBusy"] and not Config.Locations[case]["isOpened"] then
                                    ...
                                    if IsControlJustPressed(0, 38) then
                                        VRCore.Functions.TriggerCallback('vr-jewellery:server:getCops', function(cops)
                                            if cops >= Config.RequiredCops then
                                                if validWeapon() then
                                                    smashVitrine(case)
                                                else
                                                    VRCore.Functions.Notify('Your weapon is not strong enough..', 'error')
                                                end
                                            else
                                                VRCore.Functions.Notify('Not Enough Police ('.. Config.RequiredCops ..') Required', 'error')
                                            end                
                                        end)
                                    end
                                end
...
function smashVitrine(k)
    ...
    elseif math.random(1, 100) <= 5 and IsWearingHandshoes() then
        TriggerServerEvent("evidence:server:CreateFingerDrop", plyCoords)
        VRCore.Functions.Notify("You've left a fingerprint on the glass", "error")
    end

    smashing = true

    VRCore.Functions.Progressbar("smash_vitrine", "Stocking a display", Config.WhitelistedWeapons[pedWeapon]["timeOut"], false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
    }, {}, {}, {}, function() -- Done
        TriggerServerEvent('vr-jewellery:server:setVitrineState', "isOpened", true, k)
        TriggerServerEvent('vr-jewellery:server:setVitrineState', "isBusy", false, k)
        TriggerServerEvent('vr-jewellery:server:vitrineReward')
        TriggerServerEvent('vr-jewellery:server:setTimeout')
    ...
    TriggerServerEvent('vr-jewellery:server:setVitrineState', "isBusy", true, k)
...
RegisterNetEvent('vr-jewellery:client:setVitrineState')
AddEventHandler('vr-jewellery:client:setVitrineState', function(stateType, state, k)
    Config.Locations[k][stateType] = state
end)

RegisterNetEvent('vr-jewellery:client:setAlertState')
AddEventHandler('vr-jewellery:client:setAlertState', function(bool)
    robberyAlert = bool
end)

RegisterNetEvent('vr-jewellery:client:PoliceAlertMessage')
AddEventHandler('vr-jewellery:client:PoliceAlertMessage', function(title, coords, blip)
    if blip then
