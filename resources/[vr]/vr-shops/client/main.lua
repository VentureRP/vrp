                ...
                if dist < 10 then
                    ...
                    if dist < 1 then
                        ...
                        if IsControlJustPressed(0, 38) then -- E
                            local ShopItems = {}
                            ShopItems.items = {}
                            VRCore.Functions.TriggerCallback('vr-shops:server:getLicenseStatus', function(result)
                                        ...
                                        for i = 1, #products do
                                            if not products[i].requiredJob then
                                                ...
                                                for i2 = 1, #products[i].requiredJob do
                                                    if VRCore.Functions.GetPlayerData().job.name == products[i].requiredJob[i2] and not products[i].requiresLicense then
                                                        table.insert(ShopItems.items, products[i])
                                                    end
                                                end
                                            end
...
function SetupItems(shop)
    local products = Config.Locations[shop].products
    local playerJob = VRCore.Functions.GetPlayerData().job.name
    local items = {}
...
RegisterNetEvent('vr-shops:client:UpdateShop')
AddEventHandler('vr-shops:client:UpdateShop', function(shop, itemData, amount)
    TriggerServerEvent('vr-shops:server:UpdateShopItems', shop, itemData, amount)
end)

RegisterNetEvent('vr-shops:client:SetShopItems')
AddEventHandler('vr-shops:client:SetShopItems', function(shop, shopProducts)
    Config.Locations[shop]["products"] = shopProducts
end)

RegisterNetEvent('vr-shops:client:RestockShopItems')
AddEventHandler('vr-shops:client:RestockShopItems', function(shop, amount)
