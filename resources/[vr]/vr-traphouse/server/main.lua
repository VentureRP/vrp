RegisterServerEvent('vr-traphouse:server:TakeoverHouse')
AddEventHandler('vr-traphouse:server:TakeoverHouse', function(Traphouse)
    local src = source
    ...
    if not HasCitizenIdHasKey(CitizenId, Traphouse) then
        if Player.Functions.RemoveMoney('cash', Config.TakeoverPrice) then
            TriggerClientEvent('vr-traphouse:client:TakeoverHouse', src, Traphouse)
        else
...
RegisterServerEvent('vr-traphouse:server:AddHouseKeyHolder')
AddEventHandler('vr-traphouse:server:AddHouseKeyHolder', function(CitizenId, TraphouseId, IsOwner)
    local src = source
    local Player = VRCore.Functions.GetPlayer(src)
    ...
    if Config.TrapHouses[TraphouseId] ~= nil then
        ...
        if Config.TrapHouses[TraphouseId].keyholders == nil then
            ...
            TriggerClientEvent('vr-traphouse:client:SyncData', -1, TraphouseId, Config.TrapHouses[TraphouseId])
        else
            if #Config.TrapHouses[TraphouseId].keyholders + 1 <= 6 then
                ...
                TriggerClientEvent('VRCore:Notify', src, 'There Are No Slots Left')
            end
        end
    else
        TriggerClientEvent('VRCore:Notify', src, 'Error Has Occurred')
    end
end)
...
VRCore.Commands.Add("entertraphouse", "Betreed traphouse", {}, false, function(source, args)
    local src = source
    local Player = VRCore.Functions.GetPlayer(src)
 
    TriggerClientEvent('vr-traphouse:client:EnterTraphouse', src)
end)

VRCore.Commands.Add("multikeys", "Give Keys To Traphouse", {{name = "id", help = "Player id"}}, true, function(source, args)
    local src = source
    local Player = VRCore.Functions.GetPlayer(src)
            ...
            if not HasCitizenIdHasKey(TargetData.PlayerData.citizenid, Traphouse) then
                if Config.TrapHouses[Traphouse] ~= nil then
                    ...
                    if Config.TrapHouses[Traphouse].keyholders == nil then
                        ...
                        TriggerClientEvent('vr-traphouse:client:SyncData', -1, Traphouse, Config.TrapHouses[Traphouse])
                    else
                ...
                TriggerClientEvent('VRCore:Notify', src, 'This Person Already Has Keys', 'error')
            end
        else
            TriggerClientEvent('VRCore:Notify', src, 'You Do Not Own A Traphouse Or Are Not The Owner', 'error')
        end
    else
        TriggerClientEvent('VRCore:Notify', src, 'This Person Is Not In The City', 'error')
    end
end)

RegisterServerEvent('vr-traphouse:server:TakeMoney')
AddEventHandler('vr-traphouse:server:TakeMoney', function(TraphouseId)
    local src = source
    local Player = VRCore.Functions.GetPlayer(src)
    if Config.TrapHouses[TraphouseId].money ~= 0 then
        ...
        TriggerClientEvent('VRCore:Notify', src, 'There issent any money in the cupboard', 'error')
    end
end)
...
RegisterServerEvent('vr-traphouse:server:RobNpc')
AddEventHandler('vr-traphouse:server:RobNpc', function(Traphouse)
    local src = source
    local Player = VRCore.Functions.GetPlayer(src)
...
VRCore.Functions.CreateCallback('vr-traphouse:server:GetTraphousesData', function(source, cb)
