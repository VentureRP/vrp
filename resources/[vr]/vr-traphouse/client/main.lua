...
Citizen.CreateThread(function()
    ...
    if VRCore.Functions.GetPlayerData() ~= nil then
        ...
        VRCore.Functions.TriggerCallback('vr-traphouse:server:GetTraphousesData', function(trappies)
...
RegisterNetEvent('vr-traphouse:client:EnterTraphouse')
AddEventHandler('vr-traphouse:client:EnterTraphouse', function(code)
    if ClosestTraphouse ~= nil then
...
RegisterNUICallback('EnterPincode', function(d)
    ...
    if tonumber(d.pin) == data.pincode then
        ...
        VRCore.Functions.Notify('This Code Is Incorrect', 'error')
    end
end)
                                    ...
                                    if not IsEntityDead(targetPed) then
                                        if CanRob then
                                            if InDistance then
                                                ...
                                                TriggerServerEvent('vr-traphouse:server:RobNpc', ClosestTraphouse)
                                                CanRob = false
                                            end
                                        end
                                    end
                ...
                if InteractDistance < 20 then
                    ...
                    if InteractDistance < 1 then
                        if not IsKeyHolder then
                            ...
                            if IsControlJustPressed(0, 38) then
                                TriggerServerEvent('vr-traphouse:server:TakeoverHouse', CurrentTraphouse)
                            end
                            ...
                            if IsHouseOwner then
                                ...
                                if IsControlJustPressed(0, 47) then
                                    VRCore.Functions.Notify('Pincode: '..data.pincode)
                                end
                            end
                            if IsControlJustPressed(0, 74) then
                                local TraphouseInventory = {}
                            ...
                            if IsControlJustPressed(0, 38) then
                                TriggerServerEvent("vr-traphouse:server:TakeMoney", CurrentTraphouse)
                            end
                        end
                    end
                end
...
function LeaveTraphouse(data)
    ...
    DoScreenFadeOut(250)
    Citizen.Wait(250)
    exports['vr-interior']:DespawnInterior(TraphouseObj, function()
...
RegisterNetEvent('vr-traphouse:client:TakeoverHouse')
AddEventHandler('vr-traphouse:client:TakeoverHouse', function(TraphouseId)
    local ped = PlayerPedId()

    VRCore.Functions.Progressbar("takeover_traphouse", "Taking Over", math.random(1000, 3000), false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
    }, {}, {}, {}, function() -- Done
        TriggerServerEvent('vr-traphouse:server:AddHouseKeyHolder', PlayerData.citizenid, TraphouseId, true)
    end, function()
        VRCore.Functions.Notify("Acquisitions Canceled", "error")
    end)
end)
...
function AddKeyHolder(CitizenId, Traphouse)
    if #Config.TrapHouses[Traphouse].keyholders <= 6 then
        if not HasCitizenIdHasKey(CitizenId, Traphouse) then
            ...
            VRCore.Functions.Notify(CitizenId..' Has Been Added To The Traphouse!')
        else
            VRCore.Functions.Notify(CitizenId..' This Person Already Has Keys')
        end
    else
        VRCore.Functions.Notify('You Can Give Up To 6 People Access To The Trap House!')
    end
    IsKeyHolder = HasKey(CitizenId)
    IsHouseOwner = IsOwner(CitizenId)
end

RegisterNetEvent('vr-traphouse:client:SyncData')
AddEventHandler('vr-traphouse:client:SyncData', function(k, data)
