    ...
    VRCore.Functions.GetPlayerData(function(PlayerData)
        ...
        if PlayerData.job.onduty then
            if PlayerData.job.name == "mechanic" then
                TriggerServerEvent("VRCore:ToggleDuty")
            end
        end
    end)
    isLoggedIn = true
    VRCore.Functions.TriggerCallback('vr-vehicletuning:server:GetAttachedVehicle', function(plates)
        for k, v in pairs(plates) do
            Config.Plates[k].AttachedVehicle = v.AttachedVehicle
        end
    end)

    VRCore.Functions.TriggerCallback('vr-vehicletuning:server:GetDrivingDistances', function(retval)
                                    ...
                                    if IsPedInAnyVehicle(PlayerPedId()) then
                                        if not IsThisModelABicycle(GetEntityModel(veh)) then
                                            ...
                                            if IsControlJustPressed(0, 38) then
                                                ...
                                                Wait(500)
                                                DoScreenFadeIn(250)
                                                TriggerServerEvent('vr-vehicletuning:server:SetAttachedVehicle', veh, k)
                                            end
                                        else
                                            VRCore.Functions.Notify("You Cannot Put Bicycles On The Platform!", "error")
                                        end
                                    end
                                end
                            end
...
function SpawnListVehicle(model)
    ...
    VRCore.Functions.SpawnVehicle(model, function(veh)
...
function PartMenu(part)
    ClearMenu()
    Menu.addButton("Repair ("..VRCore.Shared.Items[Config.RepairCostAmount[part].item]["label"].." "..Config.RepairCostAmount[part].costs.."x)", "RepairPart", part)
    Menu.addButton("Back", "VehicleOptions", nil)
    Menu.addButton("Close Menu", "CloseMenu", nil) 
end
...
function RepairPart(part)
    ...
    VRCore.Functions.TriggerCallback('vr-inventory:server:GetStashItems', function(StashItems)
        ...
        if hasitem and countitem >= PartData.costs then
            ...
            VRCore.Functions.Progressbar("repair_part", "Repairing " ..Config.ValuesLabels[part], math.random(5000, 10000), false, true, {
                ...
                TriggerEvent('vr-vehicletuning:client:RepaireeePart', part)
                TriggerServerEvent('vr-inventory:server:SaveStashItems', "mechanicstash", StashItems)
                SetTimeout(250, function()
                    PartsMenu()
                end)
            end, function()
                VRCore.Functions.Notify("Repair Cancelled", "error")
            end)
        else
            VRCore.Functions.Notify('There Are Not Enough Materials In The Safe', 'error')
        end
    end, "mechanicstash")
end
...
RegisterNetEvent('vr-vehicletuning:client:RepaireeePart')
AddEventHandler('vr-vehicletuning:client:RepaireeePart', function(part)
    ...
    VRCore.Functions.Notify("The "..Config.ValuesLabels[part].." Is Repaired!")
end)
...
function UnattachVehicle()
    ...
    TriggerServerEvent('vr-vehicletuning:server:SetAttachedVehicle', false, ClosestPlate)
end

RegisterNetEvent('vr-vehicletuning:client:SetAttachedVehicle')
AddEventHandler('vr-vehicletuning:client:SetAttachedVehicle', function(veh, key)
    if veh ~= false then
            ...
            if #(pos - vehpos) < 5.0 then
                if not IsThisModelABicycle(GetEntityModel(veh)) then
                    ...
                    if VehicleStatus[plate] ~= nil then 
                        ...
                        VRCore.Functions.Notify("Status Unknown", "error")
                    end
                else
                    VRCore.Functions.Notify("Not A Valid Vehicle", "error")
                end
            else
                VRCore.Functions.Notify("You Are Not Close Enough To The Vehicle", "error")
            end
        else
            VRCore.Functions.Notify("You Must Be In The Vehicle First", "error")
        end
    else
        VRCore.Functions.Notify("You Must Be Outside The Vehicle", "error")
    end
end)
...
AddEventHandler('vehiclemod:client:fixEverything', function()
    if (IsPedInAnyVehicle(PlayerPedId(), false)) then
        ...
        if not IsThisModelABicycle(GetEntityModel(veh)) and GetPedInVehicleSeat(veh, -1) == PlayerPedId() then
            ...
            VRCore.Functions.Notify("You Are Not The Driver Or On A Bicycle", "error")
        end
    else
...
AddEventHandler('vehiclemod:client:repairPart', function(part, level, needAmount)
    -- if CanReapair() then
        if not IsPedInAnyVehicle(PlayerPedId(), false) then
                ...
                if #(pos - vehpos) < 5.0 then
                    if not IsThisModelABicycle(GetEntityModel(veh)) then
                        ...
                        if VehicleStatus[plate] ~= nil and VehicleStatus[plate][part] ~= nil then
                            ...
                            VRCore.Functions.Progressbar("repair_advanced", "Repair Vehicle", lockpickTime, false, true, {
                                disableMovement = true,
                                disableCarMovement = true,
                                disableMouse = false,
                                disableCombat = true,
                                ...
                                if part == "body" then
                                    ...
                                    TriggerServerEvent("VRCore:Server:RemoveItem", Config.RepairCost[part], needAmount)
                                    TriggerEvent("inventory:client:ItemBox", VRCore.Shared.Items[Config.RepairCost[part]], "remove")
                                elseif part ~= "engine" then
                                    TriggerServerEvent("vehiclemod:server:updatePart", plate, part, GetVehicleStatus(plate, part) + level)
                                    TriggerServerEvent("VRCore:Server:RemoveItem", Config.RepairCost[part], level)
                                    TriggerEvent("inventory:client:ItemBox", VRCore.Shared.Items[Config.RepairCost[part]], "remove")
                                end
                            end, function() -- Cancel
                                openingDoor = false
                            ...
                            VRCore.Functions.Notify("Not A Valid Part", "error")
                        end
                    else
                        VRCore.Functions.Notify("Not A Valid Vehicle", "error")
                    end
            ...
            VRCore.Functions.Notify("Youre Not In a Vehicle", "error")
        end
    -- end
end)
...
function CanReapair()
    ...
    for k, v in pairs(Config.Businesses) do
        retval = exports['vr-companies']:IsEmployee(v)
    end
    return retval
end
...
function SendStatusMessage(statusList)
    if statusList ~= nil then 
        TriggerEvent('chat:addMessage', {
            ...
            args = {'Vehicle Status', round(statusList["engine"]) .. "/" .. Config.MaxStatusValues["engine"] .. " ("..VRCore.Shared.Items["advancedrepairkit"]["label"]..")", round(statusList["body"]) .. "/" .. Config.MaxStatusValues["body"] .. " ("..VRCore.Shared.Items[Config.RepairCost["body"]]["label"]..")", round(statusList["radiator"]) .. "/" .. Config.MaxStatusValues["radiator"] .. ".0 ("..VRCore.Shared.Items[Config.RepairCost["radiator"]]["label"]..")", round(statusList["axle"]) .. "/" .. Config.MaxStatusValues["axle"] .. ".0 ("..VRCore.Shared.Items[Config.RepairCost["axle"]]["label"]..")", round(statusList["brakes"]) .. "/" .. Config.MaxStatusValues["brakes"] .. ".0 ("..VRCore.Shared.Items[Config.RepairCost["brakes"]]["label"]..")", round(statusList["clutch"]) .. "/" .. Config.MaxStatusValues["clutch"] .. ".0 ("..VRCore.Shared.Items[Config.RepairCost["clutch"]]["label"]..")", round(statusList["fuel"]) .. "/" .. Config.MaxStatusValues["fuel"] .. ".0 ("..VRCore.Shared.Items[Config.RepairCost["fuel"]]["label"]..")"}
        })
    end
end
