...
AddEventHandler('VRCore:Client:OnPlayerLoaded', function()
    isLoggedIn = true
    TriggerServerEvent("vr-methlab:server:LoadLocationList")
end)
...
Citizen.CreateThread(function()
    Wait(500)
    VRCore.Functions.TriggerCallback('vr-methlab:server:GetData', function(data)
            ...
            if #(pos - vector3(Config.Locations["break"].coords.x, Config.Locations["break"].coords.y, Config.Locations["break"].coords.z)) < 20 then
                ...
                if #(pos - vector3(Config.Locations["break"].coords.x, Config.Locations["break"].coords.y, Config.Locations["break"].coords.z)) < 1 then
                    ...
                    if IsControlJustPressed(0, 47) then
                        TriggerServerEvent("vr-methlab:server:breakMeth")
                    end
                end
            end
        end
                ...
                if  not machineStarted then
                    if not loadIngredients then
                        if #(pos - vector3(Config.Tasks["Furnace"].coords.x, Config.Tasks["Furnace"].coords.y, Config.Tasks["Furnace"].coords.z)) < 1 then
                            ...
                            if IsControlJustPressed(0, 47) then
                                TriggerServerEvent("vr-methlab:server:CheckIngredients")
                            end
                        end
                    else
                        if not finishedMachine then
                            ...
                            if #(pos - vector3(Config.Tasks["Furnace"].coords.x, Config.Tasks["Furnace"].coords.y, Config.Tasks["Furnace"].coords.z)) < 1 then
                                ...
                                if IsControlJustPressed(0, 38) then
                                    TriggerServerEvent("vr-methlab:server:receivemethtray")
...
RegisterNetEvent('vr-methlab:client:breakMeth')
AddEventHandler('vr-methlab:client:breakMeth', function()
    PrepareAnim()
    BreakMinigame()
end)

RegisterNetEvent('vr-methlab:client:loadIngredients')
AddEventHandler('vr-methlab:client:loadIngredients', function()
    ProcessMinigame()
end)
...
function breakingMeth(amount)
    VRCore.Functions.Progressbar("break_meth", "Breaking Meth ..", Config.BreakMethTimer, false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
    }, {}, {}, {}, function() -- Done
        TriggerServerEvent("vr-methlab:server:getmethtray", amount)
        ClearPedTasks(PlayerPedId())
    end, function() -- Cancel
    end)
end
...
function ProcessMinigame()
    ...
    Skillbar.Start({
        ...
        if SucceededAttempts + 1 >= NeededAttempts then
            TriggerServerEvent('vr-methlab:server:loadIngredients')
            VRCore.Functions.Notify("You loaded the ingredients!", "success")
            FailedAttemps = 0
            SucceededAttempts = 0
            NeededAttempts = 0
...
RegisterNetEvent('vr-methlab:client:UseLabKey')
AddEventHandler('vr-methlab:client:UseLabKey', function(labkey)
    if ClosestMethlab == Config.CurrentLab then
        ...
        if dist < 1 then
            if labkey == ClosestMethlab then
                ...
                VRCore.Functions.Notify('This is not the correct key..', 'error')
            end
        end
    end
end)
                            ...
                            if dist < 1 then
                                 if labkey == ClosestMethlab then
                                    EnterMethlab()
                                     ...
                                     VRCore.Functions.Notify('Looks like it needs a key...', 'error')
                                 end
                            end
                        end
                     end
