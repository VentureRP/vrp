...
VRCore.Functions.CreateCallback('vr-debitcard:server:requestCards', function(source, cb)
    local src = source
...
VRCore.Functions.CreateCallback('vr-debitcard:server:deleteCard', function(source, cb, data)
...
RegisterCommand('atm', function(source, args, rawCommand)
    ...
    if visas ~= nil and masters ~= nil then
        for _, v in pairs(visas) do
            ...
            local xCH = VRCore.Functions.GetPlayerByCitizenId(cardHolder)
            if xCH ~= nil then
    ...
    TriggerClientEvent('vr-atms:client:loadATM', src, cards)
end)
...
Citizen.CreateThread(function()
    while true do
        Wait(3600000)
        dailyWithdraws = {}
        TriggerClientEvent('VRCore:Notify', -1, "Daily Withdraw Limit Reset", "success")
    end
end)

RegisterServerEvent('vr-atms:server:doAccountWithdraw')
AddEventHandler('vr-atms:server:doAccountWithdraw', function(data)
    if data ~= nil then 
        local src = source
        local xPlayer = VRCore.Functions.GetPlayer(src)
        ...
        if dailyWith < Config.DailyLimit then
            ...
            if xCH ~= nil then
                ...
                if bankCount > 0 then
                    ...
                    TriggerClientEvent('VRCore:Notify', src, "Withdraw $" .. data.amount .. ' from credit card. Daily Withdraws: ' .. dailyWithdraws[cardHolder], "success")
                else
                    TriggerClientEvent('VRCore:Notify', src, "You cant go into minus in ATM.", "error")
                end
            ...
            TriggerClientEvent('vr-atms:client:updateBankInformation', src, banking)
        else
            TriggerClientEvent('VRCore:Notify', src, "You have reached the daily limit.", "error")
        end
    end
end)


VRCore.Functions.CreateCallback('vr-atms:server:loadBankAccount', function(source, cb, cid, cardnumber)
    local src = source
    local xPlayer = VRCore.Functions.GetPlayer(src)
