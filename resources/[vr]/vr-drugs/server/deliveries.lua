RegisterServerEvent('vr-drugs:server:updateDealerItems')
AddEventHandler('vr-drugs:server:updateDealerItems', function(itemData, amount, dealer)
    local src = source
    ...
    if Config.Dealers[dealer]["products"][itemData.slot].amount - 1 >= 0 then
        ...
        TriggerClientEvent('vr-drugs:client:setDealerItems', -1, itemData, amount, dealer)
    else
        ...
        TriggerClientEvent("VRCore:Notify", src, "This item is not available.. You've got an refund.", "error")
    end
end)

RegisterServerEvent('vr-drugs:server:giveDeliveryItems')
AddEventHandler('vr-drugs:server:giveDeliveryItems', function(amount)
    local src = source
    local Player = VRCore.Functions.GetPlayer(src)

    Player.Functions.AddItem('weed_brick', amount)
    TriggerClientEvent('inventory:client:ItemBox', src, VRCore.Shared.Items["weed_brick"], "add")
end)

VRCore.Functions.CreateCallback('vr-drugs:server:RequestConfig', function(source, cb)
    cb(Config.Dealers)
end)

RegisterServerEvent('vr-drugs:server:succesDelivery')
AddEventHandler('vr-drugs:server:succesDelivery', function(deliveryData, inTime)
    local src = source
    local Player = VRCore.Functions.GetPlayer(src)
    ...
    if inTime then
        if Player.Functions.GetItemByName('weed_brick') ~= nil and Player.Functions.GetItemByName('weed_brick').amount >= deliveryData["amount"] then
            ...
            TriggerClientEvent('inventory:client:ItemBox', src, VRCore.Shared.Items["weed_brick"], "remove")
            TriggerClientEvent('VRCore:Notify', src, 'The order has been delivered completely', 'success')

            SetTimeout(math.random(5000, 10000), function()
                TriggerClientEvent('vr-drugs:client:sendDeliveryMail', src, 'perfect', deliveryData)

                Player.Functions.SetMetaData('dealerrep', (curRep + 1))
            end)
        else
            TriggerClientEvent('VRCore:Notify', src, 'This doesn\'t meet the order...', 'error')
            ...
            SetTimeout(math.random(5000, 10000), function()
                TriggerClientEvent('vr-drugs:client:sendDeliveryMail', src, 'bad', deliveryData)
        ...
        TriggerClientEvent('VRCore:Notify', src, 'You\'re too late...', 'error')
        ...
        SetTimeout(math.random(5000, 10000), function()
            TriggerClientEvent('vr-drugs:client:sendDeliveryMail', src, 'late', deliveryData)
...
RegisterServerEvent('vr-drugs:server:callCops')
AddEventHandler('vr-drugs:server:callCops', function(streetLabel, coords)
    ...
    for k, v in pairs(VRCore.Functions.GetPlayers()) do
        ...
        if Player ~= nil then 
            if (Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty) then
                TriggerClientEvent("vr-drugs:client:robberyCall", Player.PlayerData.source, msg, streetLabel, coords)
                TriggerClientEvent("vr-phone:client:addPoliceAlert", Player.PlayerData.source, alertData)
            end
        end
	end
end)
...
VRCore.Commands.Add("newdealer", "Place A Dealer (Admin Only)", {
    ...
    TriggerClientEvent('vr-drugs:client:CreateDealer', source, dealerName, mintime, maxtime)
end, "admin")

VRCore.Commands.Add("deletedealer", "Delete A Dealer (Admin Only)", {
    ...
    if result[1] ~= nil then
        ...
        TriggerClientEvent('vr-drugs:client:RefreshDealers', -1, Config.Dealers)
        TriggerClientEvent('VRCore:Notify', source, "Dealer: "..dealerName.." Has Been Deleted", "success")
    else
        TriggerClientEvent('VRCore:Notify', source, "Dealer: "..dealerName.." Doesn\'t Exist", "error")
    end
end, "admin")

VRCore.Commands.Add("dealers", "View All Dealers (Admin Only)", {}, false, function(source, args)
    local DealersText = ""
    if Config.Dealers ~= nil and next(Config.Dealers) ~= nil then
        ...
        TriggerClientEvent('VRCore:Notify', source, 'No dealers have been placed.', 'error')
    end
end, "admin")

VRCore.Commands.Add("dealergoto", "Teleport To A Dealer (Admin Only)", {{name = "name", help = "Dealer name"}}, true, function(source, args)
    local DealerName = tostring(args[1])

    if Config.Dealers[DealerName] ~= nil then
        TriggerClientEvent('vr-drugs:client:GotoDealer', source, Config.Dealers[DealerName])
    else
        TriggerClientEvent('VRCore:Notify', source, 'This dealer doesn\'t exist.', 'error')
    end
end, "admin")
...
RegisterServerEvent('vr-drugs:server:CreateDealer')
AddEventHandler('vr-drugs:server:CreateDealer', function(DealerData)
    local src = source
    local Player = VRCore.Functions.GetPlayer(src)
    local result = exports.ghmattimysql:executeSync('SELECT * FROM dealers WHERE name=@name', {['@name'] = DealerData.name})
    if result[1] ~= nil then
        TriggerClientEvent('VRCore:Notify', src, "A dealer already exists with this name..", "error")
    else
