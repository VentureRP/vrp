...
AddEventHandler('VRCore:Client:OnPlayerLoaded', function()
    disable = false
    TriggerServerEvent('vr-weathersync:server:RequestStateSync')
    TriggerServerEvent('vr-weathersync:server:RequestCommands')
end)

RegisterNetEvent('vr-weathersync:client:EnableSync')
AddEventHandler('vr-weathersync:client:EnableSync', function()
    disable = false
    TriggerServerEvent('vr-weathersync:server:RequestStateSync')
end)

RegisterNetEvent('vr-weathersync:client:DisableSync')
AddEventHandler('vr-weathersync:client:DisableSync', function()
	disable = true
...
RegisterNetEvent('vr-weathersync:client:SyncWeather')
AddEventHandler('vr-weathersync:client:SyncWeather', function(NewWeather, newblackout)
    CurrentWeather = NewWeather
    blackout = newblackout
end)

RegisterNetEvent('vr-weathersync:client:RequestCommands')
AddEventHandler('vr-weathersync:client:RequestCommands', function(isAllowed)
    if isAllowed then
...
RegisterNetEvent('vr-weathersync:client:SyncTime')
AddEventHandler('vr-weathersync:client:SyncTime', function(base, offset, freeze)
    freezeTime = freeze
    timeOffset = offset
    baseTime = base
end)
