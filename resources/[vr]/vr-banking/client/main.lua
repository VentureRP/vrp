...
RegisterNetEvent('vr-banking:client:syncBanks')
AddEventHandler('vr-banking:client:syncBanks', function(data)
    banks = data
    if showing then
        showing = false
    end
...
function openAccountScreen()
    VRCore.Functions.TriggerCallback('vr-banking:getBankingInformation', function(banking)
        if banking ~= nil then
            InBank = true
...
function atmRefresh()
    VRCore.Functions.TriggerCallback('vr-banking:getBankingInformation', function(infor)
        InBank = true
        SetNuiFocus(true, true)
        SendNUIMessage({
            status = "refreshatm",
...
RegisterNetEvent('vr-banking:openBankScreen')
AddEventHandler('vr-banking:openBankScreen', function()
    openAccountScreen()
end)
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(1)
        letSleep = true
        if playerLoaded and VRCore ~= nil and not InBank then 
...
RegisterNetEvent('vr-banking:transferError')
AddEventHandler('vr-banking:transferError', function(msg)
    SendNUIMessage({
...
RegisterNetEvent('vr-banking:successAlert')
AddEventHandler('vr-banking:successAlert', function(msg)
    SendNUIMessage({
