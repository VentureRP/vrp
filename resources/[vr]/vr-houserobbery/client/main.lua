...
AddEventHandler('VRCore:Client:OnPlayerLoaded', function()
    VRCore.Functions.TriggerCallback('vr-houserobbery:server:GetHouseConfig', function(HouseConfig)
...
Citizen.CreateThread(function()
    Citizen.Wait(500)
    requiredItems = {
        [1] = {name = VRCore.Shared.Items["lockpick"]["name"], image = VRCore.Shared.Items["lockpick"]["image"]},
        [2] = {name = VRCore.Shared.Items["screwdriverset"]["name"], image = VRCore.Shared.Items["screwdriverset"]["image"]},
    }
    while true do
        inRange = false
...
function enterRobberyHouse(house)
    ...
    if Config.Houses[house]["tier"] == 1 then
        data = exports['vr-interior']:CreateTier1HouseFurnished(coords)
    end
    Citizen.Wait(100)
    houseObj = data[1]
    POIOffsets = data[2]
...
RegisterNetEvent('vr-houserobbery:client:ResetHouseState')
AddEventHandler('vr-houserobbery:client:ResetHouseState', function(house)
...
RegisterNetEvent('vr-houserobbery:client:enterHouse')
AddEventHandler('vr-houserobbery:client:enterHouse', function(house)
            ...
            if closestHouse ~= nil then
                if CurrentCops >= Config.MinimumHouseRobberyPolice then
                    if not Config.Houses[closestHouse]["opened"] then
                        PoliceCall()
                        TriggerEvent('vr-lockpick:client:openLockpick', lockpickFinish)
                        if math.random(1, 100) <= 85 and not IsWearingHandshoes() then
                            local pos = GetEntityCoords(PlayerPedId())
                            TriggerServerEvent("evidence:server:CreateFingerDrop", pos)
                        end
                    else
                        VRCore.Functions.Notify('The door is already open..', 'error', 3500)
                    end
                else
                    VRCore.Functions.Notify('Not enough Police..', 'error', 3500)
                end
            end
        else
            VRCore.Functions.TriggerCallback('VRCore:HasItem', function(result)
                if closestHouse ~= nil then
                    if result then
                        ...
                        VRCore.Functions.Notify('It looks like you are missing something...', 'error', 3500)
                    end
                end
            end, "screwdriverset")
        end
...
function PoliceCall()
    ...
    if math.random(1, 100) <= chance then
        ...
        if closestPed ~= nil then
            ...
            local gender = "Man"
            if VRCore.Functions.GetPlayerData().charinfo.gender == 1 then
                gender = "Woman"
            end
...
function lockpickFinish(success)
    if success then
        TriggerServerEvent('vr-houserobbery:server:enterHouse', closestHouse)
        VRCore.Functions.Notify('It worked!', 'success', 2500)
    else
        if usingAdvanced then
            local itemInfo = VRCore.Shared.Items["advancedlockpick"]
            if math.random(1, 100) < 20 then
                TriggerServerEvent("VRCore:Server:RemoveItem", "advancedlockpick", 1)
                TriggerEvent('inventory:client:ItemBox', itemInfo, "remove")
            end
        else
            local itemInfo = VRCore.Shared.Items["lockpick"]
        ...
        VRCore.Functions.Notify('It did not work..', 'error', 2500)
    end
end

RegisterNetEvent('vr-houserobbery:client:setHouseState')
AddEventHandler('vr-houserobbery:client:setHouseState', function(house, state)
...
function searchCabin(cabin)
    ...
    TriggerServerEvent('vr-houserobbery:server:SetBusyState', cabin, currentHouse, true)
    ...
    Skillbar.Start({
        ...
        if SucceededAttempts + 1 >= NeededAttempts then
            ...
            openingDoor = false
            ClearPedTasks(PlayerPedId())
            TriggerServerEvent('vr-houserobbery:server:searchCabin', cabin, currentHouse)
            Config.Houses[currentHouse]["furniture"][cabin]["searched"] = true
            TriggerServerEvent('vr-houserobbery:server:SetBusyState', cabin, currentHouse, false)
            SucceededAttempts = 0
...
RegisterNetEvent('vr-houserobbery:client:setCabinState')
AddEventHandler('vr-houserobbery:client:setCabinState', function(house, cabin, state)
    Config.Houses[house]["furniture"][cabin]["searched"] = state
end)

RegisterNetEvent('vr-houserobbery:client:SetBusyState')
AddEventHandler('vr-houserobbery:client:SetBusyState', function(cabin, house, bool)
