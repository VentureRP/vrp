...
RegisterNetEvent('vr-drugs:client:cornerselling')
AddEventHandler('vr-drugs:client:cornerselling', function(data)
    VRCore.Functions.TriggerCallback('vr-drugs:server:cornerselling:getAvailableDrugs', function(result)
        if CurrentCops >= Config.MinimumDrugSalePolice then
            if result ~= nil then
                ...
                if not cornerselling then
                    ...
                    VRCore.Functions.Notify('You started selling drugs')
                    startLocation = GetEntityCoords(PlayerPedId())
                    -- TaskStartScenarioInPlace(PlayerPedId(), "CODE_HUMAN_CROSS_ROAD_WAIT", 0, false)
                else
                    cornerselling = false
                    LocalPlayer.state:set("inv_busy", false, true)
                    VRCore.Functions.Notify('You stopped selling drugs')
                    -- ClearPedTasks(PlayerPedId())
                end
            else
                VRCore.Functions.Notify('You aren\'t carrying any drugs with you..', 'error')
                LocalPlayer.state:set("inv_busy", false, true)
            end
        else
            VRCore.Functions.Notify("There Are Not Enough Police On Duty (".. Config.MinimumDrugSalePolice .." Required)", "error")
        end
    end)
end)
...
function toFarAway()
    VRCore.Functions.Notify('Moved Too Far!', 'error')
    LocalPlayer.state:set("inv_busy", false, true)
    cornerselling = false
    hasTarget = false
    busySelling = false
            ...
            if IsEntityDead(stealingPed) then
                ...
                if #(pos - pedpos) < 1.5 then
                    ...
                    if IsControlJustReleased(0, 38) then
                        ...
                        Citizen.Wait(2000)
                        ClearPedTasks(ped)
                        TriggerServerEvent("VRCore:Server:AddItem", stealData.item, stealData.amount)
                        TriggerEvent('inventory:client:ItemBox', VRCore.Shared.Items[stealData.item], "add")
                        stealingPed = nil
                        stealData = {}
                    end
                end
...
RegisterNetEvent('vr-drugs:client:refreshAvailableDrugs')
AddEventHandler('vr-drugs:client:refreshAvailableDrugs', function(items)
    availableDrugs = items
    if #availableDrugs <= 0 then
        VRCore.Functions.Notify('No more drugs left to sell!', 'error')
        cornerselling = false
        LocalPlayer.state:set("inv_busy", false, true)
    end
end)
...
function SellToPed(ped)
    ...
    if hasTarget then
        while pedDist < 1.5 do
            ...
            if getRobbed == 18 or getRobbed == 9 then
                TriggerServerEvent('vr-drugs:server:robCornerDrugs', availableDrugs[drugType].item, bagAmount)
                VRCore.Functions.Notify('You have been robbed and lost '..bagAmount..' bags(\'s) '..availableDrugs[drugType].label, 'error')
                stealingPed = ped
                stealData = {
                ...
                if pedDist < 1.5 and cornerselling then
                    VRCore.Functions.DrawText3D(pedCoords.x, pedCoords.y, pedCoords.z, '~g~E~w~ '..bagAmount..'x '..currentOfferDrug.label..' for $'..randomPrice..'? / ~g~G~w~ Decline offer')
                    if IsControlJustPressed(0, 38) then
                        TriggerServerEvent('vr-drugs:server:sellCornerDrugs', availableDrugs[drugType].item, bagAmount, randomPrice)
                        hasTarget = false
                    ...
                    if IsControlJustPressed(0, 47) then
                        VRCore.Functions.Notify('Offer canceled!', 'error')
                        hasTarget = false
