                    ...
                    if Races[k] ~= nil then
                        if #(pos - vector3(Races[k].startx, Races[k].starty, Races[k].startz)) < 15.0 and not Races[k].started then
                            ...
                            if IsControlJustReleased(0, 74) then
                                TriggerServerEvent("vr-streetraces:JoinRace", k)
                            end
                        end
                    end
            ...
            if RaceId ~= 0 and InRace then
                if #(pos - vector3(Races[RaceId].endx, Races[RaceId].endy, pos.z)) < 250.0 and Races[RaceId].started then
                    ...
                    if #(pos - vector3(Races[RaceId].endx, Races[RaceId].endy, pos.z)) < 15.0 then
                        TriggerServerEvent("vr-streetraces:RaceWon", RaceId)
                        InRace = false
                    end
                end
            end
...
RegisterNetEvent('vr-streetraces:StartRace')
AddEventHandler('vr-streetraces:StartRace', function(race)
...
RegisterNetEvent('vr-streetraces:RaceDone')
AddEventHandler('vr-streetraces:RaceDone', function(race, winner)
    if RaceId ~= 0 and RaceId == race then
        RaceId = 0
        InRace = false
        VRCore.Functions.Notify("Race Is Over! The Winner Is "..winner.. "!")
    end
end)

RegisterNetEvent('vr-streetraces:StopRace')
AddEventHandler('vr-streetraces:StopRace', function()
    RaceId = 0
    InRace = false
end)

RegisterNetEvent('vr-streetraces:CreateRace')
AddEventHandler('vr-streetraces:CreateRace', function(amount)
    local pos = GetEntityCoords(PlayerPedId(), true)
    ...
    if DoesBlipExist(WaypointHandle) then
        ...
        if #(pos - vector3(cx, cy, groundZ)) > 500.0 then
            ...
            TriggerServerEvent("vr-streetraces:NewRace", race)
            VRCore.Functions.Notify("Race Made For $"..amount.."", "success")
        else
            VRCore.Functions.Notify("End Position Is Too Close", "error")
        end
    else
        VRCore.Functions.Notify("You Need To Drop A Marker", "error")
    end
end)

RegisterNetEvent('vr-streetraces:SetRace')
AddEventHandler('vr-streetraces:SetRace', function(RaceTable)
    Races = RaceTable
end)

RegisterNetEvent('vr-streetraces:SetRaceId')
AddEventHandler('vr-streetraces:SetRaceId', function(race)
    RaceId = race
    SetNewWaypoint(Races[RaceId].endx, Races[RaceId].endy)
end)
...
function RaceCountDown()
    ...
    while RaceCount ~= 0 do
        ...
        VRCore.Functions.Notify(RaceCount, 'primary', 800)
        Citizen.Wait(1000)
    ...
    VRCore.Functions.Notify("GOOOOOOOOO!!!")
end
