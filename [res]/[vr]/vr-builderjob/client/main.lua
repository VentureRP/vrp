...
function GetCurrentProject()
    VRCore.Functions.TriggerCallback('vr-builderjob:server:GetCurrentProject', function(BuilderConfig)
                    ...
                    if TaskData ~= nil then
                        ...
                        if TaskData.completed == TaskData.total then
                            ...
                            if IsControlJustPressed(0, 47) then
                                TriggerServerEvent('vr-builderjob:server:FinishProject')
                            end
                        end
...
function DoTask()
    ...
    local CountDown = 5
    TriggerServerEvent('vr-builderjob:server:SetTaskState', BuilderData.CurrentTask, true, false)
    ...
    if TaskData.type == "hammer" then
        ...
        TriggerServerEvent('vr-builderjob:server:SetTaskState', BuilderData.CurrentTask, true, true)
    end
end

RegisterNetEvent('vr-builderjob:client:SetTaskState')
AddEventHandler('vr-builderjob:client:SetTaskState', function(Task, IsBusy, IsCompleted)
...
RegisterNetEvent('vr-builderjob:client:FinishProject')
AddEventHandler('vr-builderjob:client:FinishProject', function(BuilderConfig)
    Config = BuilderConfig
end)