VRCore.Functions.CreateCallback('vr-weed:server:getBuildingPlants', function(source, cb, building)
...
RegisterServerEvent('vr-weed:server:placePlant')
AddEventHandler('vr-weed:server:placePlant', function(coords, sort, currentHouse)
    ...
    TriggerClientEvent('vr-weed:client:refreshHousePlants', -1, currentHouse)
end)

RegisterServerEvent('vr-weed:server:removeDeathPlant')
AddEventHandler('vr-weed:server:removeDeathPlant', function(building, plantId)
    ...
    TriggerClientEvent('vr-weed:client:refreshHousePlants', -1, building)
end)
...
Citizen.CreateThread(function()
    while true do
        ...
        TriggerClientEvent('vr-weed:client:refreshPlantStats', -1)
        ...
        for k, v in pairs(housePlants) do
            if housePlants[k].health > 50 then
                ...
                elseif housePlants[k].progress + Grow >= 100 then
                    if housePlants[k].stage ~= VRWeed.Plants[housePlants[k].sort]["highestStage"] then
...
VRCore.Functions.CreateUseableItem("weed_white-widow_seed", function(source, item)
    local Player = VRCore.Functions.GetPlayer(source)
    TriggerClientEvent('vr-weed:client:placePlant', source, 'white-widow', item)
end)

VRCore.Functions.CreateUseableItem("weed_skunk_seed", function(source, item)
    local Player = VRCore.Functions.GetPlayer(source)
    TriggerClientEvent('vr-weed:client:placePlant', source, 'skunk', item)
end)

VRCore.Functions.CreateUseableItem("weed_purple-haze_seed", function(source, item)
    local Player = VRCore.Functions.GetPlayer(source)
    TriggerClientEvent('vr-weed:client:placePlant', source, 'purple-haze', item)
end)

VRCore.Functions.CreateUseableItem("weed_og-kush_seed", function(source, item)
    local Player = VRCore.Functions.GetPlayer(source)
    TriggerClientEvent('vr-weed:client:placePlant', source, 'og-kush', item)
end)

VRCore.Functions.CreateUseableItem("weed_amnesia_seed", function(source, item)
    local Player = VRCore.Functions.GetPlayer(source)
    TriggerClientEvent('vr-weed:client:placePlant', source, 'amnesia', item)
end)

VRCore.Functions.CreateUseableItem("weed_ak47_seed", function(source, item)
    local Player = VRCore.Functions.GetPlayer(source)
    TriggerClientEvent('vr-weed:client:placePlant', source, 'ak47', item)
end)

VRCore.Functions.CreateUseableItem("weed_nutrition", function(source, item)
    local Player = VRCore.Functions.GetPlayer(source)
    TriggerClientEvent('vr-weed:client:foodPlant', source, item)
end)

RegisterServerEvent('vr-weed:server:removeSeed')
AddEventHandler('vr-weed:server:removeSeed', function(itemslot, seed)
    local Player = VRCore.Functions.GetPlayer(source)
    Player.Functions.RemoveItem(seed, 1, itemslot)
end)

RegisterServerEvent('vr-weed:server:harvestPlant')
AddEventHandler('vr-weed:server:harvestPlant', function(house, amount, plantName, plantId)
    local src = source
        ...
        if weedBag.amount >= sndAmount then
            if house ~= nil then
                ...
                if result[1] ~= nil then
                    ...
                    TriggerClientEvent('VRCore:Notify', src, 'The plant has been harvested', 'success', 3500)
                    TriggerClientEvent('vr-weed:client:refreshHousePlants', -1, house)
                else
                    TriggerClientEvent('VRCore:Notify', src, 'This plant no longer exists?', 'error', 3500)
                end
            else
                TriggerClientEvent('VRCore:Notify', src, 'House Not Found', 'error', 3500)
            end
        else
            TriggerClientEvent('VRCore:Notify', src, "You Don't Have Enough Resealable Bags", 'error', 3500)
        end
    else
        TriggerClientEvent('VRCore:Notify', src, "You Don't Have Enough Resealable Bags", 'error', 3500)
    end
...
RegisterServerEvent('vr-weed:server:foodPlant')
AddEventHandler('vr-weed:server:foodPlant', function(house, amount, plantName, plantId)
    local src = source
    local Player = VRCore.Functions.GetPlayer(src)
    ...
    TriggerClientEvent('VRCore:Notify', src, VRWeed.Plants[plantName]["label"]..' | Nutrition: '..plantStats[1].food..'% + '..amount..'% ('..(plantStats[1].food + amount)..'%)', 'success', 3500)
