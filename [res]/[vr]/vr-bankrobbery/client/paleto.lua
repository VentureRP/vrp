...
Citizen.CreateThread(function()
    Citizen.Wait(2000)
    local requiredItems = {
        [1] = {name = VRCore.Shared.Items["security_card_01"]["name"], image = VRCore.Shared.Items["security_card_01"]["image"]},
    }
                                    ...
                                    if lockerDist < 0.5 then
                                        ...
                                        if IsControlJustPressed(0, 38) then
                                            if CurrentCops >= Config.MinimumPaletoPolice then
                                                ...
                                                VRCore.Functions.Notify('Minimum Of '..Config.MinimumPaletoPolice..' Police Needed', "error")
                                            end
                                        end
                                    end
                                end
...
RegisterNetEvent('vr-bankrobbery:UseBankcardA')
AddEventHandler('vr-bankrobbery:UseBankcardA', function()
    local ped = PlayerPedId()
    local pos = GetEntityCoords(ped)
    local dist = #(pos - Config.BigBanks["paleto"]["coords"])
        ...
        VRCore.Functions.TriggerCallback('vr-bankrobbery:server:isRobberyActive', function(isBusy)
            if not isBusy then
                if CurrentCops >= Config.MinimumPaletoPolice then
                    if not Config.BigBanks["paleto"]["isOpened"] then 
                        TriggerEvent('inventory:client:requiredItems', requiredItems, false)
                        VRCore.Functions.Progressbar("security_pass", "Validitating card..", math.random(5000, 10000), false, true, {
                            ...
                            TriggerServerEvent('vr-bankrobbery:server:setBankState', "paleto", true)
                            TriggerServerEvent("VRCore:Server:RemoveItem", "security_card_01", 1)
                            TriggerServerEvent('vr-doorlock:server:updateState', 4, false)
                            if not copsCalled then
                                ...
                                if Config.BigBanks["paleto"]["alarm"] then
                                    TriggerServerEvent("vr-bankrobbery:server:callCops", "paleto", 0, streetLabel, pos)
                                    copsCalled = true
                                end
                            end
                        end, function() -- Cancel
                        ...
                        VRCore.Functions.Notify("It looks like the bank is already opened..", "error")
                    end
                else
                    VRCore.Functions.Notify('Minimum Of '..Config.MinimumPaletoPolice..' Police Needed', "error")
                end
            else
                VRCore.Functions.Notify("The security lock is active, the door cannot be opened at the moment..", "error", 5500)
            end
        end)
    end 
end)
...
function OpenPaletoDoor()
    TriggerServerEvent('vr-doorlock:server:updateState', 85, false)
