...
AddEventHandler("VRCore:Client:OnPlayerLoaded", function()
    ...
    VRCore.Functions.TriggerCallback('vr-bankrobbery:server:GetConfig', function(config)
        Config = config
    end)
    onDuty = true
    ResetBankDoors()
...
Citizen.CreateThread(function()
    Wait(500)
    if VRCore.Functions.GetPlayerData() ~= nil then
        PlayerJob = VRCore.Functions.GetPlayerData().job
        onDuty = true
    end
end)

RegisterNetEvent('VRCore:Client:SetDuty')
AddEventHandler('VRCore:Client:SetDuty', function(duty)
    onDuty = duty
end)
...
Citizen.CreateThread(function()
    Citizen.Wait(2000)
    local requiredItems = {
        [1] = {name = VRCore.Shared.Items["electronickit"]["name"], image = VRCore.Shared.Items["electronickit"]["image"]},
        [2] = {name = VRCore.Shared.Items["trojan_usb"]["name"], image = VRCore.Shared.Items["trojan_usb"]["image"]},
    }
    local requiredItems2 = {
        [1] = {name = VRCore.Shared.Items["thermite"]["name"], image = VRCore.Shared.Items["thermite"]["image"]},
    }
    while true do
        local ped = PlayerPedId()
        local pos = GetEntityCoords(ped)
                                    ...
                                    if lockerDist < 0.5 then
                                        ...
                                        if IsControlJustPressed(0, 38) then
                                            if CurrentCops >= Config.MinimumFleecaPolice then
                                                ...
                                                VRCore.Functions.Notify('Minimum Of '..Config.MinimumFleecaPolice..' Police Needed', "error") 
                                            end
                                        end
                                    end
                                end
...
AddEventHandler('electronickit:UseElectronickit', function()
    ...
    if closestBank ~= nil then
        VRCore.Functions.TriggerCallback('vr-bankrobbery:server:isRobberyActive', function(isBusy)
            if not isBusy then
                if closestBank ~= nil then
                    local dist = #(pos - Config.SmallBanks[closestBank]["coords"])
                    if dist < 1.5 then
                        if CurrentCops >= Config.MinimumFleecaPolice then
                            if not Config.SmallBanks[closestBank]["isOpened"] then 
                                VRCore.Functions.TriggerCallback('VRCore:HasItem', function(result)
                                    if result then 
                                        TriggerEvent('inventory:client:requiredItems', requiredItems, false)
                                        VRCore.Functions.Progressbar("hack_gate", "Connecting the hacking device ..", math.random(5000, 10000), false, true, {
                                            ...
                                            TriggerServerEvent("VRCore:Server:RemoveItem", "electronickit", 1)
                                            TriggerEvent('inventory:client:ItemBox', VRCore.Shared.Items["electronickit"], "remove")
                                            TriggerServerEvent("VRCore:Server:RemoveItem", "trojan_usb", 1)
                                            TriggerEvent('inventory:client:ItemBox', VRCore.Shared.Items["trojan_usb"], "remove")
                                            ...
                                            if not copsCalled then
                                                ...
                                                if Config.SmallBanks[closestBank]["alarm"] then
                                                    TriggerServerEvent("vr-bankrobbery:server:callCops", "small", closestBank, streetLabel, pos)
                                                    copsCalled = true
                                                end
                                            end
                                        end, function() -- Cancel
                                        ...
                                        VRCore.Functions.Notify("You're missing an item ..", "error")
                                    end
                                end, "trojan_usb")
                            else
                                VRCore.Functions.Notify("Looks like the bank is already open ..", "error")
                            end
                        else
                            VRCore.Functions.Notify('Minimum Of '..Config.MinimumFleecaPolice..' Police Needed', "error")
                        end
                ...
                VRCore.Functions.Notify("The security lock is active, opening the door is currently not possible.", "error", 5500)
            end
        end)
    end
end)

RegisterNetEvent('vr-bankrobbery:client:setBankState')
AddEventHandler('vr-bankrobbery:client:setBankState', function(bankId, state)
...
RegisterNetEvent('vr-bankrobbery:client:enableAllBankSecurity')
AddEventHandler('vr-bankrobbery:client:enableAllBankSecurity', function()
    for k, v in pairs(Config.SmallBanks) do
        Config.SmallBanks[k]["alarm"] = true
    end
end)

RegisterNetEvent('vr-bankrobbery:client:disableAllBankSecurity')
AddEventHandler('vr-bankrobbery:client:disableAllBankSecurity', function()
    for k, v in pairs(Config.SmallBanks) do
        Config.SmallBanks[k]["alarm"] = false
    end
end)

RegisterNetEvent('vr-bankrobbery:client:BankSecurity')
AddEventHandler('vr-bankrobbery:client:BankSecurity', function(key, status)
...
function openLocker(bankId, lockerId)
    ...
    TriggerServerEvent('vr-bankrobbery:server:setLockerState', bankId, lockerId, 'isBusy', true)
    if bankId == "paleto" then
        VRCore.Functions.TriggerCallback('VRCore:HasItem', function(hasItem)
            if hasItem then
                ...
                IsDrilling = true
                VRCore.Functions.Progressbar("open_locker_drill", "Breaking open the safe ..", math.random(18000, 30000), false, true, {
                    disableMovement = true,
                    disableCarMovement = true,
                    disableMouse = false,
                    disableCombat = true,
                    ...
                    TriggerServerEvent('vr-bankrobbery:server:setLockerState', bankId, lockerId, 'isOpened', true)
                    TriggerServerEvent('vr-bankrobbery:server:setLockerState', bankId, lockerId, 'isBusy', false)
                    TriggerServerEvent('vr-bankrobbery:server:recieveItem', 'paleto')
                    VRCore.Functions.Notify("Successful!", "success")
                    IsDrilling = false
                end, function() -- Cancel
                    StopAnimTask(PlayerPedId(), "anim@heists@fleeca_bank@drilling", "drill_straight_idle", 1.0)
                    TriggerServerEvent('vr-bankrobbery:server:setLockerState', bankId, lockerId, 'isBusy', false)
                ...
                VRCore.Functions.Notify("Looks like the safe lock is too strong ..", "error")
                TriggerServerEvent('vr-bankrobbery:server:setLockerState', bankId, lockerId, 'isBusy', false)
            end
        end, "drill")
    elseif bankId == "pacific" then
        VRCore.Functions.TriggerCallback('VRCore:HasItem', function(hasItem)
            if hasItem then
                ...
                VRCore.Functions.Progressbar("open_locker_drill", "Breaking open the safe ..", math.random(18000, 30000), false, true, {
                    ...
                    TriggerServerEvent('vr-bankrobbery:server:setLockerState', bankId, lockerId, 'isOpened', true)
                    TriggerServerEvent('vr-bankrobbery:server:setLockerState', bankId, lockerId, 'isBusy', false)
                    TriggerServerEvent('vr-bankrobbery:server:recieveItem', 'pacific')
                    VRCore.Functions.Notify("Successful!", "success")
                    IsDrilling = false
                end, function() -- Cancel
                    StopAnimTask(PlayerPedId(), "anim@heists@fleeca_bank@drilling", "drill_straight_idle", 1.0)
        ...
        IsDrilling = true
        VRCore.Functions.Progressbar("open_locker", "Breaking open the safe ..", math.random(27000, 37000), false, true, {
            disableMovement = true,
            disableCarMovement = true,
            disableMouse = false,
            disableCombat = true,
            ...
            TriggerServerEvent('vr-bankrobbery:server:setLockerState', bankId, lockerId, 'isOpened', true)
            TriggerServerEvent('vr-bankrobbery:server:setLockerState', bankId, lockerId, 'isBusy', false)
            TriggerServerEvent('vr-bankrobbery:server:recieveItem', 'small')
            VRCore.Functions.Notify("Successful!", "success")
            IsDrilling = false
        end, function() -- Cancel
            StopAnimTask(PlayerPedId(), "anim@gangops@facility@servers@", "hotwire", 1.0)
...
RegisterNetEvent('vr-bankrobbery:client:setLockerState')
AddEventHandler('vr-bankrobbery:client:setLockerState', function(bankId, lockerId, state, bool)
    if bankId == "paleto" then
...
RegisterNetEvent('vr-bankrobbery:client:ResetFleecaLockers')
AddEventHandler('vr-bankrobbery:client:ResetFleecaLockers', function(BankId)
...
RegisterNetEvent('vr-bankrobbery:client:robberyCall')
AddEventHandler('vr-bankrobbery:client:robberyCall', function(type, key, streetLabel, coords)
    if PlayerJob.name == "police" and onDuty then 
        ...
        if type == "small" then
            ...
            TriggerEvent('vr-policealerts:client:AddPoliceAlert', {
                timeOut = 10000,
                ...
                callSign = VRCore.Functions.GetPlayerData().metadata["callsign"],
            })
...
function OnHackDone(success, timeremaining)
    if success then
        ...
        TriggerServerEvent('vr-bankrobbery:server:setBankState', closestBank, true)
    else
		TriggerEvent('mhacking:hide')
	end
end
