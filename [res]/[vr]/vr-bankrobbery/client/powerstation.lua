        ...
        if dist < 1.5 then
            if CurrentCops >= Config.MinimumThermitePolice then
                if not Config.PowerStations[closestStation].hit then
                    ...
                    VRCore.Functions.Notify("It seems that the fuses have blown.", "error")
                end
            else
                VRCore.Functions.Notify('Minimum Of '..Config.MinimumThermitePolice..' Police Needed', "error")
            end
        end
...
RegisterNetEvent('vr-bankrobbery:client:SetStationStatus')
AddEventHandler('vr-bankrobbery:client:SetStationStatus', function(key, isHit)
...
RegisterNUICallback('thermitesuccess', function()
    ...
    local coords = GetEntityCoords(PlayerPedId())
    while time > 0 do 
        VRCore.Functions.Notify("Thermite is going off in " .. time .. "..")
        Citizen.Wait(1000)
        time = time - 1
    end
    local randTime = math.random(10000, 15000)
    ...
    if currentStation ~= 0 then
        ...
        TriggerServerEvent("vr-bankrobbery:server:SetStationStatus", currentStation, true)
    elseif currentGate ~= 0 then
        VRCore.Functions.Notify("The door is open", "success")
        TriggerServerEvent('vr-doorlock:server:updateState', currentGate, false)
        currentGate = 0
    end
end)
