...
Citizen.CreateThread(function()
    while true do
        ...
        if blackoutActive then
            TriggerEvent("vr-weathersync:server:toggleBlackout")
            TriggerClientEvent("police:client:EnableAllCameras", -1)
            TriggerClientEvent("vr-bankrobbery:client:enableAllBankSecurity", -1)
...
RegisterServerEvent('vr-bankrobbery:server:setBankState')
AddEventHandler('vr-bankrobbery:server:setBankState', function(bankId, state)
    if bankId == "paleto" then
        if not robberyBusy then
            Config.BigBanks["paleto"]["isOpened"] = state
            TriggerClientEvent('vr-bankrobbery:client:setBankState', -1, bankId, state)
            TriggerEvent('vr-scoreboard:server:SetActivityBusy', "bankrobbery", true)
            TriggerEvent('vr-bankrobbery:server:setTimeout')
        end
    elseif bankId == "pacific" then
        if not robberyBusy then
            Config.BigBanks["pacific"]["isOpened"] = state
            TriggerClientEvent('vr-bankrobbery:client:setBankState', -1, bankId, state)
            TriggerEvent('vr-scoreboard:server:SetActivityBusy', "pacific", true)
            TriggerEvent('vr-bankrobbery:server:setTimeout')
        end
    else
        if not robberyBusy then
            ...
            TriggerClientEvent('vr-bankrobbery:client:setBankState', -1, bankId, state)
            TriggerEvent('vr-banking:server:SetBankClosed', bankId, true)
            TriggerEvent('vr-scoreboard:server:SetActivityBusy', "bankrobbery", true)
            TriggerEvent('vr-bankrobbery:server:SetSmallbankTimeout', bankId)
        end
    end
    robberyBusy = true
end)

RegisterServerEvent('vr-bankrobbery:server:setLockerState')
AddEventHandler('vr-bankrobbery:server:setLockerState', function(bankId, lockerId, state, bool)
    if bankId == "paleto" then
    ...
    TriggerClientEvent('vr-bankrobbery:client:setLockerState', -1, bankId, lockerId, state, bool)
end)

RegisterServerEvent('vr-bankrobbery:server:recieveItem')
AddEventHandler('vr-bankrobbery:server:recieveItem', function(type)
    local src = source
        ...
        if WeaponChance ~= odd1 then
            if tier ~= 4 then
                 if Config.RewardTypes[itemType].type == "item" then
                     ...
                     TriggerClientEvent('inventory:client:ItemBox', src, VRCore.Shared.Items[item.item], "add")
                 elseif Config.RewardTypes[itemType].type == "money" then
                    local info = {
                        worth = math.random(2300, 3200)
                    }
                    ply.Functions.AddItem('markedbills', math.random(2,3), false, info)
                    TriggerClientEvent('inventory:client:ItemBox', src, VRCore.Shared.Items['markedbills'], "add")
                end
            else
                ply.Functions.AddItem('security_card_01', 1)
                TriggerClientEvent('inventory:client:ItemBox', src, VRCore.Shared.Items['security_card_01'], "add")
            end
        else
            ply.Functions.AddItem('weapon_stungun', 1)
            TriggerClientEvent('inventory:client:ItemBox', src, VRCore.Shared.Items['weapon_stungun'], "add")
        end
    elseif type == "paleto" then
        ...
        if WeaponChance ~= odd1 then
            if tier ~= 4 then
                ...
                TriggerClientEvent('inventory:client:ItemBox', src, VRCore.Shared.Items['security_card_02'], "add")
            end
        else
            ply.Functions.AddItem('weapon_vintagepistol', 1)
            TriggerClientEvent('inventory:client:ItemBox', src, VRCore.Shared.Items['weapon_vintagepistol'], "add")
        end
        ...
        if WeaponChance ~= odd1 or WeaponChance ~= odd2 then
            if tier ~= 4 then
                ...
                TriggerClientEvent('inventory:client:ItemBox', src, VRCore.Shared.Items['markedbills'], "add")
                 ...
                 TriggerClientEvent('inventory:client:ItemBox', src, VRCore.Shared.Items['cryptostick'], "add")
            end
        else
            ...
            if chance == odd then
                ...
                TriggerClientEvent('inventory:client:ItemBox', src, VRCore.Shared.Items['weapon_microsmg'], "add")
            else
                ply.Functions.AddItem('weapon_minismg', 1)
                TriggerClientEvent('inventory:client:ItemBox', src, VRCore.Shared.Items['weapon_minismg'], "add")
            end
...
VRCore.Functions.CreateCallback('vr-bankrobbery:server:isRobberyActive', function(source, cb)
    cb(robberyBusy)
end)

VRCore.Functions.CreateCallback('vr-bankrobbery:server:GetConfig', function(source, cb)
    cb(Config)
end)

RegisterServerEvent('vr-bankrobbery:server:setTimeout')
AddEventHandler('vr-bankrobbery:server:setTimeout', function()
    if not robberyBusy then
        if not timeOut then
            timeOut = true
            Citizen.CreateThread(function()
                ...
                timeOut = false
                robberyBusy = false
                TriggerEvent('vr-scoreboard:server:SetActivityBusy', "bankrobbery", false)
                TriggerEvent('vr-scoreboard:server:SetActivityBusy', "pacific", false)
                ...
                TriggerClientEvent('vr-bankrobbery:client:ClearTimeoutDoors', -1)
...
RegisterServerEvent('vr-bankrobbery:server:SetSmallbankTimeout')
AddEventHandler('vr-bankrobbery:server:SetSmallbankTimeout', function(BankId)
    if not robberyBusy then
        if not timeOut then
            timeOut = true
            Citizen.CreateThread(function()
                ...
                timeOut = false
                robberyBusy = false
            	TriggerClientEvent('vr-bankrobbery:client:ResetFleecaLockers', -1, BankId)
            	TriggerEvent('vr-banking:server:SetBankClosed', BankId, false)
            end)
	end
    end
end)

RegisterServerEvent('vr-bankrobbery:server:callCops')
AddEventHandler('vr-bankrobbery:server:callCops', function(type, bank, streetLabel, coords)
    local cameraId = 4
    ...
    TriggerClientEvent("vr-bankrobbery:client:robberyCall", -1, type, bank, streetLabel, coords)
    TriggerClientEvent("vr-phone:client:addPoliceAlert", -1, alertData)
end)

RegisterServerEvent('vr-bankrobbery:server:SetStationStatus')
AddEventHandler('vr-bankrobbery:server:SetStationStatus', function(key, isHit)
    Config.PowerStations[key].hit = isHit
    TriggerClientEvent("vr-bankrobbery:client:SetStationStatus", -1, key, isHit)
    if AllStationsHit() then
        TriggerEvent("vr-weathersync:server:toggleBlackout")
        TriggerClientEvent("police:client:DisableAllCameras", -1)
        TriggerClientEvent("vr-bankrobbery:client:disableAllBankSecurity", -1)
        blackoutActive = true
    else
        CheckStationHits()
    end
...
function CheckStationHits()
    ...
    if Config.PowerStations[11].hit and Config.PowerStations[1].hit and Config.PowerStations[2].hit then
        ...
        TriggerClientEvent("vr-bankrobbery:client:BankSecurity", 1, false)
        TriggerClientEvent("police:client:SetCamera", -1, 22, false)
        TriggerClientEvent("vr-bankrobbery:client:BankSecurity", 2, false)
    end
    if Config.PowerStations[8].hit and Config.PowerStations[4].hit and Config.PowerStations[5].hit and Config.PowerStations[6].hit then
        ...
        TriggerClientEvent("vr-bankrobbery:client:BankSecurity", 3, false)
    end
    if Config.PowerStations[12].hit and Config.PowerStations[13].hit then
        TriggerClientEvent("police:client:SetCamera", -1, 24, false)
        TriggerClientEvent("vr-bankrobbery:client:BankSecurity", 4, false)
        TriggerClientEvent("police:client:SetCamera", -1, 25, false)
        TriggerClientEvent("vr-bankrobbery:client:BankSecurity", 5, false)
    end
end
...
VRCore.Functions.CreateUseableItem("thermite", function(source, item)
    local Player = VRCore.Functions.GetPlayer(source)
	if Player.Functions.GetItemByName('lighter') ~= nil then
        TriggerClientEvent("thermite:UseThermite", source)
    else
        TriggerClientEvent('VRCore:Notify', source, "You're missing ignition source ", "error")
    end
end)

VRCore.Functions.CreateUseableItem("security_card_01", function(source, item)
    local Player = VRCore.Functions.GetPlayer(source)
	if Player.Functions.GetItemByName('security_card_01') ~= nil then
        TriggerClientEvent("vr-bankrobbery:UseBankcardA", source)
    end
end)

VRCore.Functions.CreateUseableItem("security_card_02", function(source, item)
    local Player = VRCore.Functions.GetPlayer(source)
	if Player.Functions.GetItemByName('security_card_02') ~= nil then
        TriggerClientEvent("vr-bankrobbery:UseBankcardB", source)
    end
end)

VRCore.Functions.CreateUseableItem("electronickit", function(source, item)
    local Player = VRCore.Functions.GetPlayer(source)
