            ...
            if bleedTickTimer >= Config.BleedTickRate and not isInHospitalBed then
                if not isDead and not InLaststand then
                    if isBleeding > 0 then
                        ...
                        TriggerServerEvent("evidence:server:CreateBloodDrop", VRCore.Functions.GetPlayerData().citizenid, VRCore.Functions.GetPlayerData().metadata["bloodtype"], coords)
...
AddEventHandler('hospital:client:UseBandage', function()
    ...
    VRCore.Functions.Progressbar("use_bandage", "Using bandage..", 4000, false, true, {
        ...
        TriggerServerEvent("VRCore:Server:RemoveItem", "bandage", 1)
        TriggerEvent("inventory:client:ItemBox", VRCore.Shared.Items["bandage"], "remove")
...
AddEventHandler('hospital:client:UsePainkillers', function()
    local ped = PlayerPedId()
    VRCore.Functions.Progressbar("use_bandage", "Taking painkillers", 3000, false, true, {
        disableMovement = false,
        disableCarMovement = false,
		disableMouse = false,
		disableCombat = true,
        ...
        TriggerServerEvent("VRCore:Server:RemoveItem", "painkillers", 1)
        TriggerEvent("inventory:client:ItemBox", VRCore.Shared.Items["painkillers"], "remove")
        onPainKillers = true
