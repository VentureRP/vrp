...
AddEventHandler('hospital:client:AiCall', function()
    ...
    local gender = VRCore.Functions.GetPlayerData().gender
...
AddEventHandler('hospital:client:CheckStatus', function()
    ...
    if player ~= -1 and distance < 5.0 then
        ...
        VRCore.Functions.TriggerCallback('hospital:GetPlayerStatus', function(result)
            if result ~= nil then
                for k, v in pairs(result) do
                    ...
                    elseif result["BLEED"] > 0 then
                        ...
                        VRCore.Functions.Notify('Player Is Healthy', 'success')
                    end
                end
        ...
        VRCore.Functions.Notify('No Player Nearby', 'error')
    end
end)

RegisterNetEvent('hospital:client:RevivePlayer') 
AddEventHandler('hospital:client:RevivePlayer', function()
    VRCore.Functions.TriggerCallback('hospital:server:HasFirstAid', function(hasItem)
        if hasItem then
            local player, distance = GetClosestPlayer()
            if player ~= -1 and distance < 5.0 then
                local playerId = GetPlayerServerId(player)
                isHealingPerson = true
                VRCore.Functions.Progressbar("hospital_revive", "Reviving person..", 5000, false, true, {
                    ...
                    VRCore.Functions.Notify("You revived the person!")
            ...
            VRCore.Functions.Notify("You Need A First Aid Kit", "error")
        end
    end, 'firstaid')
end)
...
AddEventHandler('hospital:client:TreatWounds', function()
    VRCore.Functions.TriggerCallback('hospital:server:HasBandage', function(hasItem)
        if hasItem then
            local player, distance = GetClosestPlayer()
            if player ~= -1 and distance < 5.0 then
                local playerId = GetPlayerServerId(player)
                isHealingPerson = true
                VRCore.Functions.Progressbar("hospital_healwounds", "Healing wounds..", 5000, false, true, {
                    disableMovement = false,
                    disableCarMovement = false,
                    disableMouse = false,
                    disableCombat = true,
                    ...
                    isHealingPerson = false
                    StopAnimTask(PlayerPedId(), healAnimDict, "exit", 1.0)
                    VRCore.Functions.Notify("You helped the person!")
            ...
            VRCore.Functions.Notify("You Need A Bandage", "error")
        end
    end, 'bandage')
end)
...
function TakeOutVehicle(vehicleInfo)
    ...
    VRCore.Functions.SpawnVehicle(vehicleInfo[1], function(veh)
