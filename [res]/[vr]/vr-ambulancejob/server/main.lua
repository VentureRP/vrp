...
AddEventHandler('hospital:server:SendToBed', function(bedId, isRevive)
	...
	TriggerEvent('vr-bossmenu:server:addAccountMoney', "ambulance", Config.BillCost)
...
AddEventHandler('hospital:server:RespawnAtHospital', function()
	...
	for k, v in pairs(Config.Locations["beds"]) do
		...
		Player.Functions.RemoveMoney("bank", Config.BillCost, "respawned-at-hospital")
		TriggerEvent('vr-bossmenu:server:addAccountMoney', "ambulance", Config.BillCost)
		TriggerClientEvent('VRCore:Notify', src, 'All your possessions have been taken..', 'error')
		TriggerClientEvent('hospital:client:SendBillEmail', src, Config.BillCost)
		return
	end
end)
...
AddEventHandler('hospital:server:TreatWounds', function(playerId)
	local src = source
	local Player = VRCore.Functions.GetPlayer(src)
	local Patient = VRCore.Functions.GetPlayer(playerId)
	if Patient ~= nil then
		if Player.PlayerData.job.name =="ambulance" then
			Player.Functions.RemoveItem('bandage', 1)
			TriggerClientEvent('inventory:client:ItemBox', source, VRCore.Shared.Items['bandage'], "remove")
	...
	if Patient ~= nil then
		if oldMan then
			if Player.Functions.RemoveMoney("cash", 5000, "revived-player") then
				...
				TriggerClientEvent('inventory:client:ItemBox', source, VRCore.Shared.Items['firstaid'], "remove")
				TriggerClientEvent('hospital:client:Revive', Patient.PlayerData.source)
			else
				TriggerClientEvent('VRCore:Notify', src, "You don\'t have enough money on you..", "error")
			end
		else
			Player.Functions.RemoveItem('firstaid', 1)
			TriggerClientEvent('inventory:client:ItemBox', source, VRCore.Shared.Items['firstaid'], "remove")
...
AddEventHandler('hospital:server:MakeDeadCall', function(blipSettings, gender, street1, street2)
	...
	if street2 ~= nil then
		...
		TriggerClientEvent('vr-policealerts:client:AddPoliceAlert', -1, {
            timeOut = 5000,
...
VRCore.Functions.CreateCallback('hospital:GetDoctors', function(source, cb)
	local amount = 0
	for k, v in pairs(VRCore.Functions.GetPlayers()) do
		local Player = VRCore.Functions.GetPlayer(v)
		if Player ~= nil then 
...
VRCore.Functions.CreateCallback('hospital:GetPlayerStatus', function(source, cb, playerId)
...
VRCore.Functions.CreateCallback('hospital:GetPlayerBleeding', function(source, cb)
	local src = source
...
VRCore.Commands.Add("status", "Check A Players Health", {}, false, function(source, args)
	local Player = VRCore.Functions.GetPlayer(source)
	if Player.PlayerData.job.name == "ambulance" then
		TriggerClientEvent("hospital:client:CheckStatus", source)
	else
		TriggerClientEvent('VRCore:Notify', source, "You Are Not EMS", "error")
	end
end)

VRCore.Commands.Add("heal", "Heal A Player", {}, false, function(source, args)
	local Player = VRCore.Functions.GetPlayer(source) 
	if Player.PlayerData.job.name == "ambulance" then
...
VRCore.Commands.Add("revivep", "Revive A Player", {}, false, function(source, args)
	local Player = VRCore.Functions.GetPlayer(source)
	if Player.PlayerData.job.name == "ambulance" then
...
VRCore.Commands.Add("revive", "Revive A Player or Yourself (Admin Only)", {{name="id", help="Player ID (may be empty)"}}, false, function(source, args)
	if args[1] ~= nil then
...
VRCore.Commands.Add("setpain", "Set Yours or A Players Pain Level (Admin Only)", {{name="id", help="Player ID (may be empty)"}}, false, function(source, args)
	if args[1] ~= nil then
		local Player = VRCore.Functions.GetPlayer(tonumber(args[1]))
		if Player ~= nil then
...
VRCore.Commands.Add("kill", "Kill A Player or Yourself (Admin Only)", {{name="id", help="Player ID (may be empty)"}}, false, function(source, args)
	if args[1] ~= nil then
		local Player = VRCore.Functions.GetPlayer(tonumber(args[1]))
		if Player ~= nil then
...
VRCore.Functions.CreateUseableItem("bandage", function(source, item)
	local Player = VRCore.Functions.GetPlayer(source)
...
VRCore.Functions.CreateCallback('hospital:server:HasBandage', function(source, cb)
    local player = VRCore.Functions.GetPlayer(source)
...
VRCore.Functions.CreateUseableItem("painkillers", function(source, item)
	local Player = VRCore.Functions.GetPlayer(source)
	if Player.Functions.GetItemByName(item.name) ~= nil then
...
VRCore.Functions.CreateUseableItem("firstaid", function(source, item)
	local Player = VRCore.Functions.GetPlayer(source)
	if Player.Functions.GetItemByName(item.name) ~= nil then
...
VRCore.Functions.CreateCallback('hospital:server:HasFirstAid', function(source, cb)
    local player = VRCore.Functions.GetPlayer(source)
...
AddEventHandler('hospital:server:CanHelp', function(helperId, canHelp)
	...
	if canHelp then
		...
		TriggerClientEvent('VRCore:Notify', helperId, "You can\'t help this person..", "error")
	end
end)
