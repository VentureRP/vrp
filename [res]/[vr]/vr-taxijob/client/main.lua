...
RegisterNetEvent('vr-taxi:client:DoTaxiNpc')
AddEventHandler('vr-taxi:client:DoTaxiNpc', function()
    if whitelistedVehicle() then
       -- if NpcData.CountDown == 180 then
            if not NpcData.Active then
                ...
                VRCore.Functions.Notify('The NPC Is Indicated On Your GPS', 'success')
                        ...
                        if dist < 20 then
                            ...
                            if dist < 5 then
                                ...
                                if IsControlJustPressed(0, 38) then
                                    ...
                                    VRCore.Functions.Notify('Bring The NPC To The Specified Location')
                                    if NpcData.NpcBlip ~= nil then
                                        RemoveBlip(NpcData.NpcBlip)
                                    end
                                    GetDeliveryLocation()
                ...
                VRCore.Functions.Notify('You Are Already Doing An NPC Mission')
            end
      --  else
         --   VRCore.Functions.Notify('No NPCs Are Available')
       -- end
    else
        VRCore.Functions.Notify('You Are Not In A Taxi')
    end
end)
            ...
            if dist < 20 then
                ...
                if dist < 5 then
                    ...
                    if IsControlJustPressed(0, 38) then
                        ...
                        TriggerServerEvent('vr-taxi:server:NpcPay', meterData.currentFare)
                        meterActive = false
                        SendNUIMessage({
                            action = "resetMeter"
                        })
                        VRCore.Functions.Notify('Person Was Dropped Off', 'success')
                        if NpcData.DeliveryBlip ~= nil then
                            RemoveBlip(NpcData.DeliveryBlip)
                        end
...
RegisterNetEvent('vr-taxi:client:toggleMeter')
AddEventHandler('vr-taxi:client:toggleMeter', function()
    local ped = PlayerPedId()
    ...
    if IsPedInAnyVehicle(ped, false) then
        if whitelistedVehicle() then
            ...
            VRCore.Functions.Notify('This Vehicle Has No Taxi Meter', 'error')
        end
    else
        VRCore.Functions.Notify('You\'re Not In A Vehicle ', 'error')
    end
end)

RegisterNetEvent('vr-taxi:client:enableMeter')
AddEventHandler('vr-taxi:client:enableMeter', function()
    local ped = PlayerPedId()
    ...
    if meterIsOpen then
        ...
        VRCore.Functions.Notify('The Taxi Meter Is Not Active', 'error')
    end
end)
...
RegisterNetEvent('vr-taxi:client:toggleMuis')
AddEventHandler('vr-taxi:client:toggleMuis', function()
    Citizen.Wait(400)
    if meterIsOpen then
        ...
        VRCore.Functions.Notify('No Taxi Meter In Sight', 'error')
    end
end)
...
function TakeVehicle(k)
    ...
    VRCore.Functions.SpawnVehicle(Config.AllowedVehicles[k].model, function(veh)
