...
RegisterNetEvent('vr-diving:client:NewLocations')
AddEventHandler('vr-diving:client:NewLocations', function()
...
RegisterNetEvent('vr-diving:client:SetDivingLocation')
AddEventHandler('vr-diving:client:SetDivingLocation', function(DivingLocation)
    ...
    Citizen.CreateThread(function()
        RadiusBlip = AddBlipForRadius(VRDiving.Locations[CurrentDivingLocation.Area].coords.Area.x, VRDiving.Locations[CurrentDivingLocation.Area].coords.Area.y, VRDiving.Locations[CurrentDivingLocation.Area].coords.Area.z, 100.0)
        ...
        LabelBlip = AddBlipForCoord(VRDiving.Locations[CurrentDivingLocation.Area].coords.Area.x, VRDiving.Locations[CurrentDivingLocation.Area].coords.Area.y, VRDiving.Locations[CurrentDivingLocation.Area].coords.Area.z)
...
Citizen.CreateThread(function()
    while true do
        ...
        if CurrentDivingLocation.Area ~= 0 then
            local AreaDistance = #(Pos - vector3(VRDiving.Locations[CurrentDivingLocation.Area].coords.Area.x, VRDiving.Locations[CurrentDivingLocation.Area].coords.Area.y, VRDiving.Locations[CurrentDivingLocation.Area].coords.Area.z))
            ...
            if inRange then
                for cur, CoralLocation in pairs(VRDiving.Locations[CurrentDivingLocation.Area].coords.Coral) do
                            ...
                            if not CoralLocation.PickedUp then
                                ...
                                if CoralDistance <= 1.5 then
                                    ...
                                    if IsControlJustPressed(0, 38) then
                                        ...
                                        VRCore.Functions.Progressbar("take_coral", "Collecting coral", times * 1000, false, true, {
...
function TakeCoral(coral)
    VRDiving.Locations[CurrentDivingLocation.Area].coords.Coral[coral].PickedUp = true
    TriggerServerEvent('vr-diving:server:TakeCoral', CurrentDivingLocation.Area, coral, true)
end

RegisterNetEvent('vr-diving:client:UpdateCoral')
AddEventHandler('vr-diving:client:UpdateCoral', function(Area, Coral, Bool)
...
function CallCops()
    ...
    if Call == Chance then
        TriggerServerEvent('vr-diving:server:CallCops', Coords)
    end
end

RegisterNetEvent('vr-diving:server:CallCops')
AddEventHandler('vr-diving:server:CallCops', function(Coords, msg)
...
RegisterNetEvent('vr-diving:client:UseGear')
AddEventHandler('vr-diving:client:UseGear', function(bool)
    if bool then
        GearAnim()
        VRCore.Functions.Progressbar("equip_gear", "Put on a diving suit", 5000, false, true, {}, {}, {}, {}, function() -- Done
            DeleteGear()
            ...
            TriggerServerEvent('vr-diving:server:RemoveGear')
        ...
        if currentGear.enabled then
            GearAnim()
            VRCore.Functions.Progressbar("remove_gear", "Pull out a diving suit ..", 5000, false, true, {}, {}, {}, {}, function() -- Done
                DeleteGear()

                SetEnableScuba(PlayerPedId(), false)
                SetPedMaxTimeUnderwater(PlayerPedId(), 1.00)
                currentGear.enabled = false
                TriggerServerEvent('vr-diving:server:GiveBackGear')
                ClearPedTasks(PlayerPedId())
                VRCore.Functions.Notify('You took your wetsuit off')
            end)
        else
            VRCore.Functions.Notify('You are not wearing a diving gear ..', 'error')
        end
    end
end)
...
RegisterNetEvent('vr-diving:client:RemoveGear')             --Add event to call externally
AddEventHandler('vr-diving:client:RemoveGear', function()
    TriggerEvent('vr-diving:client:UseGear', false)
end)