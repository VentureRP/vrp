...
AddEventHandler("VRCore:Client:OnPlayerLoaded", function()
    TriggerServerEvent("vr-scrapyard:server:LoadVehicleList")
end)
							...
							if IsControlJustReleased(0, 38) then
								if GetPedInVehicleSeat(vehicle, -1) == PlayerPedId() then
									if IsVehicleValid(GetEntityModel(vehicle)) then 
										...
										VRCore.Functions.Notify("This Vehicle Cannot Be Scrapped.", "error")
									end
								else
									VRCore.Functions.Notify("You Are Not The Driver", "error")
								end
							end
						end
					end
...
RegisterNetEvent('vr-scapyard:client:setNewVehicles')
AddEventHandler('vr-scapyard:client:setNewVehicles', function(vehicleList)
	...
	if Config.CurrentVehicles ~= nil and next(Config.CurrentVehicles) ~= nil then 
		...
		for k, v in pairs(Config.CurrentVehicles) do
			if Config.CurrentVehicles[k] ~= nil then 
				local vehicleInfo = VRCore.Shared.Vehicles[v]
		...
		VRCore.Functions.Notify("You Are Not Allowed To Demolish Vehicles Now", "error")
	end
end
...
function ScrapVehicle(vehicle)
	...
	VRCore.Functions.Progressbar("scrap_vehicle", "Demolish Vehicle", scrapTime, false, true, {
		...
		TriggerServerEvent("vr-scrapyard:server:ScrapVehicle", GetVehicleKey(GetEntityModel(vehicle)))
