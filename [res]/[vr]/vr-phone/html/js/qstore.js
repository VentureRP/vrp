...
$(document).on('click', '.storeapp-remove', function(e){
    ...
    VR.Phone.Data.Applications[AppData.app] = null;

    $.post('https://vr-phone/RemoveApplication', JSON.stringify({
        app: AppData.app
    }));
    setTimeout(function(){
...
$(document).on('click', '.download-password-accept', function(e){
    ...
    if (FilledInPassword == TemplatePassword) {
        ...
        $(".download-progressbar-fill").animate({
            ...
            $.post('https://vr-phone/InstallApplication', JSON.stringify({
                app: CurrentApp,
            }), function(Installed){
                if (Installed) {
                    var applicationSlot = $(".phone-applications").find('[data-appslot="'+Installed.data.slot+'"]');
                    var blockedapp = IsAppJobBlocked(Installed.data.blockedjobs, VR.Phone.Data.PlayerJob.name)
                    if ((!Installed.data.job || Installed.data.job === VR.Phone.Data.PlayerJob.name) && !blockedapp) {
                    ...
                    VR.Phone.Data.Applications[Installed.data.app] = Installed.data;
