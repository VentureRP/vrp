...
$(document).on('click', '.twitter-header-tab', function(e){
    ...
    if (PressedTwitterTab !== CurrentTwitterTab) {
        ...
        if (PressedTwitterTab === "twitter-mentions") {
            $.post('https://vr-phone/ClearMentions');
        }
...
$(document).on('click', '.twitter-new-tweet', function(e){
    ...
    VR.Phone.Animations.TopSlideDown(".twitter-new-tweet-tab", 450, 0);
});

VR.Phone.Notifications.LoadTweets = function(Tweets) {
    ...
    if (Tweets !== null && Tweets !== undefined && Tweets !== "" && Tweets.length > 0) {
        ...
        $.each(Tweets, function(i, Tweet){
            ...
            var TwtMessage = VR.Phone.Functions.FormatTwitterMessage(clean);
            var today = new Date();
...
VR.Phone.Notifications.LoadMentionedTweets = function(Tweets) {
    Tweets = Tweets.reverse();
    if (Tweets.length > 0) {
...
VR.Phone.Functions.FormatTwitterMessage = function(TwitterMessage) {
...
$(document).on('click', '#send-tweet', function(e){
    ...
    if (TweetMessage != "") {
        ...
        $.post('https://vr-phone/PostNewTweet', JSON.stringify({
            Message: TweetMessage,
            Date: CurrentDate,
            Picture: VR.Phone.Data.MetaData.profilepicture
        }), function(Tweets){
            VR.Phone.Notifications.LoadTweets(Tweets);
        });
        ...
        VR.Phone.Animations.TopSlideUp(".twitter-new-tweet-tab", 450, -120);
    } else {
        VR.Phone.Notifications.Add("fab fa-twitter", "Twitter", "Fill a message!", "#1DA1F2");
    }
});
...
$(document).on('click', '.hashtag-tag-text', function(e){
    ...
    if (!HashtagOpen) {
        ...
        $.post('https://vr-phone/GetHashtagMessages', JSON.stringify({hashtag: Hashtag}), function(HashtagData){
            VR.Phone.Notifications.LoadHashtagMessages(HashtagData.messages);
        });
...
function CopyMentionTag(elem) {
    ...
    VR.Phone.Notifications.Add("fab fa-twitter", "Twitter", $(elem).data('mentiontag')+ " copied!", "rgb(27, 149, 224)", 1250);
    document.execCommand("copy");
    $temp.remove();
}

VR.Phone.Notifications.LoadHashtags = function(hashtags) {
    if (hashtags !== null) {
...
VR.Phone.Notifications.LoadHashtagMessages = function(Tweets) {
    Tweets = Tweets.reverse();
    if (Tweets !== null && Tweets !== undefined && Tweets !== "" && Tweets.length > 0) {
...
$(document).on('click', '.twitter-hashtag', function(event){
    ...
    VR.Phone.Notifications.LoadHashtagMessages(TweetData.messages);
