...
RegisterCommand('phone', function()
    ...
    if not PhoneData.isOpen and isLoggedIn then
        local IsHandcuffed = exports['vr-policejob']:IsHandcuffed()
        if not IsHandcuffed and not PlayerData.metadata['inlaststand'] and not PlayerData.metadata['isdead'] then
            OpenPhone()
        else
            VRCore.Functions.Notify("Action not available at the moment..", "error")
        end
    end
end)
...
RegisterNetEvent('vr-phone:client:RaceNotify')
AddEventHandler('vr-phone:client:RaceNotify', function(message)
        SendNUIMessage({
...
RegisterNetEvent('vr-phone:client:AddRecentCall')
AddEventHandler('vr-phone:client:AddRecentCall', function(data, time, type)
    ...
    TriggerServerEvent('vr-phone:server:SetPhoneAlerts', "phone")
...
RegisterNUICallback('ClearRecentAlerts', function(data, cb)
    TriggerServerEvent('vr-phone:server:SetPhoneAlerts', "phone", 0)
...
RegisterNUICallback('SetBackground', function(data)
    ...
    TriggerServerEvent('vr-phone:server:SaveMetaData', PhoneData.MetaData)
end)
...
Citizen.CreateThread(function()
    while true do
        ...
        if isLoggedIn then
            VRCore.Functions.TriggerCallback('vr-phone:server:GetPhoneData', function(pData)   
...
function LoadPhone()
    ...
    VRCore.Functions.TriggerCallback('vr-phone:server:GetPhoneData', function(pData)
        ...
        PhoneData.PlayerData = VRCore.Functions.GetPlayerData()
...
RegisterNUICallback('HasPhone', function(data, cb)
    VRCore.Functions.TriggerCallback('vr-phone:server:HasPhone', function(HasPhone)
        cb(HasPhone)
    end)
end)
...
function OpenPhone()
    VRCore.Functions.TriggerCallback('vr-phone:server:HasPhone', function(HasPhone)
        if HasPhone then
            ...
            VRCore.Functions.TriggerCallback('vr-phone:server:GetGarageVehicles', function(vehicles)
                PhoneData.GarageVehicles = vehicles
            end)
        else
            VRCore.Functions.Notify("You don't have a phone", "error")
        end
    end)
end
...
RegisterNUICallback('RemoveMail', function(data, cb)
    ...
    TriggerServerEvent('vr-phone:server:RemoveMail', MailId)
    cb('ok')
end)

RegisterNetEvent('vr-phone:client:UpdateMails')
AddEventHandler('vr-phone:client:UpdateMails', function(NewMails)
    SendNUIMessage({
        action = "UpdateMails",
        Mails = NewMails
    })
...
RegisterNUICallback('AcceptMailButton', function(data)
    ...
    TriggerServerEvent('vr-phone:server:ClearButtonData', data.mailId)
end)
...
RegisterNUICallback('AddNewContact', function(data, cb)
    ...
    TriggerServerEvent('vr-phone:server:AddNewContact', data.ContactName, data.ContactNumber, data.ContactIban)
end)
...
RegisterNUICallback('GetProfilePicture', function(data, cb)
    ...
    VRCore.Functions.TriggerCallback('vr-phone:server:GetPicture', function(picture)
        cb(picture)
    end, number)
end)
...
RegisterNUICallback('SendMessage', function(data, cb)
    ...
    if PhoneData.Chats[NumberKey] ~= nil then
        ...
        if PhoneData.Chats[NumberKey].messages[ChatKey] ~= nil then
            ...
            TriggerServerEvent('vr-phone:server:UpdateMessages', PhoneData.Chats[NumberKey].messages, ChatNumber, false)
            ...
            TriggerServerEvent('vr-phone:server:UpdateMessages', PhoneData.Chats[NumberKey].messages, ChatNumber, true)
            NumberKey = GetKeyByNumber(ChatNumber)
            ReorganizeChats(NumberKey)
        end
    else
    ...
    VRCore.Functions.TriggerCallback('vr-phone:server:GetContactPicture', function(Chat)
        SendNUIMessage({
            action = "UpdateChat",
            chatData = Chat,
...
RegisterNetEvent('vr-phone:client:UpdateMessages')
AddEventHandler('vr-phone:client:UpdateMessages', function(ChatMessages, SenderNumber, New)
    ...
    if New then
        ...
        if PhoneData.isOpen then
            ...
            Wait(100)
            VRCore.Functions.TriggerCallback('vr-phone:server:GetContactPictures', function(Chats)
                SendNUIMessage({
                    action = "UpdateChat",
	    ...
	    SendNUIMessage({
            ...
            TriggerServerEvent('vr-phone:server:SetPhoneAlerts', "whatsapp")
        end
    else
...
RegisterNetEvent("vr-phone-new:client:BankNotify")
AddEventHandler("vr-phone-new:client:BankNotify", function(text)
    SendNUIMessage({
        action = "PhoneNotification",
        NotifyData = {
            title = "Bank", 
...
RegisterNetEvent('vr-phone:client:NewMailNotify')
AddEventHandler('vr-phone:client:NewMailNotify', function(MailData)
        SendNUIMessage({
            action = "PhoneNotification",
            PhoneNotify = {
                title = "Mail",
    ...
    TriggerServerEvent('vr-phone:server:SetPhoneAlerts', "mail")
end)

RegisterNUICallback('PostAdvert', function(data)
    TriggerServerEvent('vr-phone:server:AddAdvert', data.message)
end)

RegisterNetEvent('vr-phone:client:UpdateAdverts')
AddEventHandler('vr-phone:client:UpdateAdverts', function(Adverts, LastAd)
...
RegisterNUICallback('ClearAlerts', function(data, cb)
    ...
    if PhoneData.Chats[ChatKey].Unread ~= nil then
        ...
        TriggerServerEvent('vr-phone:server:SetPhoneAlerts', "whatsapp", newAlerts)
...
RegisterNUICallback('PayInvoice', function(data, cb)
    ...
    VRCore.Functions.TriggerCallback('vr-phone:server:PayInvoice', function(CanPay, Invoices)
        if CanPay then PhoneData.Invoices = Invoices end
        cb(CanPay)
    end, society, amount, invoiceId, senderCitizenId)
    TriggerServerEvent('vr-phone:server:BillingEmail', data, true)
end)
...
RegisterNUICallback('DeclineInvoice', function(data, cb)
    ...
    VRCore.Functions.TriggerCallback('vr-phone:server:DeclineInvoice', function(CanPay, Invoices)
        PhoneData.Invoices = Invoices
        cb('ok')
    end, society, amount, invoiceId)
    TriggerServerEvent('vr-phone:server:BillingEmail', data, false)
end)

RegisterNetEvent('vr-phone:client:BillingEmail')
AddEventHandler('vr-phone:client:BillingEmail', function(data, paid, name)
    if paid then
...
RegisterNUICallback('EditContact', function(data, cb)
    ...
    Citizen.Wait(100)
    cb(PhoneData.Contacts)
    TriggerServerEvent('vr-phone:server:EditContact', NewName, NewNumber, NewIban, OldName, OldNumber, OldIban)
end)
...
RegisterNetEvent('vr-phone:client:UpdateHashtags')
AddEventHandler('vr-phone:client:UpdateHashtags', function(Handle, msgData)
...
RegisterNUICallback('PostNewTweet', function(data, cb)
    ...
    for i = 2, #Hashtag, 1 do
        ...
        if Handle ~= nil or Handle ~= "" then
            ...
            TriggerServerEvent('vr-phone:server:UpdateHashtags', Handle, TweetMessage)
        end
    end
        ...
        if Handle ~= nil or Handle ~= "" then
            ...
            if (Firstname ~= nil and Firstname ~= "") and (Lastname ~= nil and Lastname ~= "") then
                if Firstname ~= PhoneData.PlayerData.charinfo.firstname and Lastname ~= PhoneData.PlayerData.charinfo.lastname then
                    TriggerServerEvent('vr-phone:server:MentionedPlayer', Firstname, Lastname, TweetMessage)
                end
            end
        end
    end
    ...
    TriggerServerEvent('vr-phone:server:UpdateTweets', PhoneData.Tweets, TweetMessage)
end)

RegisterNetEvent('vr-phone:client:TransferMoney')
AddEventHandler('vr-phone:client:TransferMoney', function(amount, newmoney)
...
RegisterNetEvent('vr-phone:client:UpdateTweets')
AddEventHandler('vr-phone:client:UpdateTweets', function(src, Tweets, NewTweetData)
...
RegisterNetEvent('vr-phone:client:GetMentioned')
AddEventHandler('vr-phone:client:GetMentioned', function(TweetMessage, AppAlerts)
...
RegisterNUICallback('ClearMentions', function()
    ...
    TriggerServerEvent('vr-phone:server:SetPhoneAlerts', "twitter", 0)
    SendNUIMessage({ action = "RefreshAppAlerts", AppData = Config.PhoneApplications })
end)
...
RegisterNUICallback('ClearGeneralAlerts', function(data)
    SetTimeout(400, function()
        ...
        TriggerServerEvent('vr-phone:server:SetPhoneAlerts', data.app, 0)
        SendNUIMessage({ action = "RefreshAppAlerts", AppData = Config.PhoneApplications })
    end)
end)
...
RegisterNUICallback('TransferMoney', function(data, cb)
    ...
    if tonumber(PhoneData.PlayerData.money.bank) >= data.amount then
        ...
        TriggerServerEvent('vr-phone:server:TransferMoney', data.iban, data.amount)
        local cbdata = {
            CanTransfer = true,
...
RegisterNUICallback('CanTransferMoney', function(data, cb)
    ...
    local PlayerData = VRCore.Functions.GetPlayerData()

    if (PlayerData.money.bank - amount) >= 0 then
        VRCore.Functions.TriggerCallback('vr-phone:server:CanTransferMoney', function(Transferd)
            if Transferd then
...
RegisterNUICallback('CallContact', function(data, cb)
    ...
    VRCore.Functions.TriggerCallback('vr-phone:server:GetCallState', function(CanCall, IsOnline, contactData)
        local status = { 
            CanCall = CanCall, 
            IsOnline = IsOnline,
...
CallContact = function(CallData, AnonymousCall)
    ...
    TriggerServerEvent('vr-phone:server:CallContact', PhoneData.CallData.TargetData, PhoneData.CallData.CallId, AnonymousCall)
    TriggerServerEvent('vr-phone:server:SetCallState', true)
...
CancelCall = function()
    TriggerServerEvent('vr-phone:server:CancelCall', PhoneData.CallData)
    ...
    TriggerServerEvent('vr-phone:server:SetCallState', false)
...
RegisterNetEvent('vr-phone:client:CancelCall')
AddEventHandler('vr-phone:client:CancelCall', function()
    if PhoneData.CallData.CallType == "ongoing" then
        SendNUIMessage({
...
RegisterNetEvent('vr-phone:client:GetCalled')
AddEventHandler('vr-phone:client:GetCalled', function(CallerNumber, CallId, AnonymousCall)
    local RepeatCount = 0
    local CallData = {
        number = CallerNumber,
        ...
        if not PhoneData.CallData.AnsweredCall then
            if RepeatCount + 1 ~= Config.CallRepeats + 1 then
                if PhoneData.CallData.InCall then
                    ...
                    TriggerServerEvent('vr-phone:server:AddRecentCall', "missed", CallData)
                    break
                end
                Citizen.Wait(Config.RepeatTimeout)
            else
...
function AnswerCall()
    if (PhoneData.CallData.CallType == "incoming" or PhoneData.CallData.CallType == "outgoing") and PhoneData.CallData.InCall and not PhoneData.CallData.AnsweredCall then
        ...
        TriggerServerEvent('vr-phone:server:AnswerCall', PhoneData.CallData)
...
RegisterNetEvent('vr-phone:client:AnswerCall')
AddEventHandler('vr-phone:client:AnswerCall', function()
    if (PhoneData.CallData.CallType == "incoming" or PhoneData.CallData.CallType == "outgoing") and PhoneData.CallData.InCall and not PhoneData.CallData.AnsweredCall then
        PhoneData.CallData.CallType = "ongoing"
        PhoneData.CallData.AnsweredCall = true
        PhoneData.CallData.CallTime = 0
...
RegisterNUICallback('FetchSearchResults', function(data, cb)
    VRCore.Functions.TriggerCallback('vr-phone:server:FetchResult', function(result)
        cb(result)
    end, data.input)
end)

RegisterNUICallback('FetchVehicleResults', function(data, cb)
    VRCore.Functions.TriggerCallback('vr-phone:server:GetVehicleSearchResults', function(result)
        if result ~= nil then 
            for k, v in pairs(result) do
                VRCore.Functions.TriggerCallback('police:IsPlateFlagged', function(flagged)
...
RegisterNUICallback('FetchVehicleScan', function(data, cb)
    ...
    VRCore.Functions.TriggerCallback('vr-phone:server:ScanPlate', function(result)
        VRCore.Functions.TriggerCallback('police:IsPlateFlagged', function(flagged)
            result.isFlagged = flagged
	    if VRCore.Shared.Vehicles[vehname] ~= nil then
                result.label = VRCore.Shared.Vehicles[vehname]['name']
            else
...
RegisterNetEvent('vr-phone:client:addPoliceAlert')
AddEventHandler('vr-phone:client:addPoliceAlert', function(alertData)
    PlayerJob = VRCore.Functions.GetPlayerData().job
...
RegisterNUICallback('SetAlertWaypoint', function(data)
    ...
    VRCore.Functions.Notify('GPS Location set: '..data.alert.title)
...
RegisterNetEvent('vr-phone:client:GiveContactDetails')
AddEventHandler('vr-phone:client:GiveContactDetails', function()
    local ped = PlayerPedId()

    local player, distance = GetClosestPlayer()
    if player ~= -1 and distance < 2.5 then
        local PlayerId = GetPlayerServerId(player)
        TriggerServerEvent('vr-phone:server:GiveContactDetails', PlayerId)
    else
        VRCore.Functions.Notify("No one nearby!", "error")
    end
end)
...
RegisterNUICallback('DeleteContact', function(data, cb)
    ...
    TriggerServerEvent('vr-phone:server:RemoveContact', Name, Number)
end)

RegisterNetEvent('vr-phone:client:AddNewSuggestion')
AddEventHandler('vr-phone:client:AddNewSuggestion', function(SuggestionData)
    ...
    Config.PhoneApplications["phone"].Alerts = Config.PhoneApplications["phone"].Alerts + 1
    TriggerServerEvent('vr-phone:server:SetPhoneAlerts', "phone", Config.PhoneApplications["phone"].Alerts)
end)

RegisterNetEvent('vr-phone:client:RemoveBankMoney')
AddEventHandler('vr-phone:client:RemoveBankMoney', function(amount)
    --if PhoneData.isOpen then
        SendNUIMessage({
            action = "PhoneNotification",
            PhoneNotify = {
...
RegisterNetEvent('vr-phone:client:AddTransaction')
AddEventHandler('vr-phone:client:AddTransaction', function(SenderData, TransactionData, Message, Title)
    local Data = {
    ...
    TriggerServerEvent('vr-phone:server:AddTransaction', Data)
end)
...
RegisterNUICallback('GetAvailableRaces', function(data, cb)
    VRCore.Functions.TriggerCallback('vr-lapraces:server:GetRaces', function(Races)
        cb(Races)
    end)
end)

RegisterNUICallback('JoinRace', function(data)
    TriggerServerEvent('vr-lapraces:server:JoinRace', data.RaceData)
end)

RegisterNUICallback('LeaveRace', function(data)
    TriggerServerEvent('vr-lapraces:server:LeaveRace', data.RaceData)
end)

RegisterNUICallback('StartRace', function(data)
    TriggerServerEvent('vr-lapraces:server:StartRace', data.RaceData.RaceId)
end)

RegisterNetEvent('vr-phone:client:UpdateLapraces')
AddEventHandler('vr-phone:client:UpdateLapraces', function()
    SendNUIMessage({
...
RegisterNUICallback('GetRaces', function(data, cb)
    VRCore.Functions.TriggerCallback('vr-lapraces:server:GetListedRaces', function(Races)
        cb(Races)
    end)
end)

RegisterNUICallback('GetTrackData', function(data, cb)
    VRCore.Functions.TriggerCallback('vr-lapraces:server:GetTrackData', function(TrackData, CreatorData)
...
RegisterNUICallback('SetupRace', function(data, cb)
    TriggerServerEvent('vr-lapraces:server:SetupRace', data.RaceId, tonumber(data.AmountOfLaps))
end)

RegisterNUICallback('HasCreatedRace', function(data, cb)
    VRCore.Functions.TriggerCallback('vr-lapraces:server:HasCreatedRace', function(HasCreated)
        cb(HasCreated)
    end)
end)

RegisterNUICallback('IsInRace', function(data, cb)
    local InRace = exports['vr-lapraces']:IsInRace()
    cb(InRace)
end)

RegisterNUICallback('IsAuthorizedToCreateRaces', function(data, cb)
    VRCore.Functions.TriggerCallback('vr-lapraces:server:IsAuthorizedToCreateRaces', function(IsAuthorized, NameAvailable)
        local data = {
            IsAuthorized = IsAuthorized,
            IsBusy = exports['vr-lapraces']:IsInEditor(),
...
RegisterNUICallback('StartTrackEditor', function(data, cb)
    TriggerServerEvent('vr-lapraces:server:CreateLapRace', data.TrackName)
end)

RegisterNUICallback('GetRacingLeaderboards', function(data, cb)
    VRCore.Functions.TriggerCallback('vr-lapraces:server:GetRacingLeaderboards', function(Races)
        cb(Races)
    end)
end)

RegisterNUICallback('RaceDistanceCheck', function(data, cb)
    VRCore.Functions.TriggerCallback('vr-lapraces:server:GetRacingData', function(RaceData)
        local ped = PlayerPedId()
        ...
        if dist <= 115.0 then
            if data.Joined then
                TriggerEvent('vr-lapraces:client:WaitingDistanceCheck')
            end
            cb(true)
        else
            VRCore.Functions.Notify('You\'re too far away from the race. GPS has been set to the race.', 'error', 5000)
...
RegisterNUICallback('IsBusyCheck', function(data, cb)
    if data.check == "editor" then
        cb(exports['vr-lapraces']:IsInEditor())
    else
        cb(exports['vr-lapraces']:IsInRace())
    end
end)

RegisterNUICallback('CanRaceSetup', function(data, cb)
    VRCore.Functions.TriggerCallback('vr-lapraces:server:CanRaceSetup', function(CanSetup)
        cb(CanSetup)
    end)
end)

RegisterNUICallback('GetPlayerHouses', function(data, cb)
    VRCore.Functions.TriggerCallback('vr-phone:server:GetPlayerHouses', function(Houses)
        cb(Houses)
    end)
end)

RegisterNUICallback('GetPlayerKeys', function(data, cb)
    VRCore.Functions.TriggerCallback('vr-phone:server:GetHouseKeys', function(Keys)
        cb(Keys)
    end)
end)
...
RegisterNUICallback('SetHouseLocation', function(data, cb)
    ...
    VRCore.Functions.Notify("GPS has been set to " .. data.HouseData.HouseData.adress .. "!", "success")
end)

RegisterNUICallback('RemoveKeyholder', function(data)
    TriggerServerEvent('vr-houses:server:removeHouseKey', data.HouseData.name, {
...
RegisterNUICallback('TransferCid', function(data, cb)
    ...
    VRCore.Functions.TriggerCallback('vr-phone:server:TransferCid', function(CanTransfer)
        cb(CanTransfer)
    end, TransferedCid, data.HouseData)
end)

RegisterNUICallback('FetchPlayerHouses', function(data, cb)
    VRCore.Functions.TriggerCallback('vr-phone:server:MeosGetPlayerHouses', function(result)
        cb(result)
    end, data.input)
end)
...
RegisterNUICallback('SetGPSLocation', function(data, cb)
    ...
    VRCore.Functions.Notify('GPS has been set!', 'success')
end)
...
RegisterNUICallback('GetCurrentLawyers', function(data, cb)
    VRCore.Functions.TriggerCallback('vr-phone:server:GetCurrentLawyers', function(lawyers)
        cb(lawyers)
    end)
end)
...
RegisterNUICallback('InstallApplication', function(data, cb)
    ...
    if NewSlot <= Config.MaxSlots then
        TriggerServerEvent('vr-phone:server:InstallApplication', {
            app = data.app,
        })
        cb({
            app = data.app,
...
RegisterNUICallback('RemoveApplication', function(data, cb)
    TriggerServerEvent('vr-phone:server:RemoveInstallation', data.app)
end)

RegisterNetEvent('vr-phone:RefreshPhone')
AddEventHandler('vr-phone:RefreshPhone', function()
    LoadPhone()
    SetTimeout(250, function()
        SendNUIMessage({
...
RegisterNUICallback('GetTruckerData', function(data, cb)
    local TruckerMeta = VRCore.Functions.GetPlayerData().metadata["jobrep"]["trucker"]
    local TierData = exports['vr-trucker']:GetTier(TruckerMeta)
    cb(TierData)
end)
