                    ...
                    if #(pos - vector3(v.x, v.y, v.z)) < 5 then
                        if #(pos - vector3(v.x, v.y, v.z)) < 1.5 then
                            ...
                            if IsControlJustReleased(0, 38) then
                                ...
                                TriggerEvent('vr-policealerts:ToggleDuty', onDuty)
                            end
                            ...
                            if #(pos - vector3(v.x, v.y, v.z)) < 1.5 then
                                ...
                                if IsControlJustReleased(0, 38) then
                                    if IsPedInAnyVehicle(PlayerPedId(), false) then
                                        ...
                                        VRCore.Functions.SpawnVehicle(Config.Helicopter, function(veh)
                    ...
                    if #(pos - vector3(v.x, v.y, v.z)) < 4.5 then
                        if onDuty then
                            if #(pos - vector3(v.x, v.y, v.z)) < 1.5 then
                                ...
                                if IsControlJustReleased(0, 38) then
                                    TriggerServerEvent("inventory:server:OpenInventory", "stash", "policestash_"..VRCore.Functions.GetPlayerData().citizenid)
                                    TriggerEvent("inventory:client:SetCurrentStash", "policestash_"..VRCore.Functions.GetPlayerData().citizenid)
                                end
                            elseif #(pos - vector3(v.x, v.y, v.z)) < 2.5 then
    ...
    VRCore.Functions.GetPlayerData(function(PlayerData)
        if PlayerData.job.name == "police" then
            ...
            if player ~= -1 and distance < 5.0 then
                local playerId = GetPlayerServerId(player)
                VRCore.Functions.TriggerCallback('police:GetPlayerStatus', function(result)
                    if result ~= nil then
...
function ImpoundVehicleList()
    VRCore.Functions.TriggerCallback("police:GetImpoundedVehicles", function(result)
        ped = PlayerPedId();
        MenuTitle = "Impounded Vehicles:"
        ClearMenu()

        if result == nil then
            VRCore.Functions.Notify("There are no impounded vehicles", "error", 5000)
            closeMenuFull()
        else
            for k, v in pairs(result) do
                ...
                Menu.addButton(VRCore.Shared.Vehicles[v.vehicle]["name"], "TakeOutImpound", v, "Impounded", " Engine: " .. enginePercent .. "%", " Body: " .. bodyPercent.. "%", " Fuel: "..currentFuel.. "%")
            end
        end
...
function OutfitsLijst()
    VRCore.Functions.TriggerCallback('apartments:GetOutfits', function(outfits)
        ...
        if outfits == nil then
            VRCore.Functions.Notify("You have no outfits saved...", "error", 3500)
            closeMenuFull()
        else
...
function removeOutfit(oData)
    ...
    VRCore.Functions.Notify(oData.outfitname.." is deleted", "success", 2500)
    closeMenuFull()
end
...
function VehicleList(isDown)
    ...
    MenuTitle = "Vehicles:"
    ClearMenu()
    local authorizedVehicles = Config.AuthorizedVehicles[VRCore.Functions.GetPlayerData().job.grade.level]
...
function TakeOutVehicle(vehicleInfo)
    ...
    VRCore.Functions.SpawnVehicle(vehicleInfo, function(veh)
	SetCarItemsInfo()
...
function IsArmoryWhitelist()
    ...
    if VRCore.Functions.GetPlayerData().job.name == 'police' then
        retval = true
    end

--[[     local citizenid = VRCore.Functions.GetPlayerData().citizenid
