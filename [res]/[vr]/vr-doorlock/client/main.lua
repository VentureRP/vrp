...
RegisterNetEvent('vr-doorlock:client:setState')
AddEventHandler('vr-doorlock:client:setState', function(doorID, state)
	VR.Doors[doorID].locked = state
end)

RegisterNetEvent('vr-doorlock:client:setDoors')
AddEventHandler('vr-doorlock:client:setDoors', function(doorList)
	VR.Doors = doorList
end)

RegisterNetEvent('VRCore:Client:OnPlayerLoaded')
AddEventHandler('VRCore:Client:OnPlayerLoaded', function()
    TriggerServerEvent("vr-doorlock:server:setupDoors")
end)
...
AddEventHandler('lockpicks:UseLockpick', function()
	...
	VRCore.Functions.TriggerCallback('VRCore:HasItem', function(hasItem)
		for k, v in pairs(VR.Doors) do
			local dist = #(pos - vector3(VR.Doors[k].textCoords.x, VR.Doors[k].textCoords.y, VR.Doors[k].textCoords.z))
			if dist < 1.5 then
				if VR.Doors[k].pickable then
					if VR.Doors[k].locked then
						if hasItem then
							closestDoorKey, closestDoorValue = k, v
							TriggerEvent('vr-lockpick:client:openLockpick', lockpickFinish)
						else
							VRCore.Functions.Notify("You Dont Have A Screwdriver Set", "error")
						end
					else
						VRCore.Functions.Notify('The Door Is Not Locked', 'error', 2500)
					end
				else
					VRCore.Functions.Notify('This Door Can Not Be Lockpicked', 'error', 2500)
				end
			end
		end
    end, "screwdriverset")
...
function lockpickFinish(success)
    if success then
		VRCore.Functions.Notify('Success!', 'success', 2500)
		setDoorLocking(closestDoorValue, closestDoorKey)
    else
        VRCore.Functions.Notify('Failed', 'error', 2500)
    end
end
...
function setDoorLocking(doorId, key)
    ...
    SetTimeout(400, function()
		...
		TriggerServerEvent('vr-doorlock:server:updateState', key, doorId.locked)
	end)
end
			...
			if distance < maxDistance then
				...
				if IsControlJustReleased(0, 38) then
					if isAuthorized then
						...
						VRCore.Functions.Notify('Not Authorized', 'error')
					end
				end
			end
		end
