							...
							DrawText3Ds(-1928.81, 2059.53, 140.84, "[E] Start Picking Grapes") 
								...
								if IsControlJustReleased(0,38) then
									if PlayerJob.name == "vineyard" then
										...
										VRCore.Functions.Notify("I dont think I work here...", "error")
									end
								end
						end
					end
	...
	while true do
		if startVineyard then
			if tasking then
				...
				TriggerEvent("vr-vineyard:client:startVineyard")
...
RegisterNetEvent('vr-vineyard:client:startVineyard')
AddEventHandler('vr-vineyard:client:startVineyard', function()
...
function pickgrapes()
	...
	if success then
		TriggerServerEvent("vr-vineyard:server:getGrapes")
		tasking = false
	end
end
...
function pickProcess()
    VRCore.Functions.Progressbar("pick_grape", "Picking Grapes ..", math.random(6000,8000), false, true, {
							...
							if not loadIngredients then
								if #(pos - vector3(Config.Vineyard["wine"].coords.x, Config.Vineyard["wine"].coords.y, Config.Vineyard["wine"].coords.z)) < 1 then
									...
									if IsControlJustPressed(0, 38) then
										if PlayerJob.name == "vineyard" then
											TriggerServerEvent("vr-vineyard:server:loadIngredients")
										else
											VRCore.Functions.Notify("I dont think I work here...", "error")
										end
									end
								...
								if not finishedWine then
									...
									if #(pos - vector3(Config.Vineyard["wine"].coords.x, Config.Vineyard["wine"].coords.y, Config.Vineyard["wine"].coords.z)) < 1 then
										...
										if IsControlJustPressed(0, 38) then
											if PlayerJob.name == "vineyard" then
												TriggerServerEvent("vr-vineyard:server:receiveWine")
												finishedWine = false
			...
			if nearlocation <= 15 then
				...
				if nearlocation <= 3 then	
					...
					if #(pos - vector3(Config.Vineyard["grapejuice"].coords.x, Config.Vineyard["grapejuice"].coords.y, Config.Vineyard["grapejuice"].coords.z)) < 1 then
						...
						if IsControlJustPressed(0, 38) then
							if PlayerJob.name == "vineyard" then
								TriggerServerEvent("vr-vineyard:server:grapeJuice")
							else
								VRCore.Functions.Notify("I dont think I work here...", "error")
							end
						end
...
RegisterNetEvent('vr-vineyard:client:grapeJuice')
AddEventHandler('vr-vineyard:client:grapeJuice', function()
	PrepareAnim()
	grapeJuiceProcess()
end)

RegisterNetEvent('vr-vineyard:client:loadIngredients')
AddEventHandler('vr-vineyard:client:loadIngredients', function()
...
function grapeJuiceProcess()
    VRCore.Functions.Progressbar("grape_juice", "Processing Grapes ..", math.random(15000,20000), false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
    }, {}, {}, {}, function() -- Done
        TriggerServerEvent("vr-vineyard:server:receiveGrapeJuice")
        ClearPedTasks(PlayerPedId())
