...
VRCore.Player.Login = function(source, citizenid, newData)
	if source ~= nil then
		if citizenid then
			...
			VRCore.Player.CheckPlayerData(source, PlayerData)
		else
			VRCore.Player.CheckPlayerData(source, newData)
		end
		return true
	else
		VRCore.ShowError(GetCurrentResourceName(), "ERROR VRCORE.PLAYER.LOGIN - NO SOURCE GIVEN!")
		return false
	end
end

VRCore.Player.CheckPlayerData = function(source, PlayerData)
	PlayerData = PlayerData ~= nil and PlayerData or {}

	PlayerData.source = source
	PlayerData.citizenid = PlayerData.citizenid ~= nil and PlayerData.citizenid or VRCore.Player.CreateCitizenId()
	PlayerData.license = PlayerData.license ~= nil and PlayerData.license or VRCore.Functions.GetIdentifier(source, 'license')
	...
	for moneytype, startamount in pairs(VRCore.Config.Money.MoneyTypes) do
	...
	PlayerData.charinfo.account = PlayerData.charinfo.account ~= nil and PlayerData.charinfo.account or "US0"..math.random(1,9).."VRCore"..math.random(1111,9999)..math.random(1111,9999)..math.random(11,99)
	...
	PlayerData.metadata["bloodtype"] = PlayerData.metadata["bloodtype"] ~= nil and PlayerData.metadata["bloodtype"] or VRCore.Config.Player.Bloodtypes[math.random(1, #VRCore.Config.Player.Bloodtypes)]
	...
	PlayerData.metadata["fingerprint"] = PlayerData.metadata["fingerprint"] ~= nil and PlayerData.metadata["fingerprint"] or VRCore.Player.CreateFingerId()
	PlayerData.metadata["walletid"] = PlayerData.metadata["walletid"] ~= nil and PlayerData.metadata["walletid"] or VRCore.Player.CreateWalletId()
	...
	PlayerData.metadata["phonedata"] = PlayerData.metadata["phonedata"] ~= nil and PlayerData.metadata["phonedata"] or {
        SerialNumber = VRCore.Player.CreateSerialNumber(),
        InstalledApps = {},
    }
	...
	PlayerData.position = PlayerData.position ~= nil and PlayerData.position or VRConfig.DefaultSpawn
	PlayerData.LoggedIn = true

	PlayerData = VRCore.Player.LoadInventory(PlayerData)
	VRCore.Player.CreatePlayer(PlayerData)
end

VRCore.Player.CreatePlayer = function(PlayerData)
	local self = {}
	self.Functions = {}
	self.PlayerData = PlayerData

	self.Functions.UpdatePlayerData = function(dontUpdateChat)
		TriggerClientEvent("VRCore:Player:SetPlayerData", self.PlayerData.source, self.PlayerData)
		if dontUpdateChat == nil then
			VRCore.Commands.Refresh(self.PlayerData.source)
		end
	end
	...
	self.Functions.SetJob = function(job, grade)
		...
		if VRCore.Shared.Jobs[job] ~= nil then
			self.PlayerData.job.name = job
			self.PlayerData.job.label = VRCore.Shared.Jobs[job].label
			self.PlayerData.job.onduty = VRCore.Shared.Jobs[job].defaultDuty
			
			if VRCore.Shared.Jobs[job].grades[grade] then
				local jobgrade = VRCore.Shared.Jobs[job].grades[grade]
			...
			TriggerClientEvent("VRCore:Client:OnJobUpdate", self.PlayerData.source, self.PlayerData.job)
			return true
		end
	...
	self.Functions.SetGang = function(gang, grade)
		...
		if VRCore.Shared.Gangs[gang] ~= nil then
			self.PlayerData.gang.name = gang
			self.PlayerData.gang.label = VRCore.Shared.Gangs[gang].label
			if VRCore.Shared.Gangs[gang].grades[grade] then
				local ganggrade = VRCore.Shared.Gangs[gang].grades[grade]
			...
			self.Functions.UpdatePlayerData()
			TriggerClientEvent("VRCore:Client:OnGangUpdate", self.PlayerData.source, self.PlayerData.gang)
			return true
		end
		return false
	end
	...
	self.Functions.AddMoney = function(moneytype, amount, reason)
		...
		if self.PlayerData.money[moneytype] ~= nil then
			...
			if amount > 100000 then
				TriggerEvent("vr-log:server:CreateLog", "playermoney", "AddMoney", "lightgreen", "**"..GetPlayerName(self.PlayerData.source) .. " (citizenid: "..self.PlayerData.citizenid.." | id: "..self.PlayerData.source..")** $"..amount .. " ("..moneytype..") added, new "..moneytype.." balance: "..self.PlayerData.money[moneytype], true)
			else
				TriggerEvent("vr-log:server:CreateLog", "playermoney", "AddMoney", "lightgreen", "**"..GetPlayerName(self.PlayerData.source) .. " (citizenid: "..self.PlayerData.citizenid.." | id: "..self.PlayerData.source..")** $"..amount .. " ("..moneytype..") added, new "..moneytype.." balance: "..self.PlayerData.money[moneytype])
			end
	...
	self.Functions.RemoveMoney = function(moneytype, amount, reason)
		...
		if amount < 0 then return end
		if self.PlayerData.money[moneytype] ~= nil then
			for _, mtype in pairs(VRCore.Config.Money.DontAllowMinus) do
			...
			self.Functions.UpdatePlayerData()
			if amount > 100000 then
				TriggerEvent("vr-log:server:CreateLog", "playermoney", "RemoveMoney", "red", "**"..GetPlayerName(self.PlayerData.source) .. " (citizenid: "..self.PlayerData.citizenid.." | id: "..self.PlayerData.source..")** $"..amount .. " ("..moneytype..") removed, new "..moneytype.." balance: "..self.PlayerData.money[moneytype], true)
			else
				TriggerEvent("vr-log:server:CreateLog", "playermoney", "RemoveMoney", "red", "**"..GetPlayerName(self.PlayerData.source) .. " (citizenid: "..self.PlayerData.citizenid.." | id: "..self.PlayerData.source..")** $"..amount .. " ("..moneytype..") removed, new "..moneytype.." balance: "..self.PlayerData.money[moneytype])
			end
			TriggerClientEvent("hud:client:OnMoneyChange", self.PlayerData.source, moneytype, amount, true)
			if moneytype == "bank" then
				TriggerClientEvent('vr-phone:client:RemoveBankMoney', self.PlayerData.source, amount)
			end
			return true
		end
		return false
	...
	self.Functions.SetMoney = function(moneytype, amount, reason)
		...
		if self.PlayerData.money[moneytype] ~= nil then
			...
			self.Functions.UpdatePlayerData()
			TriggerEvent("vr-log:server:CreateLog", "playermoney", "SetMoney", "green", "**"..GetPlayerName(self.PlayerData.source) .. " (citizenid: "..self.PlayerData.citizenid.." | id: "..self.PlayerData.source..")** $"..amount .. " ("..moneytype..") set, new "..moneytype.." balance: "..self.PlayerData.money[moneytype])
			return true
		end
		return false
	end
	...
	self.Functions.AddItem = function(item, amount, slot, info)
		local totalWeight = VRCore.Player.GetTotalWeight(self.PlayerData.items)
		local itemInfo = VRCore.Shared.Items[item:lower()]
		if itemInfo == nil then TriggerClientEvent('VRCore:Notify', source, "Item Does Not Exist", "error") return end
		local amount = tonumber(amount)
		local slot = tonumber(slot) ~= nil and tonumber(slot) or VRCore.Player.GetFirstSlotByItem(self.PlayerData.items, item)
		if itemInfo["type"] == "weapon" and info == nil then
			info = {
				serie = tostring(VRCore.Shared.RandomInt(2) .. VRCore.Shared.RandomStr(3) .. VRCore.Shared.RandomInt(1) .. VRCore.Shared.RandomStr(2) .. VRCore.Shared.RandomInt(3) .. VRCore.Shared.RandomStr(4)),
			}
		end
		if (totalWeight + (itemInfo["weight"] * amount)) <= VRCore.Config.Player.MaxWeight then
			if (slot ~= nil and self.PlayerData.items[slot] ~= nil) and (self.PlayerData.items[slot].name:lower() == item:lower()) and (itemInfo["type"] == "item" and not itemInfo["unique"]) then
				...
				self.Functions.UpdatePlayerData()
				TriggerEvent("vr-log:server:CreateLog", "playerinventory", "AddItem", "green", "**"..GetPlayerName(self.PlayerData.source) .. " (citizenid: "..self.PlayerData.citizenid.." | id: "..self.PlayerData.source..")** got item: [slot:" ..slot.."], itemname: " .. self.PlayerData.items[slot].name .. ", added amount: " .. amount ..", new total amount: ".. self.PlayerData.items[slot].amount)
				return true
			...
			elseif (itemInfo["unique"]) or (not slot or slot == nil) or (itemInfo["type"] == "weapon") then
				for i = 1, VRConfig.Player.MaxInvSlots, 1 do
					if self.PlayerData.items[i] == nil then
						...
						self.Functions.UpdatePlayerData()
						TriggerEvent("vr-log:server:CreateLog", "playerinventory", "AddItem", "green", "**"..GetPlayerName(self.PlayerData.source) .. " (citizenid: "..self.PlayerData.citizenid.." | id: "..self.PlayerData.source..")** got item: [slot:" ..i.."], itemname: " .. self.PlayerData.items[i].name .. ", added amount: " .. amount ..", new total amount: ".. self.PlayerData.items[i].amount)
						--TriggerClientEvent('VRCore:Notify', self.PlayerData.source, itemInfo["label"].. " toegevoegd!", "success")
						return true
					end
				end
			end
	...
	self.Functions.RemoveItem = function(item, amount, slot)
		...
		if slot ~= nil then
			if self.PlayerData.items[slot].amount > amount then
				self.PlayerData.items[slot].amount = self.PlayerData.items[slot].amount - amount
				self.Functions.UpdatePlayerData()
				TriggerEvent("vr-log:server:CreateLog", "playerinventory", "RemoveItem", "red", "**"..GetPlayerName(self.PlayerData.source) .. " (citizenid: "..self.PlayerData.citizenid.." | id: "..self.PlayerData.source..")** lost item: [slot:" ..slot.."], itemname: " .. self.PlayerData.items[slot].name .. ", removed amount: " .. amount ..", new total amount: ".. self.PlayerData.items[slot].amount)
				return true
			else
				self.PlayerData.items[slot] = nil
				self.Functions.UpdatePlayerData()
				TriggerEvent("vr-log:server:CreateLog", "playerinventory", "RemoveItem", "red", "**"..GetPlayerName(self.PlayerData.source) .. " (citizenid: "..self.PlayerData.citizenid.." | id: "..self.PlayerData.source..")** lost item: [slot:" ..slot.."], itemname: " .. item .. ", removed amount: " .. amount ..", item removed")
				return true
			end
		else
			local slots = VRCore.Player.GetSlotsByItem(self.PlayerData.items, item)
	...
	self.Functions.SetInventory = function(items, dontUpdateChat)
		...
		TriggerEvent("vr-log:server:CreateLog", "playerinventory", "SetInventory", "blue", "**"..GetPlayerName(self.PlayerData.source) .. " (citizenid: "..self.PlayerData.citizenid.." | id: "..self.PlayerData.source..")** items set: " .. json.encode(items))
	end

	self.Functions.ClearInventory = function()
		self.PlayerData.items = {}
		self.Functions.UpdatePlayerData()
		TriggerEvent("vr-log:server:CreateLog", "playerinventory", "ClearInventory", "red", "**"..GetPlayerName(self.PlayerData.source) .. " (citizenid: "..self.PlayerData.citizenid.." | id: "..self.PlayerData.source..")** inventory cleared")
	end

	self.Functions.GetItemByName = function(item)
		local item = tostring(item):lower()
		local slot = VRCore.Player.GetFirstSlotByItem(self.PlayerData.items, item)
		if slot ~= nil then
	...
	self.Functions.Save = function()
		VRCore.Player.Save(self.PlayerData.source)
	end

	VRCore.Players[self.PlayerData.source] = self
	VRCore.Player.Save(self.PlayerData.source)

	-- At this point we are safe to emit new instance to third party resource for load handling
	TriggerEvent('VRCore:Server:PlayerLoaded', self)
	self.Functions.UpdatePlayerData()
end

VRCore.Player.Save = function(source)
	local PlayerData = VRCore.Players[source].PlayerData
	if PlayerData ~= nil then
		...
		VRCore.Player.SaveInventory(source)
		VRCore.ShowSuccess(GetCurrentResourceName(), PlayerData.name .." PLAYER SAVED!")
	else
		VRCore.ShowError(GetCurrentResourceName(), "ERROR VRCORE.PLAYER.SAVE - PLAYERDATA IS EMPTY!")
	end
end

VRCore.Player.Logout = function(source)
	TriggerClientEvent('VRCore:Client:OnPlayerUnload', source)
	TriggerClientEvent("VRCore:Player:UpdatePlayerData", source)
	Citizen.Wait(200)
	VRCore.Players[source] = nil
end
...
VRCore.Player.DeleteCharacter = function(source, citizenid)
	...
	if license == result then
		...
		TriggerEvent("vr-log:server:CreateLog", "joinleave", "Character Deleted", "red", "**".. GetPlayerName(source) .. "** ("..VRCore.Functions.GetIdentifier(source, 'license')..") deleted **"..citizenid.."**..")
	else
		DropPlayer(source, 'You Have Been Kicked For Exploitation')
		TriggerEvent("vr-log:server:CreateLog", "anticheat", "Anti-Cheat", "white", GetPlayerName(source).." Has Been Dropped For Character Deletion Exploit", false)
	end
end

VRCore.Player.LoadInventory = function(PlayerData)
	PlayerData.items = {}
	local result = exports['ghmattimysql']:executeSync('SELECT * FROM players WHERE citizenid=@citizenid', {['@citizenid'] = PlayerData.citizenid})
	if result[1] ~= nil then 
...
VRCore.Player.SaveInventory = function(source)
	if VRCore.Players[source] ~= nil then 
		local PlayerData = VRCore.Players[source].PlayerData
...
VRCore.Player.GetTotalWeight = function(items)
	local weight = 0
	if items ~= nil then
		for slot, item in pairs(items) do
...
VRCore.Player.GetSlotsByItem = function(items, itemName)
	local slotsFound = {}
	if items ~= nil then
		for slot, item in pairs(items) do
...
VRCore.Player.GetFirstSlotByItem = function(items, itemName)
	if items ~= nil then
		for slot, item in pairs(items) do
			if item.name:lower() == itemName:lower() then
...
VRCore.Player.CreateCitizenId = function()
	local UniqueFound = false
	local CitizenId = nil

	while not UniqueFound do
		CitizenId = tostring(VRCore.Shared.RandomStr(3) .. VRCore.Shared.RandomInt(5)):upper()
...
VRCore.Player.CreateFingerId = function()
	local UniqueFound = false
	local FingerId = nil
	while not UniqueFound do
		FingerId = tostring(VRCore.Shared.RandomStr(2) .. VRCore.Shared.RandomInt(3) .. VRCore.Shared.RandomStr(1) .. VRCore.Shared.RandomInt(2) .. VRCore.Shared.RandomStr(3) .. VRCore.Shared.RandomInt(4))
...
VRCore.Player.CreateWalletId = function()
	local UniqueFound = false
	local WalletId = nil
	while not UniqueFound do
		WalletId = "VR-"..math.random(11111111, 99999999)
...
VRCore.Player.CreateSerialNumber = function()
    local UniqueFound = false
...
VRCore.EscapeSqli = function(str)
    local replacements = { ['"'] = '\\"', ["'"] = "\\'" }
