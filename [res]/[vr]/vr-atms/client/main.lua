...
RegisterNUICallback("doATMWithdraw", function(data, cb)
    if data ~= nil then
        TriggerServerEvent('vr-atms:server:doAccountWithdraw', data)
    end
end)

RegisterNetEvent('vr-atms:client:updateBankInformation')
AddEventHandler('vr-atms:client:updateBankInformation', function(banking)
    SendNUIMessage({
...
RegisterNUICallback("loadBankingAccount", function(data, cb)
    VRCore.Functions.TriggerCallback('vr-atms:server:loadBankAccount', function(banking)
...
RegisterNetEvent('vr-atms:client:loadATM')
AddEventHandler('vr-atms:client:loadATM', function(cards)
    if cards ~= nil and cards[1] ~= nil then
        ...
        for k, v in pairs(Config.ATMModels) do
            ...
            if atm then 
                ...
                VRCore.Functions.Progressbar("accessing_atm", "Accessing ATM", 1500, false, true, {
        ...
        VRCore.Functions.Notify("You do not have a debit card to pay with, please visit a branch to order a card. or ensure one is on your person.", "error")
    end
end)

RegisterNUICallback("removeCard", function(data, cb)
    VRCore.Functions.TriggerCallback('vr-debitcard:server:deleteCard', function(hasDeleted)
        if hasDeleted then
            SetNuiFocus(false, false)
            SendNUIMessage({
                status = "closeATM"
            })
            VRCore.Functions.Notify('Card has deleted.', 'success')
        else
            VRCore.Functions.Notify('Failed to delete card.', 'error')
        end
    end, data)
end)
