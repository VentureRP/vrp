    ...
    window.addEventListener('message', function (event) {
        ...
        if (data.action == "ui") {
            if (data.toggle) {
                ...
                setTimeout(function(){
                    $.post('https://vr-multicharacter/setupCharacters');
                    setTimeout(function(){
                        clearInterval(DotsInterval);
                        loadingProgress = 0;
                        originalText = "Retrieving data";
                        $(".welcomescreen").fadeOut(150);
                        VRMultiCharacters.fadeInDown('.character-info', '20%', 400);
                        VRMultiCharacters.fadeInDown('.characters-list', '20%', 400);
                        $.post('https://vr-multicharacter/removeBlur');
                    }, 2000);
                }, 2000);
            } else {
...
$('.continue-btn').click(function(e){
    ...
    // VRMultiCharacters.fadeOutUp('.welcomescreen', undefined, 400);
    // VRMultiCharacters.fadeOutDown('.server-log', undefined, 400);
    // setTimeout(function(){
    //     VRMultiCharacters.fadeInDown('.characters-list', '20%', 400);
    //     VRMultiCharacters.fadeInDown('.character-info', '20%', 400);
    //     $.post('https://vr-multicharacter/setupCharacters');
...
$('.disconnect-btn').click(function(e){
    ...
    $.post('https://vr-multicharacter/closeUI');
    $.post('https://vr-multicharacter/disconnectButton');
});
...
$(document).on('click', '.character', function(e) {
    ...
    if (selectedChar === null) {
        ...
        if ((selectedChar).data('cid') == "") {
            ...
            $.post('https://vr-multicharacter/cDataPed', JSON.stringify({
                cData: cDataPed
            }));
        } else {
            $(selectedChar).addClass("char-selected");
...
$(document).on('click', '#create', function (e) {
    ...
    console.log("FIELDS REQUIRED")
        ...
        $.post('https://vr-multicharacter/createNewCharacter', JSON.stringify({
            firstname: firstname,
            lastname: lastname,
        ...
        VRMultiCharacters.fadeOutDown('.character-register', '125%', 400);
        refreshCharacters()
    }
});
// $(document).on('click', '#create', function(e){
...
$(document).on('click', '#accept-delete', function(e){
    $.post('https://vr-multicharacter/removeCharacter', JSON.stringify({
...
$(document).on('click', '#play', function(e) {
    ...
    if (selectedChar !== null) {
        if (charData !== "") {
            $.post('https://vr-multicharacter/selectCharacter', JSON.stringify({
                cData: $(selectedChar).data('cData')
            }));
            // VRMultiCharacters.fadeInDown('.welcomescreen', WelcomePercentage, 400);
            // VRMultiCharacters.fadeInDown('.server-log', '25%', 400);
            setTimeout(function(){
                VRMultiCharacters.fadeOutDown('.characters-list', "-40%", 400);
                VRMultiCharacters.fadeOutDown('.character-info', "-40%", 400);
                VRMultiCharacters.resetAll();
            }, 1500);
        } else {
            $('.characters-list').css("filter", "blur(2px)")
            $('.character-info').css("filter", "blur(2px)")
            VRMultiCharacters.fadeInDown('.character-register', '25%', 400);
        }
    }
});
...
VRMultiCharacters.fadeOutUp = function(element, time) {
    $(element).css({"display":"block"}).animate({top: "-80.5%",}, time, function(){
        $(element).css({"display":"none"});
    });
}

VRMultiCharacters.fadeOutDown = function(element, percent, time) {
...
VRMultiCharacters.fadeInDown = function(element, percent, time) {
    $(element).css({"display":"block"}).animate({top: percent,}, time);
}

VRMultiCharacters.resetAll = function() {
