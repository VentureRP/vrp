        ...
        if distance < 6 then
            ...
            if distance < 2 then
                ...
                if IsControlJustPressed(0, 47) then
                    TriggerServerEvent("vr-coke:server:grindleaves")
    ...
    while true do
        ...
        if distance < 6 then
            ...
            if distance < 2 then
                ...
                if IsControlJustPressed(0, 47) then
                    TriggerServerEvent("vr-coke:server:processCrack")
                end
            end
...
RegisterNetEvent('vr-coke:client:grindleavesMinigame')
AddEventHandler('vr-coke:client:grindleavesMinigame', function(source)
    PrepareProcessAnim()
    ProcessMinigame(source)
end)

RegisterNetEvent('vr-coke:client:processCrack')
AddEventHandler('vr-coke:client:processCrack', function(source)
    ProcessCrackMinigame(source)
end)

function pickProcess()
    VRCore.Functions.Progressbar("grind_coke", "Picking Coca Leaves ..", math.random(80000,120000), false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
    }, {}, {}, {}, function() -- Done
        TriggerServerEvent("vr-coke:server:getleaf")
...
function cokeProcess()
    VRCore.Functions.Progressbar("grind_coke", "Process Coke Leaves ..", math.random(10000, 12000), false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
    }, {}, {}, {}, function() -- Done
        TriggerServerEvent("vr-coke:server:getcoke")
        ClearPedTasks(PlayerPedId())
        cokepicking = false
    end, function() -- Cancel
        openingDoor = false
...
function crackProcess()
    VRCore.Functions.Progressbar("grind_coke", "Process Crack ..", math.random(10000, 12000), false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
    }, {}, {}, {}, function() -- Done
        TriggerServerEvent("vr-coke:server:getcrack")
        ClearPedTasks(PlayerPedId())
        cokepicking = false
    end, function() -- Cancel
        openingDoor = false
...
function PickMinigame()
    ...
    Skillbar.Start({
        ...
        if SucceededAttempts + 1 >= NeededAttempts then
            pickProcess()
            VRCore.Functions.Notify("You picked a coca leaf", "success")
            FailedAttemps = 0
            SucceededAttempts = 0
            NeededAttempts = 0
        else    
            ...
            VRCore.Functions.Notify("You messed up the coca leaf!", "error")
            FailedAttemps = 0
            SucceededAttempts = 0
            NeededAttempts = 0
            cokepicking = false
...
function ProcessMinigame(source)
    ...
    Skillbar.Start({
        ...
        if SucceededAttempts + 1 >= NeededAttempts then
            cokeProcess()
            VRCore.Functions.Notify("You make some coke!", "success")
            FailedAttemps = 0
            SucceededAttempts = 0
            NeededAttempts = 0
        else    
            ...
            VRCore.Functions.Notify("You messed up the process!", "error")
            FailedAttemps = 0
            SucceededAttempts = 0
            NeededAttempts = 0
            cokeprocess = false
...
function ProcessCrackMinigame(source)
    ...
    Skillbar.Start({
        ...
        if SucceededAttempts + 1 >= NeededAttempts then
            crackProcess()
            VRCore.Functions.Notify("You make some crack!", "success")
            FailedAttemps = 0
            SucceededAttempts = 0
            NeededAttempts = 0
        else    
...
function knockDoorAnim(home)
    ...
    if home then
        ...
        TriggerServerEvent("vr-coke:server:cokesell")
        ...
        knockingDoor = false
        Citizen.Wait(1000)
        VRCore.Functions.Notify('It seems that no one is home..', 'error', 3500)
    end
end
