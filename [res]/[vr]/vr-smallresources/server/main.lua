...
VRCore.Functions.CreateCallback('nos:GetNosLoadedVehs', function(source, cb)
    cb(VehicleNitrous)
end)

VRCore.Commands.Add("id", "Check Your ID #", {}, false, function(source, args)
    TriggerClientEvent('VRCore:Notify', source,  "ID: "..source)
end)

VRCore.Functions.CreateUseableItem("harness", function(source, item)
...
AddEventHandler('equip:harness', function(item)
    ...
    if Player.PlayerData.items[item.slot].info.uses - 1 == 0 then
        TriggerClientEvent("inventory:client:ItemBox", source, VRCore.Shared.Items['harness'], "remove")
...
VRCore.Functions.CreateCallback('smallresources:server:GetCurrentPlayers', function(source, cb)
