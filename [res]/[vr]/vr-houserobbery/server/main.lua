VRCore.Functions.CreateCallback('vr-houserobbery:server:GetHouseConfig', function(source, cb)
    cb(Config.Houses)
end)

RegisterServerEvent('vr-houserobbery:server:enterHouse')
AddEventHandler('vr-houserobbery:server:enterHouse', function(house)
    local src = source
    ...
    if not Config.Houses[house]["opened"] then
        ...
        TriggerClientEvent('vr-houserobbery:client:setHouseState', -1, house, true)
    end
    TriggerClientEvent('vr-houserobbery:client:enterHouse', src, house)
...
function ResetHouseStateTimer(house)
    ...
    Citizen.SetTimeout(time, function()
        ...
        TriggerClientEvent('vr-houserobbery:client:ResetHouseState', -1, house)
    end)
end

RegisterServerEvent('vr-houserobbery:server:searchCabin')
AddEventHandler('vr-houserobbery:server:searchCabin', function(cabin, house)
    local src = source
    local Player = VRCore.Functions.GetPlayer(src)
    ...
    if itemFound < 4 then
        ...
        for i = 1, itemCount, 1 do
            ...
            local itemInfo = VRCore.Shared.Items[randomItem]
            if math.random(1, 100) == 69 then
                randomItem = "painkillers"
                itemInfo = VRCore.Shared.Items[randomItem]
        ...
        TriggerClientEvent('VRCore:Notify', src, 'The box is empty', 'error', 3500)
    end

    Config.Houses[house]["furniture"][cabin]["searched"] = true
    TriggerClientEvent('vr-houserobbery:client:setCabinState', -1, house, cabin, true)
end)

RegisterServerEvent('vr-houserobbery:server:SetBusyState')
AddEventHandler('vr-houserobbery:server:SetBusyState', function(cabin, house, bool)
    Config.Houses[house]["furniture"][cabin]["isBusy"] = bool
    TriggerClientEvent('vr-houserobbery:client:SetBusyState', -1, cabin, house, bool)
end)
