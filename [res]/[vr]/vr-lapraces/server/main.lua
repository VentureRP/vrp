...
VRCore.Functions.CreateCallback('vr-lapraces:server:GetRacingLeaderboards', function(source, cb)
    cb(Races)
end)
...
RegisterServerEvent('vr-lapraces:server:FinishPlayer')
AddEventHandler('vr-lapraces:server:FinishPlayer', function(RaceData, TotalTime, TotalLaps, BestLap)
    local src = source
    ...
    if Races[RaceData.RaceId].Records ~= nil and next(Races[RaceData.RaceId].Records) ~= nil then
        if BLap < Races[RaceData.RaceId].Records.Time then
            ...
            TriggerClientEvent('vr-phone:client:RaceNotify', src, 'You have won the WR from '..RaceData.RaceName..' disconnected with a time of: '..SecondsToClock(BLap)..'!')
        end
    else
        Races[RaceData.RaceId].Records = {
            Time = BLap,
        ...
        exports.ghmattimysql:execute('UPDATE lapraces SET records=@records WHERE raceid=@raceid', {['@records'] = json.encode(Races[RaceData.RaceId].Records), ['@raceid'] = RaceData.RaceId})
        TriggerClientEvent('vr-phone:client:RaceNotify', src, 'You have won the WR from '..RaceData.RaceName..' put down with a time of: '..SecondsToClock(BLap)..'!')
    end
    AvailableRaces[AvailableKey].RaceData = Races[RaceData.RaceId]
    TriggerClientEvent('vr-lapraces:client:PlayerFinishs', -1, RaceData.RaceId, PlayersFinished, Player)
    ...
    TriggerClientEvent('vr-phone:client:UpdateLapraces', -1)
end)
...
function IsWhitelisted(CitizenId)
    ...
    local Perms = VRCore.Functions.GetPermission(Player.PlayerData.source)
...
RegisterServerEvent('vr-lapraces:server:CreateLapRace')
AddEventHandler('vr-lapraces:server:CreateLapRace', function(RaceName)
    local src = source
    local Player = VRCore.Functions.GetPlayer(src)
    
    if IsWhitelisted(Player.PlayerData.citizenid) then
        if IsNameAvailable(RaceName) then
            TriggerClientEvent('vr-lapraces:client:StartRaceEditor', source, RaceName)
        else
            TriggerClientEvent('VRCore:Notify', source, 'There is already a race with this name.', 'error')
        end
    else
        TriggerClientEvent('VRCore:Notify', source, 'You have not been authorized to race\'s to create.', 'error')
    end
end)

VRCore.Functions.CreateCallback('vr-lapraces:server:GetRaces', function(source, cb)
    cb(AvailableRaces)
end)

VRCore.Functions.CreateCallback('vr-lapraces:server:GetListedRaces', function(source, cb)
    cb(Races)
end)

VRCore.Functions.CreateCallback('vr-lapraces:server:GetRacingData', function(source, cb, RaceId)
    cb(Races[RaceId])
end)

VRCore.Functions.CreateCallback('vr-lapraces:server:HasCreatedRace', function(source, cb)
    cb(HasOpenedRace(VRCore.Functions.GetPlayer(source).PlayerData.citizenid))
end)

VRCore.Functions.CreateCallback('vr-lapraces:server:IsAuthorizedToCreateRaces', function(source, cb, TrackName)
    cb(IsWhitelisted(VRCore.Functions.GetPlayer(source).PlayerData.citizenid), IsNameAvailable(TrackName))
end)
...
VRCore.Functions.CreateCallback('vr-lapraces:server:GetTrackData', function(source, cb, RaceId)
...
RegisterServerEvent('vr-lapraces:server:JoinRace')
AddEventHandler('vr-lapraces:server:JoinRace', function(RaceData)
    local src = source
    local Player = VRCore.Functions.GetPlayer(src)
    ...
    if CurrentRace ~= nil then
        ...
        if (AmountOfRacers - 1) == 0 then
            ...
            TriggerClientEvent('VRCore:Notify', src, 'You were the only one in the race, the race had ended', 'error')
            TriggerClientEvent('vr-lapraces:client:LeaveRace', src, Races[CurrentRace])
        else
    ...
    TriggerClientEvent('vr-lapraces:client:JoinRace', src, Races[RaceId], RaceData.Laps)
    TriggerClientEvent('vr-phone:client:UpdateLapraces', -1)
    local creatorsource = VRCore.Functions.GetPlayerByCitizenId(AvailableRaces[AvailableKey].SetupCitizenId).PlayerData.source
    if creatorsource ~= Player.PlayerData.source then
        TriggerClientEvent('vr-phone:client:RaceNotify', creatorsource, string.sub(Player.PlayerData.charinfo.firstname, 1, 1)..'. '..Player.PlayerData.charinfo.lastname..' the race has been joined!')
    end
end)

RegisterServerEvent('vr-lapraces:server:LeaveRace')
AddEventHandler('vr-lapraces:server:LeaveRace', function(RaceData)
    local src = source
    local Player = VRCore.Functions.GetPlayer(src)
    local RaceName
    ...
    local creatorsource = VRCore.Functions.GetPlayerByCitizenId(AvailableRaces[AvailableKey].SetupCitizenId).PlayerData.source
    if creatorsource ~= Player.PlayerData.source then
        TriggerClientEvent('vr-phone:client:RaceNotify', creatorsource, string.sub(Player.PlayerData.charinfo.firstname, 1, 1)..'. '..Player.PlayerData.charinfo.lastname..' the race has been delivered!')
    end
    local AmountOfRacers = 0
    for k, v in pairs(Races[RaceData.RaceId].Racers) do
        AmountOfRacers = AmountOfRacers + 1
    ...
    if (AmountOfRacers - 1) == 0 then
        ...
        table.remove(AvailableRaces, AvailableKey)
        TriggerClientEvent('VRCore:Notify', src, 'You were the only one in the race.The race had ended.', 'error')
        TriggerClientEvent('vr-lapraces:client:LeaveRace', src, Races[RaceId])
...
RegisterServerEvent('vr-lapraces:server:SetupRace')
AddEventHandler('vr-lapraces:server:SetupRace', function(RaceId, Laps)
            ...
            if not Races[RaceId].Started then
                ...
                SetTimeout(5 * 60 * 1000, function()
                    if Races[RaceId].Waiting then
                        ...
                        for cid,_ in pairs(Races[RaceId].Racers) do
                            local RacerData = VRCore.Functions.GetPlayerByCitizenId(cid)
                            if RacerData ~= nil then
                                TriggerClientEvent('vr-lapraces:client:LeaveRace', RacerData.PlayerData.source, Races[RaceId])
                            end
                        end
                        table.remove(AvailableRaces, AvailableKey)
                ...
                TriggerClientEvent('VRCore:Notify', source, 'The race is already running', 'error')
            end
        else
            TriggerClientEvent('VRCore:Notify', source, 'The race is already running', 'error')
        end
    else
        TriggerClientEvent('VRCore:Notify', source, 'This race does not exist :(', 'error')
    end
end)

RegisterServerEvent('vr-lapraces:server:UpdateRaceState')
AddEventHandler('vr-lapraces:server:UpdateRaceState', function(RaceId, Started, Waiting)
    Races[RaceId].Waiting = Waiting
    Races[RaceId].Started = Started
end)

RegisterServerEvent('vr-lapraces:server:UpdateRacerData')
AddEventHandler('vr-lapraces:server:UpdateRacerData', function(RaceId, Checkpoint, Lap, Finished)
    local src = source
    local Player = VRCore.Functions.GetPlayer(src)
    ...
    TriggerClientEvent('vr-lapraces:client:UpdateRaceRacerData', -1, RaceId, Races[RaceId])
end)

RegisterServerEvent('vr-lapraces:server:StartRace')
AddEventHandler('vr-lapraces:server:StartRace', function(RaceId)
    local src = source
    local MyPlayer = VRCore.Functions.GetPlayer(src)
    local AvailableKey = GetOpenedRaceKey(RaceId)
    ...
    if RaceId ~= nil then
        if AvailableRaces[AvailableKey].SetupCitizenId == MyPlayer.PlayerData.citizenid then
            ...
            for CitizenId,_ in pairs(Races[RaceId].Racers) do
                local Player = VRCore.Functions.GetPlayerByCitizenId(CitizenId)
                if Player ~= nil then
                    TriggerClientEvent('vr-lapraces:client:RaceCountdown', Player.PlayerData.source)
                end
            end
            TriggerClientEvent('vr-phone:client:UpdateLapraces', -1)
        else
            TriggerClientEvent('VRCore:Notify', src, 'You are not the creator of the race..', 'error')
        end
    else
...
RegisterServerEvent('vr-lapraces:server:SaveRace')
AddEventHandler('vr-lapraces:server:SaveRace', function(RaceData)
    local src = source
    local Player = VRCore.Functions.GetPlayer(src)
...
VRCore.Commands.Add("togglesetup", "Turn on / off racing setup", {}, false, function(source, args)
    local Player = VRCore.Functions.GetPlayer(source)

    if IsWhitelisted(Player.PlayerData.citizenid) then
        Config.RaceSetupAllowed = not Config.RaceSetupAllowed
        if not Config.RaceSetupAllowed then
            TriggerClientEvent('VRCore:Notify', source, 'No more races can be created!', 'error')
        else
            TriggerClientEvent('VRCore:Notify', source, 'Races can be created again!', 'success')
        end
    else
        TriggerClientEvent('VRCore:Notify', source, 'You have not been authorized to do this.', 'error')
    end
end)

VRCore.Commands.Add("cancelrace", "Cancel going race..", {}, false, function(source, args)
    local Player = VRCore.Functions.GetPlayer(source)
    ...
    if IsWhitelisted(Player.PlayerData.citizenid) then
        ...
        if RaceName ~= nil then
            ...
            if Races[RaceId].Started then
                ...
                TriggerClientEvent('VRCore:Notify', source, 'This race has not started yet.', 'error')
            end
        end
    else
        TriggerClientEvent('VRCore:Notify', source, 'You have not been authorized to do this.', 'error')
...
VRCore.Functions.CreateCallback('vr-lapraces:server:CanRaceSetup', function(source, cb)
