...
VRCore.Functions.CreateUseableItem("tunerlaptop", function(source, item)
    local src = source

    TriggerClientEvent('vr-tunerchip:client:openChip', src)
end)

RegisterServerEvent('vr-tunerchip:server:TuneStatus')
AddEventHandler('vr-tunerchip:server:TuneStatus', function(plate, bool)
    if bool then
...
VRCore.Functions.CreateCallback('vr-tunerchip:server:HasChip', function(source, cb)
    local src = source
...
VRCore.Functions.CreateCallback('vr-tunerchip:server:GetStatus', function(source, cb, plate)
    cb(tunedVehicles[plate])
end)

VRCore.Functions.CreateUseableItem("nitrous", function(source, item)
