...
RegisterNUICallback('save', function(data)
    VRCore.Functions.TriggerCallback('vr-tunerchip:server:HasChip', function(HasChip)
        if HasChip then
            local ped = PlayerPedId()
            local veh = GetVehiclePedIsUsing(ped)
            setVehData(veh, data)
            VRCore.Functions.Notify('TunerChip v1.05: Vehicle Tuned!', 'error')

            TriggerServerEvent('vr-tunerchip:server:TuneStatus', GetVehicleNumberPlateText(veh), true)
        end
    end)
end)

RegisterNetEvent('vr-tunerchip:server:TuneStatus')
AddEventHandler('vr-tunerchip:server:TuneStatus', function()
    local ped = PlayerPedId()
    ...
    VRCore.Functions.TriggerCallback('vr-tunerchip:server:GetStatus', function(status)
        if status then
...
RegisterNUICallback('reset', function(data)
    ...
    local veh = GetVehiclePedIsUsing(ped)
    resetVeh(veh)
    VRCore.Functions.Notify('TunerChip v1.05: Vehicle has been reset!', 'error')
end)

RegisterNetEvent('vr-tunerchip:client:openChip')
AddEventHandler('vr-tunerchip:client:openChip', function()
    local ped = PlayerPedId()
    local inVehicle = IsPedInAnyVehicle(ped)

    if inVehicle then
        VRCore.Functions.Progressbar("connect_laptop", "Tunerchip v1.05: Vehicle Has Been Reset!", 2000, false, true, {
