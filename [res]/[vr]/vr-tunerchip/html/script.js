...
$(document).on('click', '#save', function(){
    $.post('https://vr-tunerchip/save', JSON.stringify({
...
$(document).on('click', "#save-neon", function(){
    if (RainbowNeon) {
        $.post('https://vr-tunerchip/saveNeon', JSON.stringify({
...
$(document).on('click', '#save-headlights', function(){
    $.post('https://vr-tunerchip/saveHeadlights', JSON.stringify({
        value: headlightVal,
...
$(document).on('click', '#save-stancer', function(){
    ...
    $.post('https://vr-tunerchip/SetStancer', JSON.stringify({
...
VRTuner.Open = function() {
    $.post('https://vr-tunerchip/checkItem', JSON.stringify({item: "tunerlaptop"}), function(hasItem){
        if (hasItem) {
...
VRTuner.Close = function() {
    $('.container').fadeOut(250);
    $.post('https://vr-tunerchip/exit');
}