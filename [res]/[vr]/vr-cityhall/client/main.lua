...
RegisterNetEvent('vr-cityhall:client:getIds')
AddEventHandler('vr-cityhall:client:getIds', function()
    TriggerServerEvent('vr-cityhall:server:getIDs')
end)

RegisterNetEvent('vr-cityhall:client:sendDriverEmail')
AddEventHandler('vr-cityhall:client:sendDriverEmail', function(charinfo)
...
RegisterNUICallback('requestId', function(data)
    if inRange then
        ...
        TriggerServerEvent('vr-cityhall:server:requestId', idTypes[idType])
        VRCore.Functions.Notify('You have recived your '..idTypes[idType].label..' for $50', 'success', 3500)
    else
        VRCore.Functions.Notify('This will not work', 'error')
    end
end)
...
RegisterNUICallback('applyJob', function(data)
    if inRange then
        if IsAvailableJob(data.job) then
            TriggerServerEvent('vr-cityhall:server:ApplyJob', data.job)
        else
            TriggerServerEvent('vr-cityhall:server:banPlayer')
            TriggerServerEvent("vr-log:server:CreateLog", "anticheat", "POST Request (Abuse)", "red", "** @everyone " ..GetPlayerName(player).. "** has been banned for abusing localhost:13172, sending POST request\'s")         
        end
    else
        VRCore.Functions.Notify('Unfortunately will not work ...', 'error')
    end
end)
