...
AddEventHandler('apartments:server:CreateApartment', function(type, label)
    ...
    TriggerClientEvent('VRCore:Notify', src, "You got a apartment ("..label..")")
...
AddEventHandler('apartments:server:UpdateApartment', function(type)
    ...
    TriggerClientEvent('VRCore:Notify', src, "You have changed apartments")
    TriggerClientEvent("apartments:client:SetHomeBlip", src, type)
end)
...
VRCore.Functions.CreateCallback('apartments:GetAvailableApartments', function(source, cb, apartment)
    local apartments = {}
...
VRCore.Functions.CreateCallback('apartments:GetApartmentOffset', function(source, cb, apartmentId)
    local retval = 0
...
VRCore.Functions.CreateCallback('apartments:GetApartmentOffsetNewOffset', function(source, cb, apartment)
...
VRCore.Functions.CreateCallback('apartments:GetOwnedApartment', function(source, cb, cid)
    if cid ~= nil then
...
VRCore.Functions.CreateCallback('apartments:IsOwner', function(source, cb, apartment)
	local src = source
    local Player = VRCore.Functions.GetPlayer(src)
    if Player ~= nil then
        local result = exports.ghmattimysql:executeSync('SELECT * FROM apartments WHERE citizenid=@citizenid', {['@citizenid'] = Player.PlayerData.citizenid})
...
VRCore.Functions.CreateCallback('apartments:GetOutfits', function(source, cb)
	local src = source
	local Player = VRCore.Functions.GetPlayer(src)
...
RegisterServerEvent('vr-apartments:server:SetInsideMeta')
AddEventHandler('vr-apartments:server:SetInsideMeta', function(house, insideId, bool)
    local src = source
    local Player = VRCore.Functions.GetPlayer(src)
