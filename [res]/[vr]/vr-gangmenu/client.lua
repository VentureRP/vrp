...
AddEventHandler('VRCore:Client:OnGangUpdate', function(GangInfo)
    PlayerGang = GangInfo
end)
...
RegisterNetEvent('vr-gangmenu:client:openMenu')
AddEventHandler('vr-gangmenu:client:openMenu', function()
    MenuV:OpenMenu(menu)
end)
...
menu_button6:On("select", function()
    ...
    if result ~= nil then
        TriggerServerEvent("vr-gangmenu:server:withdrawMoney", tonumber(result))
        UpdateSociety()
    end
end)
...
menu_button7:On("select", function()
    ...
    if result ~= nil then
        TriggerServerEvent("vr-gangmenu:server:depositMoney", tonumber(result))
        UpdateSociety()
    end
end)
...
menu_button:On("select", function()
    menu3:ClearItems()
    VRCore.Functions.TriggerCallback('vr-gangmenu:server:GetEmployees', function(cb)
...
menu_button1:On("select", function()
    ...
    for k,v in pairs(VRCore.Functions.GetPlayersFromCoords(GetEntityCoords(playerPed), 10.0)) do
        if v and v ~= PlayerId() then
            local menu_button10 = menu4:AddButton({
                ...
                select = function(btn)
                    local select = btn.Value
                    TriggerServerEvent('vr-gangmenu:server:giveJob', GetPlayerServerId(v))
                end
            })
        end
    end
...
function UpdateSociety()
    VRCore.Functions.TriggerCallback('vr-gangmenu:server:GetAccount', function(cb)
        ...
        local menu_button9 = manageroptions:AddButton({
            ...
            select = function(btn)
                ...
                if values == 'promote' then
                    ...
                    if result ~= nil then
                        TriggerServerEvent('vr-gangmenu:server:updateGrade', employee.source, tonumber(result))
                    end
                else
                    TriggerServerEvent('vr-gangmenu:server:fireEmployee', employee.source)
                end
            end
        })
    end
