...
RegisterServerEvent('vr-storerobbery:server:takeMoney')
AddEventHandler('vr-storerobbery:server:takeMoney', function(register, isDone)
    local src = source
	...
	TriggerClientEvent('inventory:client:ItemBox', src, VRCore.Shared.Items['markedbills'], "add")
        if math.random(1, 100) <= 10 then
            ...
            TriggerClientEvent('inventory:client:ItemBox', src, VRCore.Shared.Items["stickynote"], "add")
        end
    end
end)

RegisterServerEvent('vr-storerobbery:server:setRegisterStatus')
AddEventHandler('vr-storerobbery:server:setRegisterStatus', function(register)
    Config.Registers[register].robbed   = true
    Config.Registers[register].time     = Config.resetTime
    TriggerClientEvent('vr-storerobbery:client:setRegisterStatus', -1, register, Config.Registers[register])
end)

RegisterServerEvent('vr-storerobbery:server:setSafeStatus')
AddEventHandler('vr-storerobbery:server:setSafeStatus', function(safe)
    TriggerClientEvent('vr-storerobbery:client:setSafeStatus', -1, safe, true)
    Config.Safes[safe].robbed = true

    SetTimeout(math.random(40, 80) * (60 * 1000), function()
        TriggerClientEvent('vr-storerobbery:client:setSafeStatus', -1, safe, false)
        Config.Safes[safe].robbed = false
    end)
end)

RegisterServerEvent('vr-storerobbery:server:SafeReward')
AddEventHandler('vr-storerobbery:server:SafeReward', function(safe)
    local src = source
	local Player = VRCore.Functions.GetPlayer(src)
	local bags = math.random(1,3)
	local info = {
    ...
    if luck <= 10 then
        ...
        TriggerClientEvent('inventory:client:ItemBox', src, VRCore.Shared.Items["rolex"], "add")
        if luck == odd then
            Citizen.Wait(500)
...
RegisterServerEvent('vr-storerobbery:server:callCops')
AddEventHandler('vr-storerobbery:server:callCops', function(type, safe, streetLabel, coords)
    local cameraId = 4
    if type == "safe" then
    ...
    TriggerClientEvent("vr-storerobbery:client:robberyCall", -1, type, safe, streetLabel, coords)
...
Citizen.CreateThread(function()
    while true do
        ...
        if #toSend > 0 then
            ...
            TriggerClientEvent('vr-storerobbery:client:setRegisterStatus', -1, toSend, false)
        end
...
VRCore.Functions.CreateCallback('vr-storerobbery:server:isCombinationRight', function(source, cb, safe)
    cb(SafeCodes[safe])
end)

VRCore.Functions.CreateCallback('vr-storerobbery:server:getPadlockCombination', function(source, cb, safe)
    cb(SafeCodes[safe])
end)

VRCore.Functions.CreateCallback('vr-storerobbery:server:getRegisterStatus', function(source, cb)
    cb(Config.Registers)
end)

VRCore.Functions.CreateCallback('vr-storerobbery:server:getSafeStatus', function(source, cb)
    cb(Config.Safes)
end)
