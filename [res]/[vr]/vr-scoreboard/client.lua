...
AddEventHandler('VRCore:Client:OnPlayerLoaded', function()
    isLoggedIn = true
    VRCore.Functions.TriggerCallback('vr-scoreboard:server:GetConfig', function(config)
        Config.IllegalActions = config
    end)
end)

RegisterNetEvent('vr-scoreboard:client:SetActivityBusy')
AddEventHandler('vr-scoreboard:client:SetActivityBusy', function(activity, busy)
    Config.IllegalActions[activity].busy = busy
end)

RegisterCommand('scoreboard', function()
    if not scoreboardOpen then
        VRCore.Functions.TriggerCallback('vr-scoreboard:server:GetPlayersArrays', function(playerList)
            VRCore.Functions.TriggerCallback('vr-scoreboard:server:GetActivity', function(cops, ambulance)
                VRCore.Functions.TriggerCallback("vr-scoreboard:server:GetCurrentPlayers", function(Players)
