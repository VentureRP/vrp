        ...
        if not self.cardActive then
            ...
            exports.ghmattimysql:execute('UPDATE bank_cards SET cardnumber=@cardnumber, cardPin=@cardPin, cardDecrypted=@cardDecrypted, cardActive=@cardActive, cardLocked=@cardLocked, cardType=@cardType WHERE citizenid=@citizenid AND record_id=@record_id', {
                ...
                if self.source ~= -1 then
                    TriggerClientEvent('vr-banking:client:newCardSuccess', self.source, cardNumber, friendlyName)
                    local xPlayer = VRCore.Functions.GetPlayer(self.source)
...
RegisterServerEvent('vr-banking:server:registerCurrentAccount')
AddEventHandler('vr-banking:server:registerCurrentAccount', function(cid)
...
RegisterServerEvent('vr-banking:server:registerSavingsAccount')
AddEventHandler('vr-banking:server:registerSavingsAccount', function(cid)
