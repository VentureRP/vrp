...
AddEventHandler('jobs:client:ToggleNpc', function()
    if VRCore.Functions.GetPlayerData().job.name == "tow" then
        if CurrentTow ~= nil then 
            VRCore.Functions.Notify("First Finish Your Work", "error")
            return
        end
        NpcOn = not NpcOn
        if NpcOn then
...
RegisterNetEvent('vr-tow:client:TowVehicle')
AddEventHandler('vr-tow:client:TowVehicle', function()
    local vehicle = GetVehiclePedIsIn(PlayerPedId(), true)
    if isTowVehicle(vehicle) then
        if CurrentTow == nil then 
            --[[ Replaced "VRCore.Functions.GetClosestVehicle()" with custom implementation "getVehicleInDirection"
                 VRCore native could not return polcice and other vehicles types (NPC) ]] 
            ...
            if NpcOn and CurrentLocation ~= nil then
                if GetEntityModel(targetVehicle) ~= GetHashKey(CurrentLocation.model) then
                    VRCore.Functions.Notify("This Is Not The Right Vehicle", "error")
                    return
                end
            end
            if not IsPedInAnyVehicle(PlayerPedId()) then
                if vehicle ~= targetVehicle then
                    ...
                    if #(towPos - targetPos) < 11.0 then
                        VRCore.Functions.Progressbar("towing_vehicle", "Hoisting the Vehicle...", 5000, false, true, {
                            ...
                            if NpcOn then
                                RemoveBlip(CurrentBlip)
                                VRCore.Functions.Notify("Take The Vehicle To Hayes Depot", "success", 5000)
                                ...
                                if chance < 26 then
                                    TriggerServerEvent('vr-tow:server:nano')
                                end
                            end
                            VRCore.Functions.Notify("Vehicle Towed")
                        end, function() -- Cancel
                            StopAnimTask(PlayerPedId(), "mini@repair", "fixing_a_ped", 1.0)
            ...
            VRCore.Functions.Progressbar("untowing_vehicle", "Remove The Vehicle", 5000, false, true, {
                disableMovement = true,
                disableCarMovement = true,
                disableMouse = false,
                disableCombat = true,
                ...
                CurrentTow = nil
                VRCore.Functions.Notify("Vehicle Taken Off")
            end, function() -- Cancel
                StopAnimTask(PlayerPedId(), "mini@repair", "fixing_a_ped", 1.0)
                VRCore.Functions.Notify("Failed", "error")
            end)
        ...
        VRCore.Functions.Notify("You Must Have Been In A Towing Vehicle First", "error")
    end
end)
...
Citizen.CreateThread(function()
    ...
    while true do 
        Citizen.Wait(1)
        if isLoggedIn and VRCore ~= nil then
            if PlayerJob.name == "tow" then
                    ...
                    if #(pos - vector3(Config.Locations["vehicle"].coords.x, Config.Locations["vehicle"].coords.y, Config.Locations["vehicle"].coords.z)) < 1.5 then
                        ...
                        if IsControlJustReleased(0, 38) then
                            if IsPedInAnyVehicle(PlayerPedId(), false) then
                                ...
                                TriggerServerEvent('vr-tow:server:DoBail', false)
                            else
                                MenuGarage()
                ...
                if #(pos - vector3(Config.Locations["main"].coords.x, Config.Locations["main"].coords.y, Config.Locations["main"].coords.z)) < 4.5 then
                    if #(pos - vector3(Config.Locations["main"].coords.x, Config.Locations["main"].coords.y, Config.Locations["main"].coords.z)) < 1.5 then
                        ...
                        if IsControlJustReleased(0, 38) then
                            if JobsDone > 0 then
                                RemoveBlip(CurrentBlip)
                                TriggerServerEvent("vr-tow:server:11101110", JobsDone)
                                JobsDone = 0
                                NpcOn = false
                            else
                                VRCore.Functions.Notify("You Havent Done Any Work Yet", "error")
                            end
                        end
                ...
                if NpcOn and CurrentLocation ~= nil and next(CurrentLocation) ~= nil then
                    if #(pos - vector3(CurrentLocation.x, CurrentLocation.y, CurrentLocation.z)) < 50.0 and not VehicleSpawned then
                        VehicleSpawned = true
                        VRCore.Functions.SpawnVehicle(CurrentLocation.model, function(veh)
...
function deliverVehicle(vehicle)
    ...
    JobsDone = JobsDone + 1
    VehicleSpawned = false
    VRCore.Functions.Notify("You Have Delivered A Vehicle", "success")
    VRCore.Functions.Notify("A New Vehicle Can Be Picked Up")
...
function TakeOutVehicle(vehicleInfo)
    TriggerServerEvent('vr-tow:server:DoBail', true, vehicleInfo)
    selectedVeh = vehicleInfo
end

RegisterNetEvent('vr-tow:client:SpawnVehicle')
AddEventHandler('vr-tow:client:SpawnVehicle', function()
