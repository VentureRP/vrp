...
RegisterServerEvent("vr-pawnshop:server:sellPawnItems")
AddEventHandler("vr-pawnshop:server:sellPawnItems", function()
    local src = source
    local price = 0
        ...
        for k, v in pairs(Player.PlayerData.items) do 
            if Player.PlayerData.items[k] ~= nil then 
                if ItemList[Player.PlayerData.items[k].name] ~= nil then 
                    ...
                    TriggerClientEvent('inventory:client:ItemBox', source, VRCore.Shared.Items[Player.PlayerData.items[k].name], "remove")
                end
            end
        end
        Player.Functions.AddMoney("cash", price, "sold pawnable items")
        TriggerClientEvent('VRCore:Notify', src, "You have sold your items")
    end
end)

RegisterServerEvent("vr-pawnshop:server:sellHardwarePawnItems")
AddEventHandler("vr-pawnshop:server:sellHardwarePawnItems", function()
    local src = source
    local price = 0
    local Player = VRCore.Functions.GetPlayer(src)
    if Player.PlayerData.items ~= nil and next(Player.PlayerData.items) ~= nil then 
        for k, v in pairs(Player.PlayerData.items) do 
            if Player.PlayerData.items[k] ~= nil then 
                if ItemListHardware[Player.PlayerData.items[k].name] ~= nil then 
                    ...
                    Player.Functions.RemoveItem(Player.PlayerData.items[k].name, Player.PlayerData.items[k].amount, k)
                    TriggerClientEvent('inventory:client:ItemBox', src, VRCore.Shared.Items[Player.PlayerData.items[k].name], "remove")
                end
            end
        end
        Player.Functions.AddMoney("cash", price, "sold pawnable items")
...
RegisterServerEvent("vr-pawnshop:server:getGoldBars")
AddEventHandler("vr-pawnshop:server:getGoldBars", function()
    local src = source
    local Player = VRCore.Functions.GetPlayer(src)
    if GoldBarsAmount > 0 then
        if Player.Functions.AddItem("goldbar", GoldBarsAmount) then
            ...
            Config.MeltTime = 300
            TriggerClientEvent("vr-pawnshop:client:SetTakeState", -1, false)
        else
            TriggerClientEvent('VRCore:Notify', src, "You have no space in your inventory", "error")
        end
    end
end)

RegisterServerEvent("vr-pawnshop:server:sellGold")
AddEventHandler("vr-pawnshop:server:sellGold", function()
    local src = source
    local Player = VRCore.Functions.GetPlayer(src)
    local price = 0
    if Player.PlayerData.items ~= nil and next(Player.PlayerData.items) ~= nil then 
...
RegisterServerEvent("vr-pawnshop:server:meltItems")
AddEventHandler("vr-pawnshop:server:meltItems", function()
    local src = source
    local Player = VRCore.Functions.GetPlayer(src)
    local goldbars = 0
    if Player.PlayerData.items ~= nil and next(Player.PlayerData.items) ~= nil then 
        for k, v in pairs(Player.PlayerData.items) do 
            if Player.PlayerData.items[k] ~= nil then 
                if MeltItems[Player.PlayerData.items[k].name] ~= nil then 
                    ...
                    if amount < 1 then
                        TriggerClientEvent('VRCore:Notify', src, "You do not have enough " .. Player.PlayerData.items[k].label, "error")
                    else
        ...
        if goldbars > 0 then
            ...
            TriggerClientEvent('vr-pawnshop:client:startMelting', -1)
            ...
            Citizen.CreateThread(function()
                while Config.IsMelting do
                    ...
                    if Config.MeltTime <= 0 then
                        ...
                        Config.CanTake = true
                        Config.MeltTime = 300
                        TriggerClientEvent('vr-pawnshop:client:SetTakeState', -1, true)
                    end
                    Citizen.Wait(1000)
                end
            end)
...
VRCore.Functions.CreateCallback('vr-pawnshop:server:getSellPrice', function(source, cb)
    local retval = 0
...
VRCore.Functions.CreateCallback('vr-pawnshop:melting:server:GetConfig', function(source, cb)
    cb(Config.IsMelting, Config.MeltTime, Config.CanTake)
end)

VRCore.Functions.CreateCallback('vr-pawnshop:server:getSellHardwarePrice', function(source, cb)
    local retval = 0
    local Player = VRCore.Functions.GetPlayer(source)
    if Player.PlayerData.items ~= nil and next(Player.PlayerData.items) ~= nil then 
        for k, v in pairs(Player.PlayerData.items) do 
...
VRCore.Functions.CreateCallback('vr-pawnshop:server:hasGold', function(source, cb)
	local retval = false
    local Player = VRCore.Functions.GetPlayer(source)
