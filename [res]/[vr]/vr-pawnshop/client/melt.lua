...
AddEventHandler("VRCore:Client:OnPlayerLoaded", function()
    VRCore.Functions.TriggerCallback('vr-pawnshop:melting:server:GetConfig', function(IsMelting, MeltTime, CanTake)
                ...
                if not Config.IsMelting then
                    if Config.CanTake then
                        ...
                        if IsControlJustReleased(0, 38) then
                            TriggerServerEvent("vr-pawnshop:server:getGoldBars")
                        end
                    else
                        ...
                        if IsControlJustReleased(0, 38) then 
                            ...
                            ScrapAnim(1000)
                            VRCore.Functions.Progressbar("drop_golden_stuff", "Grab Items", 1000, false, true, {
                                ...
                                if not Config.IsMelting then
                                    ...
                                    TriggerServerEvent("vr-pawnshop:server:meltItems")
                                end
                            end)
                        end
                    end
                ...
                if GetClockHours() >= 9 and GetClockHours() <= 18 then
                    ...
                    elseif sellItemsSet and hasGold then
                        ...
                        if IsControlJustReleased(0, 38) then
                            ...
                            VRCore.Functions.Progressbar("sell_gold", "Selling Gold", lockpickTime, false, true, {
                                disableMovement = true,
                                disableCarMovement = true,
                                disableMouse = false,
                                disableCombat = true,
                                ...
                                TriggerServerEvent('vr-pawnshop:server:sellGold')
                            end, function() -- Cancel
                                openingDoor = false
                                ClearPedTasks(PlayerPedId())
...
function HasPlayerGold()
	local retval = false
	VRCore.Functions.TriggerCallback('vr-pawnshop:server:hasGold', function(result)
		retval = result
	end)
    Citizen.Wait(500)
	return retval
...
RegisterNetEvent('vr-pawnshop:client:startMelting')
AddEventHandler('vr-pawnshop:client:startMelting', function()
    if not Config.IsMelting then
...
RegisterNetEvent('vr-pawnshop:client:SetTakeState')
AddEventHandler('vr-pawnshop:client:SetTakeState', function(state)
